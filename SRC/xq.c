...
/*  0  = interior, 1 = edge, 2 = corner // irregular at bound
 * < 0 = quad doesnt exist
 * NOTE: quad is boundary if it has at least one vertex type > 0
 * even if all its adjacents are != -1. It happens for high valences
 */
static int EG_quadIsBoundary(meshMap *qm, int qID)
{
  int i, q, v, bV = 0 , bQ = 0;

  if (qID < 0 || qID > qm -> totQ) return EGADS_INDEXERR;
  for (i = 0; i < 4; i++) {
      v  = qm ->qIdx[4 * (qID - 1) + i];
      q  = qm ->qAdj[4 * (qID - 1) + i];
...
static void EG_commonVerts(meshMap *qm, int q1, int q2, int *v)
{
  ...
  v[0] = 0;
#ifdef DEBUG
  if (q1 < 0 || q2 < 0 || q1 > qm -> totQ || q2 > qm -> totQ ||
	  qm -> qIdx[4 * ( q1 - 1 )] == -2 ||
	  qm -> qIdx[4 * ( q2 - 1 )] == -2 ) {
  ...
  for (k = i = 0; i < 4; i++) {
      for (j = 0; j < 4; j++) {
	  if (qm -> qIdx[4 * (q1 - 1) + i] == qm -> qIdx[4 * (q2 - 1) + j])
	    v[++k] = qm -> qIdx[4 * (q2 - 1) + j];
      }
  }
  v[0] = k;
...
static int EG_normalProjection (meshMap *qm, double *uv, double *normal,
				int n, double *pos, double *pxyz ) {
  int  i;
  double eval[18], c, dotNN, norm1 = 0.0, norm2 = 0.0, du[3], dv[3], alpha;
  normal[0] = normal[1] = normal[2] = 0.0;
  if (EG_evaluate(qm->face, uv, eval) == EGADS_SUCCESS) {
      norm1 = eval[3] * eval[3] + eval[4] * eval[4] + eval[5] * eval[5];
      norm2 = eval[6] * eval[6] + eval[7] * eval[7] + eval[8] * eval[8];
  }
  if (norm1 < EPS11 || norm2 < EPS11) {
      for ( i = 0 ; i < n; i++ ) {
	  if (EG_evaluate(qm->face, &pos[2 * i], eval) != EGADS_SUCCESS ) continue;
	  pxyz[3 * i    ] = eval[0];
	  pxyz[3 * i + 1] = eval[1];
	  pxyz[3 * i + 2] = eval[2];
      }
  ...
  du[0] = eval[3]; du[1] = eval[4]; du[2] = eval[5];
  dv[0] = eval[6]; dv[1] = eval[7]; dv[2] = eval[8];
  ...
  for (i      = 0; i < n; i++) {
      if (EG_evaluate(qm->face, &pos[2 * i], eval) != EGADS_SUCCESS ) continue;
      alpha           = (c - DOT(normal, eval)) / dotNN;
      pxyz[3 * i    ] = eval[0] + alpha * normal[0];
      pxyz[3 * i + 1] = eval[1] + alpha * normal[1];
      pxyz[3 * i + 2] = eval[2] + alpha * normal[2];
  }
  return EGADS_SUCCESS;
}
...
{
  int    stat;
  double dot, normal[3], cross[3], proj[6];
  double centre[18], v01[3], v02[3], norm1, norm2, pos[4];

  pos[0] = qm -> uvs[2 * (v1 - 1)    ];
  pos[1] = qm -> uvs[2 * (v1 - 1) + 1];
  pos[2] = qm -> uvs[2 * (v2 - 1)    ];
  pos[3] = qm -> uvs[2 * (v2 - 1) + 1];
  stat      = EG_evaluate(qm -> face, &qm -> uvs[2 * (vC - 1)], centre);
  stat     += EG_normalProjection(qm, &qm -> uvs[2 * (vC - 1)], normal, 2, pos, proj );
  if (stat != EGADS_SUCCESS ) {
      printf(" NORMAL PROJECTION IN EG_angleAtVnormal plane failed !!\n ");
      return stat;
  }
  v01[0] = proj[0] - centre[0];
  v01[1] = proj[1] - centre[1];
  v01[2] = proj[2] - centre[2];
  v02[0] = proj[3] - centre[0];
  v02[1] = proj[4] - centre[1];
  v02[2] = proj[5] - centre[2];
  norm1  = DOT(v01, v01);
  norm2  = DOT(v02, v02);
  CROSS       (v01, v02, cross);
  dot   = DOT(v01, v02) / sqrt ( norm1 * norm2 );
  if      (dot >=  1.0)       *angle = 0.0;
  else if (dot <= -1.0)       *angle = PI;
  else                        *angle = acos(dot);
  if (DOT(normal, cross) < 0) *angle =  2.0 * PI - *angle;
  return EGADS_SUCCESS;
}
...
static int EG_quadAngleOrientation(meshMap *qm, double minT, double maxT,
                                   int qID, int *ori, int *order,
                                   double *theta, double *quadSize)
{
  int    i, k, k1, k2, sign, stat, vA, vB, vC, area = 0, count = 0, doublet = 0;
  int    qV[4], pos, selfint = 1;
  double projABCD[12], cross[3], qNormal[3], vAB[3], vAC[3], quv[2] ;
  double signedAngle, norm1, norm2, norm3, angle, dot, size, uvs[8];
  ...
  uvs[0] = qm -> uvs [ 2 * qV[0]    ];
  uvs[1] = qm -> uvs [ 2 * qV[0] + 1];
  uvs[2] = qm -> uvs [ 2 * qV[1]    ];
  uvs[3] = qm -> uvs [ 2 * qV[1] + 1];
  uvs[4] = qm -> uvs [ 2 * qV[2]    ];
  uvs[5] = qm -> uvs [ 2 * qV[2] + 1];
  uvs[6] = qm -> uvs [ 2 * qV[3]    ];
  uvs[7] = qm -> uvs [ 2 * qV[3] + 1];
  quv[0] = 0.25 * (uvs[0] + uvs[2] + uvs[4] + uvs[6] );
  quv[1] = 0.25 * (uvs[1] + uvs[3] + uvs[5] + uvs[7] );
  stat   = EG_normalProjection(qm, quv, qNormal, 4, uvs, projABCD );
  ...
  for (pos   = doublet = count = sign = k = 0; k < 4; ++k) {
	  ...
	  for (k1    = 1; k1 < 3; k1++) {
	      ...
	      norm3  = sqrt(cross[0] * cross[0] + cross[1] * cross[1] +
		            cross[2] * cross[2]);
	      size  += 0.5 * norm3;
...
theta[k] += angle;
  ...
  if      (count                                      == 4) area = 1;
  else if ((doublet == 1 || qm -> pp == 1) && selfint == 0) area = 2;
  else area = 0;
  for (k = 0; k < 3; k++) {
    for (i = k + 1; i < 4; i++) {
      k1 = 0;
...
static void EG_computeCoords(meshMap *qm, int vID, double minAngle,
                             double maxAngle, /*@null@*/ int *areas)
{
  ...
  for (k = i = 0; i < star -> nQ; i++) {
    if (areas == NULL)
      j = EG_quadAngleOrientation(qm, minAngle, maxAngle, star -> quads[i],
...
{
  int          f, stat = 0, stat2 = 0, j, q, i, auxID, k, kk, kOK, len, ne;
...
{
  int    d, stat;
  double normal[4], proj[6], cross[3], dt, duv[2], xyz0[18];
  double prevt[2], v1[3], v2[3], pos[4];
  ...
  for (d    = 0; d < 2; d++) {
      ...
      pos[2]    = t1[0];
      pos[3]    = t1[1];
      stat      = EG_normalProjection(qm, t0, normal, 2, pos, proj );
      stat     += EG_evaluate(qm -> face, t0, xyz0);
      if (stat != EGADS_SUCCESS) {
#ifdef DEBUG
	  printf("EG_rotationUV :: stat for new postition %d!!\n",  stat);
#endif
	  t1[0] = prevt[0]; t1[1] = prevt[1];
	  return;
      }
      ...
      v2[2] = proj[5] - xyz0[2];
      CROSS(v1, v2, cross);
      if (DOT(normal, cross) * (double) counterClockWise > 0)
	return;
      else {
	  if (d == 1) {
	      t1[0] = prevt[0]; t1[1] = prevt[1];
                                 ...
                                 int qID, int smooth )
{
  ...
  double dista, distb, maxT, minT, uv[2], totDist = 0.0, qsize;
  vStar  *star = NULL;
  ...
  for (it  = 0; it < itMAX; it++) {
    area = EG_quadAngleOrientation(qm, minAngle, maxAngle, qID, ori, piv,
                                   angles, &qsize );
    ...
    if (doublet == 0) {
      for (j = 0; j < 4; j++) {
        ...
        EG_computeCoords(qm, k + 1, minAngle, maxAngle, sa);
        for (i = 0; i < star -> nQ; i++) {
          if (star -> quads[i] == qID) continue;
...
static int EG_makeValidMesh(meshMap *qm, int nP, /*@null@*/ int *pList,
                            int fullReg)
{
  int    i, j, kq, kv, q, k, it = 0, itMax, ori[4], piv[4];
  int    *quads = NULL, *verts = NULL, recover = 0, stat = EGADS_SUCCESS;
  double dthetam, dthetaM, minT, maxT, angles[4], *uvxyz = NULL, quv[8], dist, qsize;
#ifdef DEBUG
  char   buffer[100];
#endif
  vStar  *star = NULL;
  ...
  if (fullReg == 0) { // move around only affected vertices
    ...
    qm -> vFix[0] = 0;
    for (i = 0; i < kq; i++) {
      if (EG_makePositiveAngles(qm, 0.0, PI, quads[i], 0) <= 0) {
        qm -> vFix[0] = 0;
        recover       = 1;
        break;
      }
    ...
    if (recover == 0) {
      for (i = 0; i < kv; i++) {
        dist = (uvxyz[5 * (verts[i] - 1)    ] - qm -> uvs[2 * (verts[i] - 1)    ]) *
               (uvxyz[5 * (verts[i] - 1)    ] - qm -> uvs[2 * (verts[i] - 1)    ]) +
               (uvxyz[5 * (verts[i] - 1) + 1] - qm -> uvs[2 * (verts[i] - 1) + 1]) *
               (uvxyz[5 * (verts[i] - 1) + 1] - qm -> uvs[2 * (verts[i] - 1) + 1]);
        if (dist < 1.e-08 || qm -> vType[verts[i] - 1] != -1) continue;
        ...
        for (j = 0; j < star -> nQ; j++) {
          if (star -> quads[j] == -1) continue;
          if (EG_quadAngleOrientation(qm, 0.0, PI, star -> quads[j],
                                      ori, piv, angles, &qsize) <= 0) {
            recover = 1;
            break;
          }
        }
    ...
    for (it = 0; it < itMax; it++) {
      ...
      for (q = 0; q < qm -> totQ; q++) {
        ...
        if (EG_makePositiveAngles(qm, minT, maxT, q + 1, 1) != 1) continue;
        for (k = 0 ; k < 4; k++) {
          kv = qm -> qIdx[4 * q + k] - 1;
...
static int EG_createQuadGroup(meshMap *qm, quadGroup *qg, int q0, int q1)
{
  ...
  centre = common[1];
  if (qm -> valence[common[2] -1][1] >
      qm -> valence[common[1] -1][1]) centre = common[2];
...
static int EG_swappingOperation(meshMap *qm, quadGroup qg, int swap,
                                int *activity)
{
  int    area, nq, stat, i0, i1, i, j, q, adj, idx[4], qID[2], adjQmap[6];
  int    piv[4], ori[4];
  ...
  *activity = 0;
  ...
  for (j  = 0; j < 2; j++) {
      ...
      area = EG_quadAngleOrientation(qm, DEG10, PI, qID[q],
                                     ori, piv, angles, &qsize);
      if (area <  0) return area;
      if (area == 0) EG_computeCoords(qm, i, DEG10, PI, NULL);
  }
  *activity = 1;
                                 ...
                                 int *activity)
{
  ...
  *activity   = 0;
  ...
  EG_computeCoords(qm, poly[0], DEG10, PI, NULL);
  EG_computeCoords(qm, poly[3], DEG10, PI, NULL);
  *activity = 1;
...
static int EG_mergeVertices(meshMap *qm, int qC, int centre, int *activity)
{
  ...
  *activity  = 1;
  if (qm ->vType  [oldQ[1] - 1]    == -1 &&
    ...
    EG_computeCoords(qm, oldQ[1], 0.0, PI, NULL);
  if (qm ->vType  [oldQ[3] - 1]    == -1 &&
      qm ->valence[oldQ[3] - 1][2] ==  2)
    EG_computeCoords(qm, oldQ[3], 0.0, PI, NULL);
...
int EG_meshRegularization(meshMap *qm)
{
  ...
  for (k = it = 0; it < ITMAX; it++) {
    ...
    for (i = 0; i < qm -> totQ; i++) {
      if (qm -> qIdx [ 4 * i ] == -2) continue;
      q = EG_quadAngleOrientation(qm, DEG20, PI, i + 1, ori, piv, angle, &qsize);
      if (q == 1 &&
          qsize >= qm -> avsize * qm -> avsize * (double)(it + 1)/(double)ITMAX) continue;
#ifdef DEBUG
      printf(" quad %d area %d is below average %lf < %lf ! \n ", i + 1, q,
...
#ifdef REPORT
	      printf("\n\n======================================\n");
	      printf(" Quad Regularization FACE %d \n ", f + 1);
	      printf("\n\n======================================\n");
#endif
