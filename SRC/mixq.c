...
static int EG_setValence(meshMap *qm, int vID)
{
  ...
  if (i != EGADS_SUCCESS || qm->star[si] == NULL) {
      printf(" In EG_setValence for vertex %d stat from EG_buildStar = %d\n", vID, i);
      return i;
  }
                                  ...
                                  double *normal, double *angle)
{
  ...
  double cross[3], proj1[3], proj2[3], lambda, xyz[18], v01[3],
         v02[3], norm1, norm2, c, uv[2], dotNP;
  ...
  *angle = 0.0;
  ...
  if (norm1 < qEPS || tc == 1) {
      ...
      if (norm1 < qEPS) {
          ...
          printf("EG_angAtNPlane VERT %d TYPE %d LINK TO %d %d "
...
//  c = DOT(normal, xyz);
  ...
  if (sing == 0 ) {
    if (t1 <= 0 && t1 + tc <= -1) {
			if (EG_evaluate(qm->face, &qm->uvs[2 * (v1  - 1)], xyz) == EGADS_SUCCESS) {
     ...
     if (t2 <= 0 && t2 + tc <= -1) {
  		if (EG_evaluate(qm->face, &qm->uvs[2 * (v2  - 1)], xyz) == EGADS_SUCCESS) {
    			 dotNP    = DOT(normal, xyz);
           lambda   = (c - dotNP);
...
static int EG_vertexArea(meshMap *qm, int vID)
{
  ...
  double pQ[12], cross[3], vNormal[6], vAB[3], vAC[3], vAD[3], nXYZ[18], xyz[18], tria[8], vr[2], uv[2];
...
#ifdef DEBUG2
  printf(" NORMAL %lf %lf %lf ANGLE %lf ORI %d\n", vNormal[0], vNormal[1], vNormal[2], ang[k], ori[k]);
#endif
      ...
      if (qm->star[vID-1]->area[s] != QA0) {
          ...
          else if (qm->star[vID-1]->area[s] % QA2 == 0 )
              ...
              if (qm->star[v]->ratio[j] < 0.0)
                  ...
                  if (corner == j) {
                      ...
                      EG_angAtNPlane (qm, qm->star[v]->verts[la],
                                              qm->star[v]->verts[la + 1], vID, normal, &errbd);
#ifdef DEBUG2
                      ...
                      EG_angAtNPlane(qm, qm->star[v]->verts[la], vID,
                                             qm->star[v]->verts[v1], normal, &errbd);
#ifdef DEBUG2
