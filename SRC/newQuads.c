...
static int validBoundary ( meshMap *qm, double minAngle, double maxAngle ) {
  int v, i, j, k, kv, aux, aux2, q, stat, links[2], i0, nv, dir, area, orient[4], piv[4] ;
  double range[4], eval[18], angleAtV, thetaRot, cross[4], normal[4], angles[4], tAB[4], tn[4], proj[6], vec[8], segAv;
  vStar *star = NULL;
  ...
  for ( v = 0 ; v < qm -> mesh -> totVerts; v++ ) {
	...
	vec[i] = proj[i] - qm -> mesh -> xyzs [ 3 * v + i];
	  ...
	  if ( area != 1 ) j = 1;
		  ...
		  if (angles [ piv[k] ] > maxAngle || angles[ piv[k] ] < minAngle ) j = 1;
	      }
	  }
	  ...
	  if ( dir  ==  1 ) thetaRot *= -1.0;
	  angles[1]  = 2 * thetaRot;
	  printf(" VERTEX %d type %d \n ", aux, qm -> mesh -> vType [ aux ] );
	  printVertexCoords (qm, aux + 1 );
	  weightedAverage   (qm, aux + 1);
	  printVertexCoords (qm, aux + 1 );
	  k          = star -> verts [ star -> idxV [ 2 * (i0 + j ) + 1 ] ] - 1;
	  angles[2]  = EG_angleAtVnormalPlane ( qm, star -> verts[0],    k + 1, aux + 1);
	  angles[3]  = EG_angleAtVnormalPlane ( qm, star -> verts[0], links[0], aux + 1);
	  if ( angles[2] < PI && angles[3] > angles[0] && angles[3] < angles[1] )  continue;
	  ...
	  tn[3]      = tAB[1] + tn[1];
	  segAv      = 0.0;
	  for ( kv = k = 0 ; k < qm -> mesh -> valence[aux][0]; k++ ) {
	      aux2 = qm -> mesh -> valence[aux][2 + k] - 1;
	      if ( qm -> mesh -> vType [aux2] != -1 ) {
		  segAv += EG_segment ( qm, &qm -> mesh -> uvs [ 2 * aux ], & qm -> mesh -> uvs [ 2 * aux2 ]);
		  kv++;
	      }
	  }
	  if ( kv >= 2 ) {
	      segAv  /= kv;
	      stat  = moveDistAway ( qm, segAv, tAB, &tn[2] );
	  }
	  else stat = moveDistAway ( qm, qm -> minsize * 0.5, tAB, &tn[2] );
	  stat      = EG_evaluate ( qm -> face, &tn[2], eval);
	  printf(" Evaluate %lf %lf --> %lf %lf %lf \n ", tn[2], tn[3], eval[0], eval[1], eval[2]);
	  if ( stat != EGADS_SUCCESS ) {
      ...
      printMesh (qm, buffer,0);
      for (j = 0 ; j < star -> nQ ; j++ ) {
	  aux = star -> verts [ star -> idxV [ 2 * j + 1] ] - 1;
	  if ( aux < 0 || qm -> mesh -> vType [ aux ] != -1 ) continue;
	  weightedAverage (qm, aux + 1);
      }
      EG_freeStar (&star);
  }
...
static int validInterior(meshMap *qm, int qID, double minAngle, double maxAngle ) {
  int    vj, k, stat, area, links[2], orient[4], piv[4], it = 0, itMAX = 20, aux ;
  double angles[4];
  ...
  if ( area == 2 || ( area == 1 && (angles [ piv[0] ] > maxAngle || angles[ piv[3] ] < minAngle ))) area = 0 ;
  if ( area != 1 ) area = 0;
  else {
      for ( k = 0 ; k < 4; k++ ) {
  ...
  while ( area == 0 && it < itMAX ) {
	  ...
	  if (  vertexLinksToBounds (qm, vj ) == 4 ) continue;
...
static int optimize_angles(meshMap *qm, int nP, /*@unused@*/ /*@null@*/int *pList, int fullRegularization)
{
  ...
  double dthetam, dthetaM, minT, maxT;
  vStar *star = NULL;
