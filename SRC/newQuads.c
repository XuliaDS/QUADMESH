...
typedef struct {
  int      fID, oriQ, oriV, minQ, extraQuads, smooth;
  ego      face;
...
static double EG_angleAtPlaneUsingUVs ( meshMap *qm, int vC, double *uv1, double *uv2);
...
static int    vertexLinksToBounds     (meshData *mesh, int vID );
...
static void   weightedAverage         (meshMap *qm, int vC, int *fix );
...
static int  EG_reduceAngle            (meshMap *qm,  double dtheta, int iA, int iB, int iC, int reverse);
...
static int
vertexFixedToBounds (meshMap *qm, int vID, int **bounds ) {
  int i, stat, j = 0;
  vStar *star   = NULL;
  stat           = EG_buildStar ( qm -> mesh, &star, vID ) ;
  if ( star      == NULL || stat != EGADS_SUCCESS) return stat;
  (*bounds)       = (int * )EG_alloc ( (1 + star -> nQ ) * sizeof (int ) );
  if ((*bounds)  == NULL ) {
      EG_freeStar ( & star );
      return EGADS_MALLOC;
  }
  for ( (*bounds)[0] = i = 0; i < star -> nQ; i++) {
      if ( qm -> mesh -> vType [ star -> verts[2 * i + 1] -1 ] >= 0 ) {
	  (*bounds)[ ++(*bounds)[0] ]   = i;
      }
  }
  printf(" TOTAL BOUND %d \n ", (*bounds)[0] );
  //if ( star ->nQ == 4 && ( (*bounds)[0] > 2 || ( (*bounds)[0] == 2 && abs ( (*bounds)[1] - (*bounds)[2] ) != 1
  //	&& abs ( (*bounds)[1] - (*bounds)[2] ) != 3 ))) j = 1;
  if ( (*bounds)[0] > 2 || ( (*bounds)[0] == 2 && abs ( (*bounds)[1] - (*bounds)[2] ) != 1
      && abs ( (*bounds)[1] - (*bounds)[2] ) != 3 )) j = 1;
  for ( i = 0 ; i < (*bounds)[0]; i++ )
    (*bounds)[1 + i ] = star -> verts [ 2 * (*bounds)[1 + i] + 1 ];
  EG_freeStar ( &star);
  return j;
}
...
EG_reduceAngle ( meshMap *qm, double theta, int iA, int iB, int iC, int reverse) {
  int stat, d, i, a, v, counterClockWise = 1,  *nb = NULL ;
  double eval[18], vec[8], t[8], normal[4], proj[6], cross[4], range[4], dt, lenBC, lenAV;
  t[0] = qm -> mesh -> uvs [ 2 * (iA - 1)    ];
  t[1] = qm -> mesh -> uvs [ 2 * (iA - 1) + 1];
  stat = EG_normalToSurface (qm, t, normal);
  if ( stat != EGADS_SUCCESS ) {
      printf("EG_rotateVertex :: EG_normalToSurface at %d ->%d!!\n", iA, stat);
      return stat;
  }
  if ( reverse == 1 ) counterClockWise *= -1;
  printf(" REDUCE ANGLE %d %d %d reverse %d  \n ", iA, iB, iC, reverse);
  lenBC = EG_segment (qm, &qm -> mesh -> uvs [ 2 * (iB - 1)],
		      &qm -> mesh -> uvs [ 2 * (iC - 1)]);
  for ( a = 0 ; a < 2; a++) {
      counterClockWise *= -1;
      if ( a == 0 ) v = iB - 1;
      ...
      i = vertexFixedToBounds ( qm, v + 1, &nb ) ;
      EG_free ( nb);
      if ( qm -> smooth > 0 && i  == 1 )  continue;
      if ( a == 0 )
	printf("# iB %d --> %f %f %f \n ", iB, qm -> mesh -> xyzs [ 3 * v],
	...
	printf("# iC %d --> %f %f %f \n ", iC, qm -> mesh -> xyzs [ 3 * v],
	       qm -> mesh -> xyzs [ 3 * v + 1], qm -> mesh -> xyzs [ 3 * v + 2] );
      t[2]  = qm -> mesh -> uvs [ 2 * v    ];
      t[3]  = qm -> mesh -> uvs [ 2 * v + 1];
      lenAV = EG_segment (qm, &qm -> mesh -> uvs [ 2 * (iA - 1)],
			  &qm -> mesh -> uvs [ 2 * v ]);
      stat = EG_projectToTangentPlane(normal, &qm -> mesh -> xyzs [ 3 * (iA - 1)],
				      &qm -> mesh -> xyzs [ 3 * v ], proj);
      for ( i = 0 ; i < 3; i++ )
	vec[i] = proj[i] - qm -> mesh -> xyzs [ 3 * (iA - 1) + i];
      if ( stat != EGADS_SUCCESS ) {
	  printf("EG_rotateVertex :: EG_projectToTangentPlane %d at %d -> %d!!\n", iA, iB, stat);
	  return stat;
      }
      stat = EG_getRange ( qm -> face, range, &i);
      dt   = lenBC / lenAV * theta;
      for ( d = 0 ; d < 2; d++ ) {
	  if ( d == 1 ) dt *= -1.0;
	  t[4] = ( t[2] - t[0] ) * cos ( dt ) - ( t[3] - t[1] ) * sin ( dt );
	  t[5] = ( t[2] - t[0] ) * sin ( dt ) + ( t[3] - t[1] ) * cos ( dt );
	  t[6] =   t[0] + t[4];
	  t[7] =   t[1] + t[5];
	  if ( t[6] < range[0] || t[6] > range[1] || t[7] < range[2] || t[7] > range[3] ) {
	      printf(" OUT OF RANGE \n ");
	      continue;
	      if ( d == 0 ) continue;
	  ...
	  stat       = EG_evaluate ( qm -> face, &t[6], eval);
	  if ( stat != EGADS_SUCCESS ) {
	      if ( d == 0 ) continue;
	      printf(" Both directions seem invalid! BUG!\n");
	      return EGADS_GEOMERR;
	  }
	  stat = EG_projectToTangentPlane(normal, &qm -> mesh -> xyzs [ 3 * (iA - 1)], eval, &proj[3]);
	  if ( stat != EGADS_SUCCESS ) {
	      printf("EG_rotateVertex :: EG_projectToTangentPlane rotation %d -> %d!!\n", iA, stat);
	      return stat;
	  }
	  for ( i = 0 ; i < 3; i++ )
	    vec[4 + i] = proj[3 + i] - qm -> mesh -> xyzs [ 3 * (iA - 1) + i];
	  unitVector    (&vec[4], &vec[7] );
	  cross_product (vec, &vec[4], cross);
	  if ( dotProduct (normal, cross ) * (double)counterClockWise > 0 ) break;
	  else {
	      if ( d == 1 ) {
      ...
      updateVertex (qm, v + 1, &t[6]);
      if ( a == 0 )
	printf("#ROTATED iB %d --> %f %f %f \n ", iB, qm -> mesh -> xyzs [ 3 * v],
	       qm -> mesh -> xyzs [ 3 * v + 1], qm -> mesh -> xyzs [ 3 * v + 2] );
      else
	printf("#ROTATED iC %d --> %f %f %f \n ", iC, qm -> mesh -> xyzs [ 3 * v],
	       qm -> mesh -> xyzs [ 3 * v + 1], qm -> mesh -> xyzs [ 3 * v + 2] );
  }
  return stat;
}
...
static int validCollapse ( meshData *mesh, int qID, int v ) {
  ...
  if ( mesh -> vType [ aux -1 ] != -1 && vertexLinksToBounds ( mesh, v ) != 0 ) return 0;
  for ( j = 0 ; j < 2; j++ ) {
...
static void weightedAverage (meshMap *qm, int vID, int *fix ) {
  int i, j, auxID = 0, stat, *nB = NULL, links[2], nI, i0, l1, l2;
  double uvc[2], angle, optangle, edgeSize[2], ang1, ang2, rotate;
  vStar * star = NULL;
  *fix = 0;
  ...
  j      = vertexFixedToBounds (qm, vID, &nB);
  printf("========== weighted Average coordinates %d fix %d smooth %d \n ", vID, j, qm -> smooth ) ;
  if ( j == 1 && qm -> smooth  >0 ) {
      for ( i   = 0; i < nB[0]; i++) {
	  uvc[0] += qm -> mesh -> uvs[2*( nB[ i + 1] - 1 )    ];
	  uvc[1] += qm -> mesh -> uvs[2*( nB[ i + 1] - 1 ) + 1];
      }
      i = nB[0];
  } else {
      for ( i     = 0; i < star -> nQ; i++) {
  ...
  if ( qm -> smooth > 0 && ( j == 0 || ( j == 1 && star -> nQ > 4 )) ) {
      for ( j = 0 ; j < nB[0]; j++ ) {
	  stat  = EG_buildStar ( qm -> mesh, &star, nB[j + 1] );
	  if ( star  == NULL || stat != EGADS_SUCCESS) return;
	  stat      = EG_angleAtBoundaryVertex (qm, nB[j + 1], links, &angle);
	  ...
	  auxID       = nB [ j + 1 ] -1;
	  l1          = star -> verts [ star -> idxV [ 2 * ( i0 + i     ) + 1 ]] - 1;
	  l2          = star -> verts [ star -> idxV [ 2 * ( i0 + i + 2 ) + 1 ]] - 1;
	  edgeSize[0] = EG_segment (qm, &qm -> mesh -> uvs [ 2 * auxID], &qm -> mesh -> uvs [ 2 * l1] );
	  edgeSize[1] = EG_segment (qm, &qm -> mesh -> uvs [ 2 * auxID], &qm -> mesh -> uvs [ 2 * l2] );
	  printf(" CENTRE %d size WRT %d %d == %f  %f\n ", auxID + 1, l1 + 1, l2 + 1, edgeSize[0], edgeSize[1] );
	  if (stat != EGADS_SUCCESS ) {
	  ...
	  ang1     = EG_angleAtVnormalPlane ( qm, auxID + 1, links[0], vID);
	  ang2     = EG_angleAtVnormalPlane ( qm, auxID + 1, l1 + 1,   vID);
	  printf(" Angle WRT ORIGIN: %d %d %d --> %f \t Angle WRT PREVIOUS : %d %d %d --> %f\n",
		 links[0], auxID + 1, vID, ang1, l1 + 1, auxID + 1, vID, ang2 );
	  if (ang2 > PI || ( qm -> smooth != 2 && fabs ( ang1 - optangle ) > PI * 0.25 ) ) {
	      *fix = 1;
	      printf( " angle at %d %d %d ---> %f Opti %f\n ", links[0], auxID + 1, vID, ang1, optangle );
	      rotate = PI * 0.05 ;
	      if ( qm -> smooth ==2 ) rotate = 0.02 * PI ;
	      if ( ang1 > optangle )
		stat    = EG_reduceAngle(qm, rotate, auxID + 1, links[0], vID, 1);
	      else stat = EG_reduceAngle(qm, rotate, auxID + 1, links[0], vID, 0);
	      ang1     = EG_angleAtVnormalPlane ( qm, auxID + 1, links[0], vID);
	      ang2     = EG_angleAtVnormalPlane ( qm, auxID + 1, l1 + 1, vID);
	      printf(" After adjust angle WRT ORIGIN: %d %d %d --> %f \t Angle WRT PREVIOUS : %d %d %d --> %f\n",
		     links[0], auxID + 1, vID, ang1, l1 + 1, auxID + 1, vID, ang2 );
	      stat = moveDistAway (qm, 0.85, auxID + 1, vID);
	  }
	  ang1 = EG_segment (qm, &qm -> mesh -> uvs [ 2 * auxID], &qm -> mesh -> uvs [ 2 * ( vID - 1 )] );
	  if ( ang1 < qm -> minsize * 0.25 )
	    stat = moveDistAway (qm, 0.0, auxID + 1, vID);
	  else if (qm -> smooth != 2 && (ang1 > edgeSize[0] || ang1 > edgeSize[1] )) {
	      printf(" segment %f  sizes %f  %f REDUCE!!!!\n ", ang1, edgeSize[0], edgeSize[1] );
	      stat = moveDistAway (qm, 0.9, auxID + 1, vID);
	  }
      }
  }
  EG_freeStar ( &star );
...
static int vertexLinksToBounds (meshData *mesh, int vID ) {
  int i, j, k = 0;
  if ( mesh -> vType[ vID -1 ] != -1 ) k = 1;
  for (i = 0 ; i < mesh -> valence[ vID -1 ][0]; i++ ) {
      j  =  mesh -> valence[ vID -1 ][2 + i] - 1;
      if ( mesh -> vType[ j ] >= 4 )  return 4;
      else if ( mesh -> vType[ j ] != -1 ) k = 1;
  }
  return k;
}
...
static int validInterior(meshMap *qm, int qID,  double minAngle, double maxAngle ) {
  int    vj, k,i, bd, stat, area, links[2], orient[8], piv[8], it = 0, itMAX = 20, aux, boundlink ;
  double angles[8], dtheta = 0.05 * PI;
  printf("\n\n-------------------------------------------------\n");
  printf("-----------  Interior CheckInvalid quad %d min max angles %lf %lf------------\n", qID, minAngle, maxAngle );
  printf("-------------------------------------------------\n");
  if ( EG_quadIsBoundary (qm -> mesh, qID ) != 0 ) return EGADS_SUCCESS;
  for ( boundlink = k = 0 ; k < 4; k++ )
    if (vertexLinksToBounds (qm -> mesh, qm -> mesh -> quadIdx [ 4 * (  qID -1 ) + k ] ) > 0 ) boundlink = 1;
  stat = quadAngleOrientation(qm, qID, &area, orient, piv, angles);
  if ( stat != EGADS_SUCCESS ) {
      printf("In checkInvalidElement: quadAlgebraic area at %d is %d !!\n", qID, stat);
      return stat;
  }
  if ( area == 2 && maxAngle <= PI  ) area = 0;
  else if ( area == 1 ) {
      if ( qm -> smooth > 0 ) {
	  for ( k = 0 ; k < 4; k++ ) {
	      aux = qm -> mesh -> quadIdx [ 4 * ( qID -1 ) + piv[k] ];
	      printf(" Invalid interior: weighted average about %d\n ", aux ) ;
	      if (vertexLinksToBounds (qm -> mesh, aux) == 0 ) weightedAverage (qm, aux, &i);
	  }
	  stat = quadAngleOrientation(qm, qID, &area, orient, piv, angles);
	  if ( area ==1 ) return EGADS_SUCCESS;
      }
      if ( boundlink == 1 ) return EGADS_SUCCESS;
      for ( k = 0 ; k < 4; k++ ) {
  ...
  while ( area == 0 && it < itMAX ) {
      if ( it %10 == 0 ) printMesh(qm, buffer, 0 );
      for  ( k = 0; k < 4; k++) {
	  ...
	  printf("\n\n ITERATION %d k = %d piv %d  Looking at %d linked to %d %d \n\n\n ", it,  k, piv[k], vj, links[0], links[1] );
	  if ( vertexLinksToBounds (qm -> mesh, vj) > 0 ) {
	      printf(" Vertex Links to bounds !! \n ");
	      for ( bd = i = 0 ; i < qm -> mesh -> valence[vj-1][0]; i++) {
		  bd = qm -> mesh -> valence[vj-1][2 + i];
		  if ( qm -> mesh -> vType [ bd -1 ] != -1 ) break;
	      }
	      stat += moveDistAway (qm, 0.95, bd, vj);
	      if (vertexLinksToBounds (qm -> mesh, links[0]) == 0 &&
		  vertexLinksToBounds (qm -> mesh, links[1]) == 0 )
		stat = EG_reduceAngle(qm, dtheta, vj, links[0], links[1], 1);
	      else if ( qm -> smooth == 0 ) {
		  for ( bd = i = 0 ; i < qm -> mesh -> valence[links[0]-1][0]; i++) {
		      bd = qm -> mesh -> valence[links[0]-1][2 + i];
		      if ( qm -> mesh -> vType [ bd -1 ] != -1 ) break;
		  }
		  stat += moveDistAway (qm, 0.95, bd, links[0] );
		  for ( bd = i = 0 ; i < qm -> mesh -> valence[links[1]-1][0]; i++) {
		      bd = qm -> mesh -> valence[links[1]-1][2 + i];
		      if ( qm -> mesh -> vType [ bd -1 ] != -1 ) break;
		  }
		  stat += moveDistAway (qm, 0.95, bd, links[1] );

	      }

	  } else if ( orient [piv[k]] == -1  ) {
	      printQuadCoords (qm, qID );
	      averageCoordsMinusLinks (qm, vj, links[0], links[1]);
	      printQuadCoords (qm, qID );
	  }
	  else if ( angles[ piv[k]] < minAngle && minAngle < PI * 0.25) { //}||
	      //(   vertexLinksToBounds (qm -> mesh, links[0]) == 1 ||
	      //vertexLinksToBounds (qm -> mesh, links[1]) == 1 ) )   {
	      printQuadCoords (qm, qID );
	      averageCoordsUsingLinks (qm, vj, links[0], links[1]);
	      printQuadCoords (qm, qID );
	  }
	  else weightedAverage ( qm, vj, &i ) ;
	  stat = quadAngleOrientation(qm, qID, &area, &orient[4], &piv[4], &angles[4]);
	  if ( area == 2 && maxAngle < PI ) area = 0;
	  else if ( area == 1 ) {
	      if ( qm -> smooth > 0 ) break;
	      for ( i = 4 ; i < 7; i++ ) {
		  aux = qm -> mesh -> quadIdx [ 4 * (  qID -1 ) + piv[i] ] - 1;
		  if (angles [ 4 + piv[i] ] > maxAngle || angles[4 + piv[i] ] < minAngle ) {
		      printf(" piv %d has %f > %f < %f  \n", piv[i], angles[4 + piv[i]],minAngle, maxAngle);
		      area = 0 ;
		  }
	      }
	  }
...
static int nBoundLinks ( meshMap *qm, int v ){
  int i, j, nb;
  if ( qm -> mesh -> vType[ v - 1 ] != -1 ) return -1;
  for ( nb = i = 0; i < qm -> mesh -> valence[v -1 ][0]; i++ ) {
      j = qm -> mesh -> valence[v -1 ][2 + i] -1;
      if (qm -> mesh -> vType[j] >= 0 ) nb++;
  }
  return nb;
}
...
static int validBoundary ( meshMap *qm, double minAngle, double maxAngle ) {
  int iA, iB, iC, iD, iP, v, fixB, fixC, fixD,i, j, q, qq, stat = 0 , aux, i0 = -1, area, it,  itMAX = 10, ok, nI, *vfix = NULL;
  int  *nb = NULL, *nc = NULL, *nd = NULL, aLR[2], orient[4], piv[4];
  double  angleAtV, edgeSize[2], angles[4], angleA, angleB, angleC, angleD, seg, uvback[8], optiAngle, dtheta = 0.05 * PI ;
  vStar *star = NULL;
  printf("\n\n=============================================================\n");
  printf(" VALID BOUNDARY CHECK  MIN MAX ANGLE %f  %f  \n ", minAngle, maxAngle );
  printf("=============================================================\n");
  printMesh(qm, buffer,0);
  vfix      = (int * ) EG_alloc ( qm -> mesh -> totVerts * sizeof (int) );
  if ( vfix == NULL ) return EGADS_MALLOC;
  vfix[0]   = 0;
  for ( v = 0 ; v < qm -> mesh -> totVerts; v++ ) {
      if ( qm -> mesh -> vType[v] < 0 ) continue;
      iA        = v + 1;
      printf(" VALID BOUNDARY FOR VERTEX %d SMOOTHING %d \n ", iA, qm -> smooth );
      stat      = EG_buildStar (qm ->mesh, &star, iA);
      if (stat != EGADS_SUCCESS || star == NULL ) goto cleanup;
      stat      = EG_angleAtBoundaryVertex (qm, iA, aLR, &angleAtV);
      if (stat != EGADS_SUCCESS ) {
	  printf("validBoundary: EG_angleAtBoundaryVertex %d --> %d!!\n ", iA, stat );
	  goto cleanup;
      }
      for  ( nI = q = 0; q < star -> nQ; q++ ) {
	  if ( star -> quads [q] == -1 ) i0 = q + 1;
	  else if (qm ->mesh -> vType[ star -> verts [ 2 * q + 1 ] - 1] == -1 ) nI++;
      }
      iP        = aLR[0];
      optiAngle = angleAtV / (double)( nI + 1 );
      for ( qq  = 0; qq < star -> nQ; qq++ )
	{
	  q     = star -> idxQ [ qq + i0 ] ;
	  if ( star -> quads[q] == -1 ) break;
	  printf("q = %d  Quad %d check orientation \n ", q, star -> quads[q] );
	  ...
	  if ( area != 1 ) j = 1 ;
	      ...
	      if ( qm -> smooth > 0 ) {
		  printf(" WE ARE GOING TO SMOOTHEN QUAD \n");
		  for ( i = 0 ; i < 4; i++ ) {
		      aux = qm -> mesh -> quadIdx [ 4 * (  star -> quads[q] -1 ) + i ] - 1;
		      printf("\n\n Q%d [%d] = %d\n ", star -> quads[q], i, aux + 1 );
		      uvback [ 2 * i    ] = qm -> mesh -> uvs [ 2 * aux    ];
		      uvback [ 2 * i + 1] = qm -> mesh -> uvs [ 2 * aux + 1];
		      if ( vfix[0] > 0 ) {
			  for ( iB = 1 ; iB <= vfix[0]; iB++ )
			    if ( vfix[iB] == aux + 1 ) aux = -10;
		      }
		      if ( aux == -10 ) continue;
		      printf(" VERTEX %d MOVES \n", aux + 1 );
		      weightedAverage (qm, aux + 1, &iB);
		      if ( iB == 1 )  vfix[++vfix[0]] = aux + 1;
		  }
		  printMesh(qm ,buffer, 0);
		  stat = quadAngleOrientation(qm, star -> quads[q], &area, orient, piv, angles);
		  if ( area == 1 ) {
		      printf(" CONTINUE \n");
		      iP = star -> verts [ star -> idxV [ 2 * q + 1 ] ];
		      continue;
		  }
		  printf(" bad idea. Reset coordinates\n");
		  for ( i = 0 ; i < 4; i++ ) {
		      aux = qm -> mesh -> quadIdx [ 4 * (  star -> quads[q] -1 ) + i ] - 1;
		      vfix[++vfix[0]] = aux + 1;
		      updateVertex (qm, aux + 1, &uvback[ 2 * i ]);
		  }
		  printMesh(qm ,buffer, 0);
		  stat = quadAngleOrientation(qm, star -> quads[q], &area, orient, piv, angles);
		  for ( i = 0 ; i < 4; i++ ) {
		      aux = qm -> mesh -> quadIdx [ 4 * (  star -> quads[q] -1 ) + piv[i] ] - 1;
		      if (qm -> mesh -> vType[aux  ] != -1 || qm -> mesh -> valence[aux][0] != 4 ) continue;
		      if (angles [ piv[i] ] > maxAngle || angles[ piv[i] ] < minAngle ) j = 1;
		  }
	      }
	  }
	  if (j == 0 ) {
	  ...
	  printQuadSpecs (qm -> mesh, star -> quads[q] );
	  itMAX = 10;
	  iB   = star -> verts [ star -> idxV [ 2 * q + 1 ] ];
	  iC   = star -> verts [ star -> idxV [ 2 * q + 2 ] ];
	  iD   = star -> verts [ star -> idxV [ 2 * q + 3 ] ];
	  fixB = vertexFixedToBounds ( qm, iB, &nb );
	  fixC = vertexFixedToBounds ( qm, iC, &nc );
	  fixD = vertexFixedToBounds ( qm, iD, &nd );
	  if ( qm -> smooth == 0 || area == 0  )  {
	      fixB = 0; fixC = 0; fixD = 0;
	  }
	  edgeSize[0] = EG_segment (qm, &qm -> mesh -> uvs [ 2 * ( iA - 1 ) ], &qm -> mesh -> uvs [ 2 * ( iP - 1 )] );
	  edgeSize[1] = EG_segment (qm, &qm -> mesh -> uvs [ 2 * ( iA - 1 ) ], &qm -> mesh -> uvs [ 2 * ( iD - 1 )] );
	  for ( it = 0 ; it < itMAX; it++ ) {
	      ok = 0;
	      printf("\n\n************** Inside quad %d at boundary::: CENTRE AT %d : it %d \n ", star -> quads[q], iA, it );
	      if ( qm -> smooth != 2) {
		  if ( qm -> mesh -> vType [ iB -1 ] == -1 && fixB == 0  ) {
		      seg   = EG_segment (qm, &qm -> mesh -> uvs [ 2 * ( iA - 1 ) ], &qm -> mesh -> uvs [ 2 * ( iB - 1 )] );
		      printf(" segment ab %f size at edges %f  %f   nlinks %d \n ", seg, edgeSize[0], edgeSize[1], nBoundLinks (qm, iB));
		      if ( seg > edgeSize[0] && seg > edgeSize[1]  ) {
			  printf(" REDUCE SIZE of %d MOVING %f \n ", iB , seg * 0.95 );
			  stat += moveDistAway (qm, 0.95, iA, iB);
		      }
		  }
		  if ( qm -> mesh -> vType [ iC -1 ] == -1 && fixC == 0  ) {
		      seg   = EG_segment (qm, &qm -> mesh -> uvs [ 2 * ( iA - 1 ) ], &qm -> mesh -> uvs [ 2 * ( iC - 1 )] );
		      printf(" segment ab %f size at edges %f  %f   nlinks %d \n ", seg, edgeSize[0], edgeSize[1], nBoundLinks (qm, iC));
		      if ( seg > edgeSize[0] && seg > edgeSize[1]  ) {
			  printf(" REDUCE SIZE of %d MOVING %f \n ", iB , seg * 0.95);
			  stat += moveDistAway (qm, 0.95, iA, iC);
		      }
		  }
		  if ( qm -> mesh -> vType [ iD -1 ] == -1 && fixD == 0  ) {
		      seg   = EG_segment (qm, &qm -> mesh -> uvs [ 2 * ( iA - 1 ) ], &qm -> mesh -> uvs [ 2 * ( iD - 1 )] );
		      printf(" segment ab %f size at edges %f  %f   nlinks %d \n ", seg, edgeSize[0], edgeSize[1], nBoundLinks (qm, iD));
		      if ( seg > edgeSize[0] && seg > edgeSize[1]  ) {
			  printf(" REDUCE SIZE of %d MOVING %f \n ", iD , seg * 0.95 );
			  stat += moveDistAway (qm, 0.95, iA, iD);
		      }
		  }
	      }
	      printMesh( qm , buffer, 0);
	      // check that quad is valid //
	      if ( area != 1 ) {
		  angleA = EG_angleAtVnormalPlane ( qm, iA, iP, iB );
		  if ( angleA > PI ) {
		      ok = 1;
		      printf(" AngleA = %f OAB %d,%d,%d maxangle %f opti %f \n ",angleA, iP, iA, iB, maxAngle, optiAngle );
		      stat = EG_reduceAngle(qm, dtheta, iA, iP, iB, 1);
		      if ( stat != EGADS_SUCCESS ) {
			  printf("EG_rotateVertex OAB -> %d!!\n ", stat);
			  goto cleanup;
		      }
		      angleA = EG_angleAtVnormalPlane ( qm, iA, iP, iB );
		      if ( angleA > PI ) continue;
		  }
		  angleB = EG_angleAtVnormalPlane ( qm, iA, iB, iC );
		  angleC = EG_angleAtVnormalPlane ( qm, iA, iC, iD );
		  if ( angleB > PI && angleC < PI) {
		      ok = 1;
		      printf(" AngleB = %f BAC %d,%d,%d \n ",angleB, iB, iA, iC );
		      printf(" AngleC = %f CAD %d,%d,%d maxangle %f dtheta %f \n ",angleC, iC, iA, iD, PI, dtheta );
		      if ( fabs (angleB - PI ) < fabs ( angleB - 2.0 * PI ) )
			stat = EG_reduceAngle(qm, dtheta, iA, iB, iC , 1);
		      else
			stat = EG_reduceAngle(qm, dtheta, iA, iB, iC , 0 );
		      ok     = 1;
		      if ( stat != EGADS_SUCCESS ) {
			  printf("EG_rotateVertex OAB -> %d!!\n ", stat);
			  goto cleanup;
		      ...
		      angleB = EG_angleAtVnormalPlane ( qm, iA, iB, iC );
		      if ( angleB > PI ) continue;
		  }
		  angleC = EG_angleAtVnormalPlane ( qm, iA, iC, iD );
		  angleD = EG_angleAtVnormalPlane ( qm, iA, iB, iD );
		  if ( angleD > PI && angleC < PI ) {
		      printf(" Angle BAD %d %d %d  %f  \n ", iB, iA, iD, angleD);
		      stat = EG_reduceAngle(qm, dtheta, iA, iB, iD , 1 );
		      ok   = 1;
		      if ( stat != EGADS_SUCCESS ) {
			  printf("EG_rotateVertex OAB -> %d!!\n ", stat);
			  goto cleanup;
		      ...
		      angleD = EG_angleAtVnormalPlane ( qm, iA, iB, iD );
		      if (angleD > PI ) continue;
		  }
		  angleC = EG_angleAtVnormalPlane ( qm, iA, iC, iD );
		  if ( angleC > PI ) {
		      stat = EG_reduceAngle(qm, dtheta, iA, iC, iD, 0 );
		      ok   = 1;
		      if ( stat != EGADS_SUCCESS ) {
			  printf("EG_rotateVertex OAB -> %d!!\n ", stat);
			  return stat;
		      ...
		      angleC = EG_angleAtVnormalPlane ( qm, iA, iC, iD );
		      if ( angleC > PI ) continue;
		  }
		  angleC = EG_angleAtVnormalPlane ( qm, iC, iD, iB );
		  if ( angleC > PI )
		    averageCoordsMinusLinks(qm, iC, iD, iB);
	      } else {
		  if ( qm -> smooth != 1 ) break;
		  angleA = EG_angleAtVnormalPlane ( qm, iA, iP, iB );
		  if ( angleA > maxAngle || ( angleA > optiAngle
		      && nBoundLinks (qm, iB ) * nBoundLinks (qm, iP ) <= 1 ) ) {
		      stat = EG_reduceAngle(qm, dtheta, iA, iP, iB, 1);
		      if ( stat != EGADS_SUCCESS ) {
			  printf("EG_rotateVertex OAB -> %d!!\n ", stat);
			  goto cleanup;
		  ...
		  angleD = EG_angleAtVnormalPlane ( qm, iA, iB, iD );
		  if (angleD > maxAngle || ( angleD > optiAngle
		      && nBoundLinks (qm, iB ) * nBoundLinks (qm, iP ) <= 1 )  ) {
		      stat = EG_reduceAngle(qm,dtheta, iA, iB, iD, 0);
		      if ( stat != EGADS_SUCCESS ) {
			  printf("EG_rotateVertex OAB -> %d!!\n ", stat);
			  goto cleanup;
		      }
		  } else if ( angleD < optiAngle
		      && nBoundLinks (qm, iB ) * nBoundLinks (qm, iD ) <= 1 ) {
		      stat = EG_reduceAngle(qm, dtheta, iA, iB, iD , 1 );
		      if ( stat != EGADS_SUCCESS ) {
			  printf("EG_rotateVertex OAB -> %d!!\n ", stat);
			  goto cleanup;
		  ...
		  printf("\n AngleC = %f CDB %d,%d,%d maxAngle %f\n ",angleC, iC, iD, iB, maxAngle );
		  angleC = EG_angleAtVnormalPlane ( qm, iC, iD, iB );
		  if ( angleC > maxAngle ) {
		      if ( fixB == 0 ) {
			  printf(" MOVE B %d inwards \n", iB);
			  stat += moveDistAway (qm, 0.95, iA, iB);
			  if ( stat != EGADS_SUCCESS ) {
			      printf("EG_adjust vertex %d,%d -> %d!!\n ", iA, iB, stat);
			      EG_freeStar(&star);
			      return stat;
			  }
		      }
		      if ( fixC == 0 ) {
			  printf(" MOVE C = %d away \n", iC);
			  stat += moveDistAway (qm, 1.05, iA, iC);
			  if ( stat != EGADS_SUCCESS ) {
			      printf("EG_adjust vertex %d,%d -> %d!!\n ", iA, iC, stat);
			      goto cleanup;
			  }
  ...
  printf(" CHECK BOUNDS BEFORE LEAVING \n ");
  for ( q = 0 ; q < qm -> mesh -> totQuads; q++ ) {
      if ( checkQuad ( qm -> mesh, q + 1) != EGADS_SUCCESS ) continue;
      for ( iA = aux = i = 0 ; i < 4; i++ ) {
	  aux = qm -> mesh -> quadIdx [ 4 * q + i];
	  if ( vertexLinksToBounds ( qm -> mesh, aux ) > 0 ) iA = 1;
      }
      if ( EG_quadIsBoundary ( qm -> mesh, q + 1 ) == 0  ) {
	  if ( iA == 0 )continue;
	  printf(" IN BOUNDARY Check valid interior at quad %d \n", q + 1);
	  printQuadCoords (qm , q + 1);
	  stat = validInterior (qm, q + 1,  minAngle, maxAngle );
	  printQuadCoords (qm  , q+ 1);
	  if ( stat != EGADS_SUCCESS ) {
	      printf(" LEAVE VALID BOUNDARY %d is invalid\n", q+1);
	      //return stat;
	  }
      }
      printQuadCoords (qm, q+1 );
      stat  = quadAngleOrientation(qm, q + 1, &area, orient, piv, angles);
      if ( area == 0 ) {
	  printf(" LEAVE VALID BOUNDARY %d is invalid\n", q+1);
	  //return EGADS_GEOMERR;
      }
...
static double
EG_angleAtPlaneUsingUVs ( meshMap *qm, int vC, double *uv1, double *uv2) {
  int i, stat;
  double dot1, dot2, evalVert[18], normal[4], cross[3], proj0[3], proj1[3], proj2[3], v01[4], v02[4], eval1[18], eval2[18];
  ...
  stat = EG_projectToTangentPlane(normal, evalVert, eval1, proj1);
  stat = EG_projectToTangentPlane(normal, evalVert, eval2, proj2);
  for ( i = 0 ; i < 3 ; i++ ) {
...
EG_angleAtVnormalPlane ( meshMap *qm, int vC, int v1, int v2 ) {
  return EG_angleAtPlaneUsingUVs(qm,  vC, &qm -> mesh -> uvs[ 2 * ( v1 - 1 )], &qm -> mesh -> uvs[ 2 * ( v2 - 1 ) ]);
}
...
static int moveDistAway ( meshMap *qm, double alpha, int iA, int iB) {
  int    i, stat, it;
  double range[4], seg, uvm[2], uvcopy[4], norm = 0.0, uv0[2], uv1[2], eval[18], size, dt, segprev;
  if ( qm  -> mesh -> vType[iB -1] != -1 ) return EGADS_SUCCESS;
  ...
  printf("---- MOVEDISTAWAY   HOMOTECIA %d -%d  SEG %f target %f \n", iA, iB, seg, size);
  ...
  while ( seg < size ) {
      ...
      if (uvm[0] < range[0] || uvm[0] > range[1] ||
	  uvm[1] < range[2] || uvm[1] > range[3] ) return EGADS_SUCCESS;
      ...
      printf(" uv0 %f  %f uvm %f  %f  -> seg %f ize %f dt %f \n ",
	     uv1[0], uv1[1], uvm[0], uvm[1], seg, size, dt );
      if ( fabs ( segprev - seg ) < size / 10.0 ) dt += 0.05;
      segprev = seg;
  }
  uvcopy[2] = uv1[0]; uvcopy[3] = uv1[1];
  it        = 0 ;
  printf(" BISECTION: ORI%f TARGET %f error %f \n", seg, size, fabs( seg - size ) / size);
...
quadAngleOrientation(meshMap *qm, int qID, int *validArea, int *ori, int *order, double *theta ) {
  ...
  *validArea = 0;
  ...
  printf("------------ AREA QUAD %d  Internal angles at quad %d ordered by size --------------\n", *validArea, qID);
  for ( sign = k = 0 ; k < 4; ++k)
...
EG_swappingOperation(meshMap *qm, quadGroup qg, int swap ) {
  ...
  for ( j  = 0 ; j < 2; j++ ) {
      ...
      printf(" VERTEX %d type  %d linke %d \n", vc,
	     qm -> mesh -> vType [ vc - 1], vertexLinksToBounds (qm -> mesh, vc ) );
      if ( qm -> mesh -> vType [ vc - 1] != -1 || vertexLinksToBounds (qm -> mesh, vc ) == 4 ) continue;
      ...
      if ( area == 1 && ori[order[0]] == 1 && angles[order[0]] <= PI ) continue;
      i0   = qg.verts[(3 * j + 1)%6];
      i1   = qg.verts[(3 * j + 5)%6];
  ...
  for ( j = 1 ; j < 6; j++ ) {
      if ( j == 3 ) continue;
      if (vertexLinksToBounds (qm -> mesh, qg.verts[j] ) == 4 ) continue;
      weightedAverage (qm, qg.verts[j], &i);
  }
  printMesh(qm, buffer,0);
...
static int optimize_angles(meshMap *qm, int nP, /*@unused@*/ /*@null@*/int *pList, int fullRegularization)
{
  int  i, j, stat = EGADS_SUCCESS,  it = 0, itMax = 100;
  ...
  if ( fullRegularization == 0 ) { // move around only affected vertices
...
#endif
      for ( j = 0 ; j < nP; j++) {
	  if ( qm -> mesh -> vType [ pList[j] -1 ] != -1 ||
	      vertexLinksToBounds (qm -> mesh, pList[j]) != 0 ) continue;
	  stat = EG_buildStar (qm -> mesh, &star, pList[j]);
	  if ( stat != EGADS_SUCCESS || star == NULL ) return stat;
	  for ( i = 0 ; i < star -> nQ; i++ ) {
	      stat = validInterior ( qm, star -> quads[i], 0.0, PI + 0.25 * PI );
	      if ( stat != EGADS_SUCCESS ) {
      ...
      while ( it <= itMax ) {
	  if ( it > 5  ) qm -> smooth = 1;
	  if ( it > 15 ) qm -> smooth = 2;
	  for ( i = 0 ; i < qm -> mesh -> totQuads; i++) {
	      if (checkQuad ( qm -> mesh, i + 1) != EGADS_SUCCESS ||
		  EG_quadIsBoundary ( qm -> mesh, i+ 1) != 0 ) continue;
	      stat = validInterior ( qm, i + 1,  minT, maxT);
	  }
