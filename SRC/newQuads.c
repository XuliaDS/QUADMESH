...
EG_doubleCollapse ( meshMap *qm, quadGroup qg, int forcing, int *activity ) {
  ...
  *activity = 0;
  ...
  for ( i = 0 ; i < 2; i++ ) {
      printf(" Try collapsing at quad %d \n", qg.q[(q3+1)%2]);
      if ( validCollapse (qm -> mesh, qg.q[ ( q3 + i ) %2 ], qg.verts[i3] ) == 0 ) {
	  printf(" Collapse seems invalid \n");
	  return EGADS_SUCCESS;
      }
...
static int EG_swapSplit(meshMap *qm,quadGroup qg, int forcing, int *activity  ) {
  ...
  *activity = 0;
  if  ( qg.vals[0] * qg.vals[3] != 20 ) return EGADS_SUCCESS;
  if      (qg.vals[1] == 3 ) i3 = 1;
  else if (qg.vals[5] == 3 ) i5 = 5;
  for ( i = 1; i < 6; i++)
    if (qg.vals[i] == 5 ) i5 = i;
  if (      i3 != -1 && ( i5 == -1 || ( i5 != -1 && i5 == (i3 + 3)%6 ) )) v3opp = 3;
