...
static int checkVertex ( meshData *mesh, int v ) {
  int q, i;
  if ( v == -2 ||     mesh ->vType [ v - 1 ] == -2  ) return EGADS_EMPTY;
...
EG_createQuadGroup ( meshData *mesh, quadGroup *qg, int q0, int q1 ) {
  int i,  ids[2], piv = 0, aux, vaux[6], common[3], centre;
  qg -> q[0]     = q0;
  qg -> q[1]     = q1;
  for ( i = 0 ; i < 6; i++) {
      qg -> verts[i] = -1;
  ...
  if ( common[0] == 2 ) {
	...
	qg -> verts[i] = mesh -> quadIdx [ 4 * (qg ->q[piv] - 1) + (ids[piv] + i ) % 4 ];
      ...
      if ((   getValence (mesh, common[2]      ) == getValence (mesh, common[1] ) &&
	  ((( getValence (mesh, qg -> verts[4] ) != 4 || getValence (mesh, qg -> verts[2] ) != 4 ) &&
	      getValence (mesh, qg -> verts[1] ) == 4 && getValence (mesh, qg -> verts[5] ) == 4 ) ||
	      (   getValence (mesh,      common[2] ) != 4 && getValence (mesh,      common[1] ) == 4 ) )))
	{
  ...
  for ( i = 0 ; i < 6; i++ )
    qg -> vals  [i] = getValence ( mesh, qg->verts[i]);
  return EGADS_SUCCESS;
}
...
EG_doubleCollapse ( meshMap *qm, quadGroup qg, int forcing, int *activity ) {
  ...
  *activity = 0;
  if (qg.vals[0] * qg.vals[3] == 16 ) {
      ...
      else if ( forcing == 1 && qg.vals[(i3 + 3)%6] != 3 ) return EGADS_SUCCESS;
  }
  else if (qg.vals[0] * qg.vals[3] == 12 ) {
      for ( i5 = 0 ; i5 < 6; i5++ ) if ( qg.vals[i5]  == 5 ) break;
      if  (qg.vals[i5] != 5 || qg.vals[( i5 + 3) % 6] != 5 ) return EGADS_SUCCESS;
  ...
  q3  = 0; if ( i5 > 3 ) q3 = 1;
  for ( i = 0 ; i < 2; i++ ) {
      if ( validCollapse (qm -> mesh, qg.q[ ( q3 + i ) %2 ], i3 ) == 0 ) return EGADS_SUCCESS;
...
EG_swapDoubleSplit (meshMap *qm, quadGroup qg, int *activity ) {
  int  i5, q, i55, i3, i0, i,  stat, adj[2], adj2[2];
  *activity  = 0;
  ...
  stat = EG_adjQtoPair (qm -> mesh, qg.q[ ( q + 1 )%2], qg.verts[i3], qg.verts[i0], adj2 );
  *activity = 1;
  ...
  stat = EG_createQuadGroup (qm -> mesh, &qg,  qg.q[ ( q + 1 ) %2 ], adj2[1] );
  if ( stat != EGADS_SUCCESS ) {
...
static int EG_transferValences ( meshMap *qm, int *qID, int try5533, int *transfering, int *activity  ) {
  ...
  *activity   = 0;
  ...
  printf(" LEAVE TRANSFER WITH  %d  %d  TRANSFERRING %d !\n", qID[0], qID[1], *transfering );
  return stat;
}
...
static int EG_cleanQuad (meshMap *qm, int qID, int useAdj, int transfer, int forcing, int *activity ) {
  ...
  *activity      = 0;
  ...
  printf(" Inside clean quad %d using adj %d BASIC \n ", qID, useAdj );
  if ( transfer == 0 ) {
...
EG_forceCollapse ( meshMap *qm, int qID, int *activity ) {
  ...
  *activity  = 0;
  ...
  if (validCollapse (qm -> mesh, qID, centre) == 0) return EGADS_SUCCESS;
  *activity = 1;
...
EG_doubleSwap( meshMap *qm, quadGroup qg, int forcing, int *activity ) {
  ...
  *activity = 0;
  ...
  if      (qg. vals[0] == 3 || qg. vals[3] == 3 ) return EGADS_SUCCESS;
	  ...
	  piv3 = qg.vals[1]; if ( qg.vals[1] != 3 ) piv3 = qg.vals[5];
	  piv5 = (piv3 + 3)%6;
	  swap = piv3;
	  if ( forcing == 0 && getValence(qm -> mesh, qg.verts[piv5] ) != 4 ) return EGADS_SUCCESS;
	  fprintf(stderr,"DIAGONAL SWAPPING \n" );
      }
  }
  else if ( forcing == 1 ) {
  ...
  if ( piv3 != -1 ) {
	...
	if (auxQg.verts[i] == qg.verts[link5] ) break;
      if (  auxQg.vals [ ( i + 5 ) %6 ] <= 4 ) return EGADS_SUCCESS;
      fprintf(stderr, " DIAGONAL SWAPPING WORKS \n ");
  }
  else {
      for ( i = 0 ; i < 6; i++)
	if (  auxQg.verts[i] == v30 &&
	    ( auxQg.vals [ ( i + 3 ) %6 ] > 4 || qm -> mesh -> vType [ auxQg.verts[ ( i + 3 ) %6 ] -1 ] == 0 ) ) return EGADS_SUCCESS;
  }
...
static int EG_swapSplit(meshMap *qm,quadGroup qg, int forcing, int *activity  ) {
  int  stat, i, i3 = -1, i5 = -1, q5, vL5, vL5adj, swap = 0, adj[2];
  *activity   = 0;
  if (qg.vals[0] * qg.vals[3] != 20 &&
      qg.vals[0] * qg.vals[3] != 16 ) return EGADS_SUCCESS;
  if      ( qg.vals[1] == 3 ) i3 = 1;
  else if ( qg.vals[5] == 3 ) i3 = 5;
  if (i3 == -1 ) return EGADS_SUCCESS;
  if ( forcing ==1 ) printf("SWAP SPLIT FORCING SWAP\n");
  i5  = ( i3 + 3 ) % 6;
  vL5 = ( i5 + 1 ) % 6;
  if ( vL5 == 3 || vL5 == 0 ) vL5 = (i5 + 5) % 6;
  q5 = 0;
  if ( EG_quadVertIdx  (qm -> mesh, qg.q[q5], qg.verts[vL5] ) < 0 ) q5 = 1;
  stat = EG_adjQtoPair (qm -> mesh, qg.q[q5], qg.verts[i5], qg.verts[vL5], adj );
  if ( stat != EGADS_SUCCESS) exit(1);
  if ( adj[1] == -1 ) return EGADS_SUCCESS;
  i      = EG_quadVertIdx (qm -> mesh, adj[1], qg.verts[i5]);
  vL5adj = qm -> mesh -> quadIdx [ 4 * ( adj[1] - 1 ) + ( i + 1 ) %4 ];
  if ( EG_quadVertIdx (qm -> mesh, qg.q[q5], vL5adj) >= 0 )
    vL5adj = qm -> mesh -> quadIdx [ 4 * ( adj[1] - 1 ) + ( i + 3) %4 ];
  if      ( forcing    == 0 &&
      ((    qg.vals[0] == 5 && ( qg.vals[i5] == 5 || getValence (qm ->mesh, vL5adj) == 3 ) )||
	  ( qg.vals[0] == 4 && ( qg.vals[i5] == 5 && getValence (qm ->mesh, vL5adj) == 3 )) ) )                   swap = i3;
  else if ( forcing    == 1 && ( qg.vals[0]  == 5 || qg.vals[i5] == 5 || getValence (qm ->mesh, vL5adj) == 3 )  ) swap = i3;
  if ( swap == 0 ) return EGADS_SUCCESS;
  if ( validSwap (qm -> mesh, qg.verts[0], qg.verts[3] ) == 0 ) return EGADS_SUCCESS;
  printMesh(qm, buffer,0);
  stat       = EG_swappingOperation( qm, qg, swap );
...
static int EG_swapCollapse (meshMap *qm,quadGroup qg, int forcing, int *activity  ) {
  ...
  *activity   = 0;
  if (qg.vals[0] * qg.vals[3] != 16 ) return EGADS_SUCCESS;
  if (qg.vals[1] * qg.vals[2] == 15 ) {
	  ...
	  qg.vals[2] * qg.vals[5] == 15 ) {
	  i3 = 1; if ( qg.vals[1] != 3 && qg.vals[4] != 3 ) i3 = 2;
	  if ( qg.vals[i3] != 3 ) i3 = ( i3 + 3)%6;
	  printf(" 3,5 pair vertex 3 %d and 5 %d \n ", qg.verts[i3], qg.verts[(i3 + 3)%6] );
      }
      else if( qg.vals[1] * qg.vals[5] == 9 ||
	  qg.vals[2] * qg.vals[4] == 9 ) {
	  printMesh(qm, buffer,0);
	  printf(" In swapCollapse -> using 33 pairs \n ");
  ...
  if (( forcing == 0 && qg.vals[vL5] == 4 && getValence (qm -> mesh, vL5adj) >= 4 ) ||
      ( forcing == 1 && getValence (qm -> mesh, vL5adj) > 4 ) ) return EGADS_SUCCESS;
  stat  = EG_createQuadGroup (qm -> mesh, &qg, qg.q[q5], adj[1]);
  if ( stat != EGADS_SUCCESS ) {
...
static int EG_cleanNeighborhood (meshMap *qm, int qID, int adjacent, int transfer, int *activity ) {
  ...
  *activity += act;
  ...
  for ( i = 0 ; i < 4; i++ ) {
	  ...
	  if (  checkQuad ( qm -> mesh, star -> quads[j]) != EGADS_SUCCESS ) continue;
...
static int EG_fullMeshRegularization(meshMap *qm )
{
  ...
  if ( iV > 2 ) {
	  ...
	  for ( k = 0 ; k < 2; k++ ) {
	      ...
	      else          fprintf(stderr," TRANSFER VALENCES ALSO USING 55, 33 PAIRS \n");
	      for ( transfer = q = 0 ; q < qm -> mesh-> totQuads; q++) {
		  if ( checkQuad( qm -> mesh, q+ 1 ) != EGADS_SUCCESS ) continue;
		  quadPair[0] = q + 1;
		  quadPair[1] = -1;
		  printf("============== TRANSFER VALENCES AROUND QUAD  %d \n", q + 1);
...
static int optimize_angles(meshMap *qm, int nP, /*@unused@*/ /*@null@*/int *pList, int fullRegularization)
{
  ...
  if ( fullRegularization == -1 ) { // move around only affected vertices
#ifdef DEBUGG
      printf(" POINT LIST %d \n", nP );
      for ( i = 0 ; i < nP; i++ ) {
	  printf(" P %d = %d \n", i, pList[i]);
  ...
  while ( it < itMax ) {
      ...
      if ( fullRegularization == 0 || stat != EGADS_SUCCESS || q == 0 ) break;
