...
static int EG_rotateVertex            (meshMap *qm, int clockwise, int iA, int iB, int iC, int oriABC);
 ...
 * theta rotation (signed) of vertex iB until angle iA,iC,iB ( iA,iB,iC) is  [0,180]. Updates coordinates iB in qm
 */
static int
EG_rotateVertex ( meshMap *qm, int clockwise, int iA, int iB, int iC, int oriABC ) {
	int stat, d, i, it = 0, itMAX = 10;
	double eval[18], vec[8], t[8], normal[4], proj[6], cross[4], range[4], size, daux = 0.0, theta, angABC;
	size = EG_segment ( qm, &qm -> mesh -> uvs [ 2 * (iA - 1) ], &qm -> mesh -> uvs [ 2 * (iB - 1 ) ]);
	/*for ( i = 0 ; i < qm -> mesh -> valence[ iB - 1][0]; i++ ) {
		d = qm -> mesh -> valence[iB -1][2 + i];
		if ( qm -> mesh -> vType [ d -1 ] >= 0 ) {
			daux = EG_segment ( qm, &qm -> mesh -> uvs [ 2 * (iB - 1) ], &qm -> mesh -> uvs [ 2 * (d - 1 ) ]);
			if ( daux < size ) size = daux;
		}
	}*/
	size = 0.0;
	for ( i = 0 ; i < qm -> mesh -> valence[ iB - 1][0]; i++ ) {
		d = qm -> mesh -> valence[iB -1][2 + i];
		size += EG_segment ( qm, &qm -> mesh -> uvs [ 2 * (iB - 1) ], &qm -> mesh -> uvs [ 2 * (d - 1 ) ]);
	}
	size /= qm -> mesh -> valence[ iB - 1][0];
	t[0] = qm -> mesh -> uvs [ 2 * (iA - 1)    ];
	t[1] = qm -> mesh -> uvs [ 2 * (iA - 1) + 1];
	t[2] = qm -> mesh -> uvs [ 2 * (iB - 1)    ];
	t[3] = qm -> mesh -> uvs [ 2 * (iB - 1) + 1];
	printf(" ROTATE VERTEX CLOCK %d --> AB %d %d AND CALCULATE ANGLES WRT %d  ANGLE ORIABC %d\n ", clockwise, iA, iB, iC, oriABC);
	if ( qm -> mesh ->vType [ iB -1 ] != -1 ) {
		printf(" Trying to rotate a boundary!!!\n");
		return EGADS_INDEXERR;
	}
	stat = EG_normalToSurface (qm, &qm -> mesh -> uvs [ 2 * (iA -1 ) ], normal);
	if ( stat != EGADS_SUCCESS ) {
		printf("EG_rotateVertex :: EG_normalToSurface at %d ->%d!!\n", iA, stat);
		return stat;
	}
	stat = EG_projectToTangentPlane(normal, &qm -> mesh -> xyzs [ 3 * (iA - 1)],
			&qm -> mesh -> xyzs [ 3 * (iB - 1)],proj);
	for ( i = 0 ; i < 3; i++ )
	...
	theta = 0.05 * PI;
	if ( oriABC == 1 ) angABC = EG_angleAtVnormalPlane ( qm, iA, iB, iC );
	else               angABC = EG_angleAtVnormalPlane ( qm, iA, iC, iB );
	if ( angABC < PI ) {
	...
	stat = EG_evaluate ( qm -> face, &t[6], eval);
	for ( i = 0; i < 3; i++ ) {
		if ( i < 2 ) qm -> mesh -> uvs[ 2 * (iB -1) + i] = t[6 + i];
		qm -> mesh -> xyzs[ 3 * (iB -1) + i]             = eval[i];
	}
	if ( size < qm -> minsize * 0.5 ) size = qm -> minsize * 0.5;
	stat = moveDistAway ( qm, size, t, &t[6]);
	if ( stat != EGADS_SUCCESS ) {
		printf("In rotateVertex :: moveDistAway %d!!\n ", stat);
		return stat;
	}
	return stat;
}
...
static void weightedAverage (meshMap *qm, int vID ) {
	...
	if ( star -> nQ == 4 && (
		...
		uvc[0] /= (double)star -> nQ;
		uvc[1] /= (double)star -> nQ;
	}
	EG_free (length);
	EG_free (nB);
...
static int validInterior(meshMap *qm, int qID, int smooth, double minAngle, double maxAngle ) {
	int    reg,vj, k, i, i0, i4, sum , stat, area, links[2], orient[8], piv[8], it = 0, itMAX = 20, aux ;
	double angles[8], daux, tn[2], eval[18] ;
	printf(" \n\n-------------------------------------------------\n");
	...
	while ( area == 0 && it < itMAX ) {
		for  ( k = 0; k < 4; k++) {
			...
			if ( vertexLinksToBounds (qm -> mesh, vj ) > 0 ) {
				printf(" VERTEX LINKS TO BOUNDS %d", vertexLinksToBounds (qm ->mesh, vj ));
				if (  vertexLinksToBounds (qm -> mesh, vj ) == 4 && orient[piv[k]] == 1 ) continue;
				for ( reg = i4 = i0 = sum = i = 0 ; i < qm -> mesh -> valence[vj -1][0]; i++ ) {
					aux = qm -> mesh -> valence[vj -1][2 + i];
					if ( qm -> mesh -> vType[aux -1] >= 0) {
						if ( getValence ( qm -> mesh, aux ) == 4 ) reg++;
						if (    qm -> mesh -> vType[aux -1] == 4) i4 = aux;
						else                                      i0 = aux;
						sum++;
					}
				}
				tn[0] = qm -> mesh -> uvs[ 2 * (vj - 1)    ];
				tn[1] = qm -> mesh -> uvs[ 2 * (vj - 1) + 1];
				if ( reg == 4 && smooth == 1 ) weightedAverage ( qm, vj ) ;
				else if ( sum == 1 ) {
					if ( i4 == 0 ) i4 = i0;
					printf(" SINGLE BOUNDS: PULL FROM %d \n ", i4 );
					daux = 0.9 * EG_segment ( qm, &qm -> mesh -> uvs [ 2 * (vj -1 ) ], & qm -> mesh -> uvs [ 2 * (i4 -1) ]);
					printf(" SEGMENT SIZE %f min %f \n", daux, qm -> minsize * 0.5  );
					if ( daux < qm -> minsize * 0.5 ) daux = qm -> minsize * 0.5;
					stat  = moveDistAway ( qm, daux, &qm -> mesh -> uvs [ 2 * (i4 -1 ) ], tn );
				}
				stat  = EG_evaluate ( qm -> face, tn, eval);
				printf(" Evaluate %lf %lf --> %lf %lf %lf \n ", tn[0], tn[1], eval[0], eval[1], eval[2]);
				if ( stat != EGADS_SUCCESS ) {
					printf("In validInterior: pulling vertex towards sharp corner EG_evaluate %d!!\n", stat);
					return stat;
				}
				for ( i = 0; i < 3; i++ ) {
					if ( i < 2 ) qm -> mesh -> uvs[ 2 * (vj - 1) + i] = tn[i];
					qm -> mesh -> xyzs[ 3 * (vj - 1) + i]             = eval[i];
				}
...
static int validBoundary ( meshMap *qm, int smooth, double minAngle, double maxAngle ) {
	...
	double  angleAtV, angles[4], angleA, angleB, angleC, angleD ;
	vStar *star = NULL;
	printf("\n\n=============================================================\n");
	...
	for ( v = 0 ; v < qm -> mesh -> totVerts; v++ ) {
		...
		for ( qq = 0; qq < star -> nQ; qq++ )
		{
			...
			printf(" Quad %d check orientation \n ", star -> quads[q] );
			...
			for ( it = 0 ; it < itMAX; it++ ) {
				...
				if ( qm -> mesh -> vType[iB -1 ] == -1  &&  angleA > PI ) {
					printf(" Angle OAB %d,%d,%d is obtuse: rotate clockwise from B !!\n ", iA, iP, iB );
					stat = EG_rotateVertex(qm, -1, iA, iB, iP, 0);
					ok   = 1;
					if ( stat != EGADS_SUCCESS ) {
				...
				angleC = EG_angleAtVnormalPlane ( qm, iA, iP, iC );
				printf(" Angle ABC %d,%d,%d = %f\n", iA,iB,iC,angleB);
				printf(" Angle AOC %d,%d,%d = %f\n", iA,iP,iC,angleC);
				if ( qm -> mesh -> vType[iB -1 ] == -1 && angleB > PI && angleC < PI ) {
					printf(" Angle ABC is obtuse rotate %d clockwise \n ", iB );
					stat = EG_rotateVertex(qm, -1, iA, iB, iC, 1);
					ok   = 1;
					if ( stat != EGADS_SUCCESS ) {
						printf("EG_rotateVertex  ABC -> %d!!\n ", stat);
						EG_freeStar(&star);
						return stat;
					}
				}
				angleC = EG_angleAtVnormalPlane ( qm, iB, iC, iD );
				if ( qm -> mesh -> vType[iC -1 ] == -1 && angleC > PI ) {
					printf(" Angle BCD is obtuse rotate %d clockwise \n ", iC );
					stat = EG_rotateVertex(qm, -1, iB, iC, iD, 1);
					ok   = 1;
					if ( stat != EGADS_SUCCESS ) {
						printf("EG_rotateVertex  ACD -> %d!!\n ", stat);
						EG_freeStar(&star);
						return stat;
					}
				}
				angleC = EG_angleAtVnormalPlane ( qm, iB, iC, iD );
				if ( qm -> mesh -> vType[iD -1 ] == -1 && angleC > PI ) {
					printf(" Angle BCD is obtuse rotate %d counter-clockwise \n ", iD );
					stat = EG_rotateVertex(qm, 1, iB, iD, iC, 0);
					ok   = 1;
					if ( stat != EGADS_SUCCESS ) {
						printf("EG_rotateVertex  ACD -> %d!!\n ", stat);
						EG_freeStar(&star);
				...
				printf(" Angle ABD  %d,%d,%d = %f\n", iA,iB,iD, angleD);
				if ( qm -> mesh -> vType[iB -1 ] == -1  &&  angleD > PI ) {
					printf(" Angle ABD is obtuse rotate %d counter-clockwise \n ", iB );
					stat = EG_rotateVertex(qm, 1, iA, iB, iD, 1);
					ok   = 1;
					if ( stat != EGADS_SUCCESS ) {
						printf("EG_rotateVertex  ABD -> %d!!\n ", stat);
						EG_freeStar(&star);
						return stat;
					}
				}
				angleD = EG_angleAtVnormalPlane ( qm, iA, iB, iD );
				printf(" Angle ABD  %d,%d,%d = %f\n", iA,iB,iD, angleD);
				if ( qm -> mesh -> vType[iD -1 ] == -1  &&  angleD > PI ) {
					printf(" Angle ABD is obtuse rotate %d counter-clockwise \n ", iB );
					stat = EG_rotateVertex(qm, -1, iA, iD, iB, 0);
					ok   = 1;
					if ( stat != EGADS_SUCCESS ) {
						printf("EG_rotateVertex  ABD -> %d!!\n ", stat);
						EG_freeStar(&star);
	...
	for ( i = 0 ; i < qm -> mesh -> totQuads; i++ ) {
		...
		if ( EG_quadIsBoundary ( qm -> mesh, i + 1 ) == 0 ) continue;
		stat  = quadAngleOrientation(qm, i + 1, &area, orient, piv, angles);
		if ( area == 0 ) return EGADS_GEOMERR;
	}
	return EGADS_SUCCESS;
}
...
static int moveDistAway ( meshMap *qm, double size, double *uv0, double *uv1 ) {
	...
	{
		//      printf(" OUT OF RANGE %f %f [%f,%f]x[%f,%f]\n",
...
static int optimize_angles(meshMap *qm, int nP, /*@unused@*/ /*@null@*/int *pList, int fullRegularization)
{
	int  i, j, q, stat = EGADS_SUCCESS,  it = 0, itMax = 100, smooth, orient[8], piv[8], area;
