...
static int
EG_rotateVertex ( meshMap *qm, double theta, double size, int iA, int iB, int iC) {
  int stat, d, i;
  ...
  stat = EG_evaluate ( qm -> face, &t[6], eval);
  for ( i = 0; i < 3; i++ ) {
      if ( i < 2 ) qm -> mesh -> uvs[ 2 * (iC -1) + i] = t[6 + i];
      qm -> mesh -> xyzs[ 3 * (iC -1) + i]             = eval[i];
  }
  return stat;
...
static int validInterior(meshMap *qm, int qID, int smooth, double minAngle, double maxAngle ) {
  ...
  while ( area == 0 && it < itMAX ) {
	  ...
	  printf(" Looking at %d linked to %d %d \n ", vj, links[0], links[1] );
...
static int validBoundary ( meshMap *qm, int smooth, double minAngle, double maxAngle ) {
  ...
  double  angleAtV, angles[4],segAB, angleA, angleB, angleC, angleD, dt;
  vStar *star = NULL;
  printf("\n\n=============================================================\n");
  ...
  for ( v = 0 ; v < qm -> mesh -> totVerts; v++ ) {
      ...
      for ( qq = 0; qq < star -> nQ; qq++ )
	{
	  ...
	  if ( smooth == 1 ) {
		  ...
		  if  (qm -> mesh -> vType[aux  ] != -1 || vertexLinksToBounds (qm -> mesh, aux + 1) == 4) continue;
	  ...
	  if ( area != 1 ) j = 1 ;
		  ...
		  aux = qm -> mesh -> quadIdx [ 4 * (  star -> quads[q] -1 ) + piv[i] ] - 1;
		  if      (qm -> mesh -> vType[aux  ] != -1 || qm -> mesh -> valence[aux][0] != 4 ) continue;
		  if      (qm -> mesh -> vType[iA -1]    == 4 && (angles [ piv[i] ] > PI * 0.8 || angles[ piv[i] ] < PI * 0.1 )) j = 1;
		  else if (qm -> mesh -> vType[iA -1]    != 4 && (angles [ piv[i] ] > maxAngle || angles[ piv[i] ] < minAngle )) j = 1;
	      }
	  }
	  if (j == 0 ) {
	  ...
	  for ( it = 0 ; it < itMAX; it++ ) {
	      ...
	      printf(" \n\n--------- VERTEX %d iB %d iC %d iD %d QUAD %d \n", iA, iB, iC, iD, star -> quads[q]);
	      angleA = EG_angleAtVnormalPlane ( qm, iA, iP, iB );
	      printf(" Angle with respect previous vertex %d,%d,%d = %f\n", iA,iP,iB,angleA);
	      segAB  = EG_segment ( qm, &qm -> mesh -> uvs [ 2 * (iA - 1) ], &qm -> mesh -> uvs [ 2 * (iB - 1 ) ]);
	      if ( qm -> mesh -> vType[iB -1 ] == -1  &&  angleA > PI ) {
		  printf(" Vertex %d %d is obtuse!!\n ", iA, iB );
		  stat = EG_rotateVertex(qm, angleA * 0.5, segAB, iA, iP, iB);
		  ok = 1;
		  if ( stat != EGADS_SUCCESS ) {
		      printf("EG_rotateVertex %d about %d,%d -> %d!!\n ", iB, iA, iP, stat);
		      EG_freeStar(&star);
		      return stat;
		  }
		  segAB  = EG_segment ( qm, &qm -> mesh -> uvs [ 2 * (iA - 1) ], &qm -> mesh -> uvs [ 2 * (iB - 1 ) ]);
	      ...
	      printf(" Angle APC  %d,%d,%d = %f\n", iA,iP,iC,angleC);
	      if ( qm -> mesh -> vType[iB -1 ] == -1  && angleB > PI && angleC < PI ) {
		  printf(" AB > AC : vertex %d is clockwise %d \n ", iB, iC );
		  stat = EG_rotateVertex(qm, 0.85 * angleC, segAB, iA, iP, iB);
		  ok = 1;
		  if ( stat != EGADS_SUCCESS ) {
		      printf("ValidBoundary :: EG_rotateVertex %d about %d,%d -> %d!!\n ", iB, iA, iP, stat);
		      EG_freeStar(&star);
		      return stat;
		  }
		  segAB  = EG_segment ( qm, &qm -> mesh -> uvs [ 2 * (iA - 1) ], &qm -> mesh -> uvs [ 2 * (iB - 1 ) ]);
	      ...
	      printf(" Angle ACD  %d,%d,%d = %f\n", iA,iC,iD,angleC);
	      if ( angleC < PI ) {
		  if ( qm -> mesh -> vType[iB -1 ] == -1  &&  angleD > PI ) {
		      printf(" angle at Vertex %d is obtuse with last %d rotate %f !!\n ", iB, iD, angleD - PI );
		      stat = EG_rotateVertex(qm, 0.1 * PI , segAB, iA, iB, iB);
		      ok = 1;
		      if ( stat != EGADS_SUCCESS ) {
			  printf("ValidBoundary :: EG_rotateVertex %d about %d,%d -> %d!!\n ", iB, iA, iB, stat);
			  EG_freeStar(&star);
		  ...
		  angleC = EG_angleAtVnormalPlane ( qm, iA, iB, iC );
		  angleD = EG_angleAtVnormalPlane ( qm, iC, iB, iD );
		  if ( angleC < PI && angleD > PI &&  vertexLinksToBounds (qm -> mesh, iC) == 0 ) {
		      ok = 1;
		      printf(" Pull vertex %d EXCLUDINT %d %d \n ", iC, iB, iD ) ;
		      averageCoordsMinusLinks (qm, iC, iB, iD ) ;
		  }
	      }
	      if ( ok == 0 ) break;
	      else {
...
static int optimize_angles(meshMap *qm, int nP, /*@unused@*/ /*@null@*/int *pList, int fullRegularization)
{
  int  i, v, q, stat = EGADS_SUCCESS, nV = 0, it = 0, itMax = 100, *vID = NULL, smooth;
  ...
  if ( fullRegularization == 0 ) { // move around only affected vertices
      ...
      for ( v = 0 ; v < nV; v++) {
	  if ( vertexLinksToBounds (qm -> mesh, vID[v] ) > 0 ) {
      ...
      for ( v = 0 ; v < nV; v++) {
	  if ( vertexLinksToBounds (qm -> mesh, vID[v] ) > 0 ) continue;
	  ...
	  for ( q = v = 0 ; v < qm -> mesh -> totQuads; v++) {
	      ...
	      stat = validInterior ( qm, v + 1, smooth,  minT, maxT);
	  }
	  ...
	  for ( q = v = 0 ; v < qm -> mesh -> totQuads; v++) {
		  ...
		  stat = validInterior ( qm, v + 1,0, MINVALIDANGLE, MAXVALIDANGLE);
		  if ( stat != EGADS_SUCCESS ) {
	  ...
	  if ( stat == EGADS_SUCCESS ) stat = validBoundary (qm, 0, MINVALIDANGLE, MAXVALIDANGLE);
