...
static double EG_angleAtQnormalPlane ( meshMap *qm, int q, int v1, int v2, int v3 );
...
EG_reduceAngle ( meshMap *qm, double theta, int iA, int iB, int iC, int reverse) {
	...
	double eval[18], vec[8], t[8], normal[4], proj[6], cross[4], dt, lenBC, lenAV;
	...
	for ( a = 0 ; a < 2; a++) {
		...
		printf(" VERTEX %d direction %d \n ", v + 1, counterClockWise );
		if ( qm -> mesh -> vType [v] != -1 || vertexLinksToBounds (qm -> mesh, v + 1) ==4 ) continue;
		...
		printf(" LENGTH AB (%d,%d) = %f    BC (%d,%d) = %f  ----> rotation %f VS %f\n ",
				iA, v+ 1, lenAV, iB, iC, lenBC, dt, 0.01);
		if ( dt > 0.01 ) dt = 0.01;
		dt = 0.01;
		for ( d = 0 ; d < 2; d++ ) {
			if ( d == 1 ) dt *= -1.0;
			...
			t[6] =   t[0] + t[4];
			t[7] =   t[1] + t[5];
			if ( t[6] < qm ->range[0] || t[6] > qm ->range[1] || t[7] < qm ->range[2] || t[7] > qm ->range[3] ) {
...
static void weightedAverage (meshMap *qm, int vID, int *fix ) {
	...
	double uvc[2], angle, optangle, edgeSize[2], ang1, ang2, rotate, angc,  *length = NULL, totLength = 0.0;
	vStar * star = NULL;
	*fix = 0;
	...
	if ( j == 1 && qm -> smooth  >0 && nB[0] == 2 ) {
		uvc[0] = 0.5 * (qm -> mesh -> uvs[2*( nB[1] - 1 )    ] + qm -> mesh -> uvs[2*( nB[2] - 1 )    ]);
		uvc[1] = 0.5 * (qm -> mesh -> uvs[2*( nB[1] - 1 ) + 1] + qm -> mesh -> uvs[2*( nB[2] - 1 ) + 1]);
		i      = 1;
	} else {
		if ( qm -> smooth != 4 ) {
			...
			uvc[0] /= (double)( star -> nQ + 1);
			uvc[1] /= (double)( star -> nQ + 1);
		}
		else {
			...
			for ( i = 0; i < star -> nQ; i++) {
				...
				length[i] = EG_segment (qm, &qm -> mesh -> uvs [ 2 * (auxID - 1) ],
						&qm -> mesh -> uvs [ 2 * ( vID - 1)] );
				totLength += length[i];
			}
			for ( i     = 0; i < star -> nQ; i++) {
	...
	if ( qm -> smooth > 0 && ( j == 0 || ( j == 1 && star -> nQ > 4 )) ) {
		for ( j = 0 ; j < nB[0]; j++ ) {
			...
			angc     = EG_angleAtQnormalPlane (qm, star -> quads[star -> idxQ[i0]], auxID + 1, links[0], vID );
			printf(" Angle at quad centre %d ---> %f compare to %f \n ", star -> quads[star -> idxQ[i0]], angc, ang1);
			angc     = EG_angleAtQnormalPlane (qm, star -> quads[star -> idxQ[i0 + i]], auxID + 1, l1 + 1, vID );
			printf(" Angle at quad centre %d ---> %f compare to %f \n ", star -> quads[star -> idxQ[i0 +i]], angc, ang2);
...
static void makePositiveAngles (meshMap *qm, int qID, int *piv, int *ori ) {
	int i, j, k, iO, iA, iB, stat, fullNegative = 0;
	double ang;
	fullNegative = ori[0] + ori[1] + ori[2] + ori[3];
	printf(" FULLNEGATIVE %d \n ", fullNegative);
	k = -1;
	for ( j   = 0; j < 4; j++ ) {
	...
	if ( k != -1 )
		for ( j   = 0; j < 4; j++ ) piv[j] = ( k + j ) %4 ;
	for ( j   = 0; j < 4; j++ ) {
		k     = piv[j];
		iO    = qm -> mesh -> quadIdx [ 4 * ( qID - 1 ) + k];
		printf(" CENTRE %d ( j %d ) \n ", iO, j);
		if ( ori[k] == -1 && qm -> mesh -> vType [ iO - 1 ] == -1 && fullNegative != -4  ) continue;
		printf(" CENTRE %d ( j %d ) \n ", iO, j);
		printf ("=========================================\n");
		for (i  = 1; i <= 2; i++ ) {
			iA  = qm -> mesh -> quadIdx [ 4 * ( qID - 1 ) + ( k + i    ) %4];
			iB  = qm -> mesh -> quadIdx [ 4 * ( qID - 1 ) + ( k + i + 1) %4];
			ang = EG_angleAtVnormalPlane( qm, iO, iA, iB);
			printf("\n\n i = %d --> angle centre,%d  %d %d --> %f \n ",i, iO, iA, iB , ang );
			if ( ang > PI ) {
				if ( fabs ( ang - PI ) < fabs ( ang - 2 * PI ) )
					EG_reduceAngle(qm, 0.05 * PI, iO, iA, iB , 1);
				else
					EG_reduceAngle(qm, 0.05 * PI, iO, iA, iB , 0);
				ang = EG_angleAtVnormalPlane  (qm, iO, iA, iB);
...
static int validInterior(meshMap *qm, int qID,  double minAngle, double maxAngle ) {
	int     k, i, stat, area, orient[4], piv[4], it = 0, itMAX = 20, aux, boundlink;
	double angles[4];
	printf("\n\n-------------------------------------------------\n");
	...
	if ( area == 1 && qm -> smooth > 0 ) {
		for ( k = 0 ; k < 4; k++ ) {
			aux = qm -> mesh -> quadIdx [ 4 * ( qID -1 ) + piv[k] ];
			printf(" Laplacian weighted average about %d\n ", aux ) ;
			if (vertexLinksToBounds (qm -> mesh, aux) == 0 ) weightedAverage (qm, aux, &i);
		}
		stat = quadAngleOrientation(qm, qID, &area, orient, piv, angles);
		if ( area ==1 ) return EGADS_SUCCESS;
	}
	else if ((area == 1 && boundlink == 1 ) ||
			( area == 2 && maxAngle   > PI )) return EGADS_SUCCESS;
	for ( k = 0 ; k < 4; k++ ) {
	...
	while   ( it < itMAX ) {
		...
		printf(" ------- ITERATION %d \n ", it );
		makePositiveAngles (qm, qID, piv, orient );
		stat = quadAngleOrientation (qm, qID, &area, orient, piv, angles);
...
static int validBoundary ( meshMap *qm, double minAngle, double maxAngle ) {
	int vi[3], fixVi[3], iA, iP, v, i, j, k, q, qq, stat = 0 , aux, i0 = -1, area, it,  itMAX = 10, ok, nI, *vfix = NULL;
	int  **nb = NULL, *nc = NULL, *nd = NULL, aLR[2], orient[4], piv[4];
	double  angleAtV, edgeSize[2], angles[4], angleA, angleC, angleD, seg, uvback[8], optiAngle, dtheta = 0.05 * PI, dt ;
	vStar *star = NULL;
	printf("\n\n=============================================================\n");
	...
	nb   = (int ** ) EG_alloc ( 3 *                      sizeof (int *) );
	if ( vfix == NULL || nb == NULL ) return EGADS_MALLOC;
	vfix[0]   = 0;
	for ( v = 0 ; v < qm -> mesh -> totVerts; v++ ) {
		...
		for ( qq  = 0; qq < star -> nQ; qq++ )
		{
			...
			if ( area != 1 ) j = 1 ;
				...
				if ( qm -> smooth > 0 ) {
					...
					for ( i = 0 ; i < 4; i++ ) {
						...
						if ( vfix[0] > 0 ) {
							for (  k = 1 ; k <= vfix[0]; k++ )
								if ( vfix[k] == aux + 1 ) aux = -10;
						}
						if ( aux == -10 ) continue;
						weightedAverage (qm, aux + 1, &k);
						if ( k == 1 )  vfix[++vfix[0]] = aux + 1;
					}
					printMesh(qm ,buffer, 0);
					stat = quadAngleOrientation(qm, star -> quads[q], &area, orient, piv, angles);
					if ( area == 1 ) {
			...
			itMAX = 10;
			for ( i = 1; i  <= 3; i++ ) {
				vi   [i - 1] = star -> verts [ star -> idxV [ 2 * q + i ] ];
				fixVi[i - 1] = vertexFixedToBounds ( qm, vi[i - 1], &nb[0] );
				if ( qm -> smooth == 0 || area == 0 ) fixVi[i - 1] = 0;
			}
			edgeSize[0] = EG_segment (qm, &qm -> mesh -> uvs [ 2 * ( iA - 1 ) ], &qm -> mesh -> uvs [ 2 * ( iP     - 1 )] );
			edgeSize[1] = EG_segment (qm, &qm -> mesh -> uvs [ 2 * ( iA - 1 ) ], &qm -> mesh -> uvs [ 2 * ( vi[2] - 1 )] );
			for ( it = 0 ; it < itMAX; it++ ) {
				ok = 0;
				printf("\n\n************** Inside quad %d at boundary::: CENTRE AT %d : it %d \n ", star -> quads[q], iA, it );
				for ( i = 0 ; i < 3; i++ ) {
					if ( qm -> mesh -> vType [ vi[i] -1 ] == -1 && fixVi[i] == 0 ) {
						seg   = EG_segment (qm, &qm -> mesh -> uvs [ 2 * ( iA - 1 ) ], &qm -> mesh -> uvs [ 2 * ( vi[i] - 1 )] );
						printf(" segment ab %f size at edges %f  %f   nlinks %d \n ", seg, edgeSize[0], edgeSize[1], nBoundLinks (qm, vi[i]));
						if ( seg > edgeSize[0] && seg > edgeSize[1]  ) {
							printf(" REDUCE SIZE of %d MOVING %f \n ", vi[i] , seg * 0.95 );
							stat += moveDistAway (qm, 0.95, iA, vi[i] );
						}
					}
				}
				printMesh( qm , buffer, 0);
				// check that quad is valid //
				if ( area != 1 ) makePositiveAngles(qm, star -> quads[q], piv, orient);
				else {
					if ( qm -> smooth != 1 ) break;
					angleA = EG_angleAtVnormalPlane ( qm, iA, iP, vi[0] );
					if ( angleA > maxAngle || ( angleA > optiAngle
							&& nBoundLinks (qm, vi[0] ) * nBoundLinks (qm, iP ) <= 1 ) ) {
						stat = EG_reduceAngle(qm, dtheta, iA, iP, vi[0], 1);
						if ( stat != EGADS_SUCCESS ) {
							printf("EG_rotateVertex OAB -> %d!!\n ", stat);
							goto cleanup;
						}
					}
					angleD = EG_angleAtVnormalPlane ( qm, iA, vi[1], vi[2]);
					if (angleD > maxAngle || ( angleD > optiAngle
							&& nBoundLinks (qm, vi[0] ) * nBoundLinks (qm, iP ) <= 1 )  ) {
						stat = EG_reduceAngle(qm,dtheta, iA, vi[0], vi[2], 0);
						if ( stat != EGADS_SUCCESS ) {
							printf("EG_rotateVertex OAB -> %d!!\n ", stat);
							goto cleanup;
						}
					} else if ( angleD < optiAngle
							&& nBoundLinks (qm, vi[0] ) * nBoundLinks (qm, vi[2] ) <= 1 ) {
						stat = EG_reduceAngle(qm, dtheta, iA, vi[0], vi[2], 1 );
						if ( stat != EGADS_SUCCESS ) {
							printf("EG_rotateVertex OAB -> %d!!\n ", stat);
							goto cleanup;
						}
					}
					printf("\n AngleC = %f CDB %d,%d,%d maxAngle %f\n ",angleC, vi[1], vi[2], vi[0], maxAngle );
					angleC = EG_angleAtVnormalPlane ( qm, vi[1], vi[2], vi[0]);
					if ( angleC > maxAngle ) {
						for ( i = 0 ; i < 2; i++ ) {
							if (fixVi[i] == 0 ) {
								if ( i%2 == 0 ) dt = 0.95;
								else            dt = 1.05;
								stat += moveDistAway (qm, dt, iA, vi[i]);
								if ( stat != EGADS_SUCCESS ) {
...
static double
EG_angleAtQnormalPlane ( meshMap *qm, int q, int v1, int v2, int v3 ) {
	int i, stat;
	double dot1, dot2, normal[4], cross[3], proj[9], vec12[4], vec13[4], xyz[3 * 18], uvq[5], qxyz[18];
	stat  = quadAverageCoords (qm, q, uvq, &uvq[2]);
	//snprintf(buffer, 500, "NPQ_%d_%d", q, MESHPLOTS++);
	//sampleNormalPlane(qm, buffer, uvq );
	stat += EG_evaluate(qm -> face, uvq, qxyz);
	stat += EG_evaluate(qm -> face, & qm -> mesh -> uvs[ 2 * ( v1 - 1)],  xyz);
	stat += EG_evaluate(qm -> face, & qm -> mesh -> uvs[ 2 * ( v2 - 1)], &xyz[    18]);
	stat += EG_evaluate(qm -> face, & qm -> mesh -> uvs[ 2 * ( v3 - 1)], &xyz[2 * 18]);
	if ( stat != EGADS_SUCCESS) return stat;
	if ( qm -> face -> mtype == SREVERSE )
		cross_product(&qxyz[6], &qxyz[3], normal);
	else
		cross_product(&qxyz[3], &qxyz[6], normal);
	unitVector(normal, &normal[3]);
	//printf(" %f %f %f  CENTROID \n ", uvq[2], uvq[3], uvq[4] );
	//printf(" %f %f %f A \n ", xyz[0], xyz  [1], xyz[2]);
	//printf(" %f %f %f B \n ", xyz[18], xyz[19], xyz[20]);
	//printf(" %f %f %f C \n ", xyz[36], xyz[37], xyz[38]);
	stat  = EG_projectToTangentPlane(normal, qxyz,  xyz    ,  proj   );
	stat += EG_projectToTangentPlane(normal, qxyz, &xyz[18], &proj[3]);
	stat += EG_projectToTangentPlane(normal, qxyz, &xyz[36], &proj[6]);
	//printf(" %f %f %f PA \n ", proj[0], proj[1], proj[2]);
	//printf(" %f %f %f PB \n ", proj[3], proj[4], proj[5]);
	//printf(" %f %f %f PC \n ", proj[6], proj[7], proj[8]);
	for ( i = 0 ; i < 3 ; i++ ) {
		vec12[i] = proj[3 + i] - proj[i];
		vec13[i] = proj[6 + i] - proj[i];
	}
	unitVector (vec12,&vec12[3] );
	unitVector (vec13,&vec13[8] );
	cross_product (vec12, vec13, cross );
	dot1 = dotProduct ( vec12,  vec13);
	dot2 = dotProduct ( normal, cross);
	if      ( dot1 >=  1.0 ) return 0.0;
	else if ( dot1 <= -1.0 ) return PI;
	if ( dot2 > 0 ) return  acos ( dot1 );
...
EG_swappingOperation(meshMap *qm, quadGroup qg, int swap ) {
	...
	for ( j = 1 ; j < 6; j++ ) {
		if ( j == 3 ) continue;
		if ( j < 3 )
			stat = quadAngleOrientation(qm, qID[0], &area, ori, order, angles);
		else
			stat = quadAngleOrientation(qm, qID[1], &area, ori, order, angles);
		if ( area == 1 && ori[order[0]] == 1 && angles[order[0]] <= PI ) continue;
...
static int optimize_angles(meshMap *qm, int nP, /*@unused@*/ /*@null@*/int *pList, int fullRegularization)
{
	...
	if ( fullRegularization == 0 ) { // move around only affected vertices
		...
		for ( j = 0 ; j < nP; j++) {
			...
			if ( stat != EGADS_SUCCESS || star == NULL ) return stat;
			for ( i = 0 ; i < star -> nQ; i++ ) {
				stat = validInterior ( qm, star -> quads[i], 0.0, PI + 0.1 * PI );
				if ( stat != EGADS_SUCCESS ) {
