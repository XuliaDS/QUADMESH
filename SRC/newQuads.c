...
static void   weightedAverage         (meshMap *qm, int vC, int regular );
...
EG_reduceAngle ( meshMap *qm, double theta, int iA, int iB, int iC, int reverse) {
	int stat, d, i, a, v, counterClockWise = 1, it, itMax = 100, *nb = NULL ;
	double eval[18], vec[8], t[8], normal[4], proj[6], cross[4], range[4], physTheta, angInt[2], dt;
	...
	printVertexCoords (qm ,iA);
	printVertexCoords (qm ,iB);
	printVertexCoords (qm ,iC);
	for ( a = 0 ; a < 2; a++) {
		counterClockWise *= -1;
		if ( a == 0 ) v = iB - 1;
		else          v = iC - 1;
		...
		for ( d = 0 ; d < 2; d++ ) {
			if ( d == 1 ) theta *= -1.0;
			t[4] = ( t[2] - t[0] ) * cos ( theta ) - ( t[3] - t[1] ) * sin ( theta );
			t[5] = ( t[2] - t[0] ) * sin ( theta ) + ( t[3] - t[1] ) * cos ( theta );
			t[6] =   t[0] + t[4];
			t[7] =   t[1] + t[5];
		...
		it        = 0 ;
		angInt[0] = 0.0; angInt[1] = theta;
		if ( counterClockWise == -1 )
			physTheta = EG_angleAtPlaneUsingUVs ( qm , iA, &t[6], &t[2]);
		else
			physTheta = EG_angleAtPlaneUsingUVs ( qm , iA, &t[2], &t[6] );
		printf(" physical angle %f  target %f  it %d ERROR %f > 0.005?\n ", physTheta, theta, it, fabs( physTheta - theta ) / theta  );
		/*while ( fabs( physTheta - theta ) / theta > 0.005 && it < itMax ) {
			dt   = 0.5 * ( angInt[0] + angInt[1] );
			...
			if ( counterClockWise == -1 )
				...
				physTheta = EG_angleAtPlaneUsingUVs ( qm , iA, &t[2], &t[6] );
			if ( physTheta > theta ) angInt[1] = dt;
			else angInt[0] = dt;
			it++;
			printf(" physical angle %f  target %f  it %d \n ", physTheta, theta, it);
		}*/
...
static void weightedAverage (meshMap *qm, int vID, int regular ) {
	int i, j, auxID = 0, stat, *nB = NULL, links[2], nI, i0, l1, l2;
	double uvc[2], angle, optangle, edgeSize[2], angAtV, rotate, avlength;
	vStar * star = NULL;
	if ( qm -> mesh -> vType[ vID -1] != -1 ) return;
	stat   = EG_buildStar ( qm -> mesh, &star, vID ) ;
	if ( star == NULL || stat != EGADS_SUCCESS) return;
	printf("========== weighted Average coordinates arclength scaled %d \n ", vID ) ;
	printVertexCoords (qm, vID );

	avlength = 0.0;
	for ( i = 0; i < star -> nQ; i++) {
		auxID     = star -> verts[2 * i + 1] ;
		avlength += EG_segment (qm, &qm -> mesh -> uvs [ 2 * ( auxID - 1 ) ], &qm -> mesh -> uvs [ 2 * ( vID - 1 )] );
	}
	avlength /= (double)star -> nQ;
	uvc[0] = qm -> mesh -> uvs [ 2 * ( vID - 1 )     ];
	uvc[1] = qm -> mesh -> uvs [ 2 * ( vID - 1 ) + 1 ];
	j = vertexFixedToBounds (qm, vID, &nB);
	printf(" Vertex %d is fix ??? %d smooth %d \n ", vID, j, qm -> smooth );
	if ( j == 1 && qm -> smooth == 1 ) {
		for ( i   = 0; i < nB[0]; i++) {
	...
	if ( j == 0 || ( j == 1 && star -> nQ > 4 ) ) {
		for ( j = 0 ; j < nB[0]; j++ ) {
			...
			edgeSize[0] = EG_segment (qm, &qm -> mesh -> uvs [ 2 * ( auxID - 1 ) ], &qm -> mesh -> uvs [ 2 * l1] );
			edgeSize[1] = EG_segment (qm, &qm -> mesh -> uvs [ 2 * ( auxID - 1 ) ], &qm -> mesh -> uvs [ 2 * l2] );
			printf(" CENTRE %d size WRT %d %d == %f  %f\n ", auxID, l1 + 1, l2 + 1, edgeSize[0], edgeSize[1] );
			if (stat != EGADS_SUCCESS ) {
			...
			angAtV   = EG_angleAtVnormalPlane ( qm, auxID, links[0], vID);
			if ( fabs ( angAtV - optangle ) > PI * 0.25) {
				printf( " angle at %d %d %d \n ", links[0], auxID, vID );
				printf(" Angle opti %f  at vertex %f \n ", optangle, angAtV );
				printf(" weighted average angle %f opti %f \n", angAtV, optangle);
				rotate = PI * 0.05;
				if ( angAtV > optangle )
					stat  = EG_reduceAngle(qm, rotate, auxID, links[0], vID, 1);
				else stat = EG_reduceAngle(qm, rotate, auxID, links[0], vID, 0);
			}
			angAtV    = EG_segment (qm, &qm -> mesh -> uvs [ 2 * ( auxID - 1 ) ], &qm -> mesh -> uvs [ 2 * ( vID - 1 )] );
			if (angAtV > edgeSize[0] && angAtV > edgeSize[1] ) {
				printf(" segment %f  sizes %f  %f average %f \n ", angAtV, edgeSize[0], edgeSize[1], avlength);
				stat = moveDistAway (qm, 0.95, auxID, vID);
			}
...
static int validInterior(meshMap *qm, int qID,  double minAngle, double maxAngle ) {
	...
	printf("\n\n-------------------------------------------------\n");
	printf("-----------   CheckInvalid quad %d min max angles %lf %lf------------\n", qID, minAngle, maxAngle );
	printf("-------------------------------------------------\n");
	...
	if ( area == 2 && (qm -> smooth == 1 || maxAngle < MAXVALIDANGLE)) area = 0;
	else if ( area == 1 ) {
		if ( qm -> smooth == 1 ) {
			dtheta = 0.05* PI;
			for ( k = 0 ; k < 4; k++ ) {
				...
				if (vertexLinksToBounds (qm -> mesh, aux) == 0 ) weightedAverage (qm, aux, 1);
			}
			stat = quadAngleOrientation(qm, qID, &area, orient, piv, angles);
	...
	while ( area == 0 && it < itMAX ) {
		...
		for  ( k = 0; k < 4; k++) {
			...
			printf("k = %d piv %d  Looking at %d linked to %d %d \n ", k, piv[k], vj, links[0], links[1] );
			...
			if ( area == 2 && maxAngle < MAXVALIDANGLE) area = 0;
				...
				for ( i = 4 ; i < 7; i++ ) {
					...
					if (angles [ piv[i] ] > maxAngle || angles[ piv[i] ] < minAngle ) area = 0 ;
				}
			}
			if ( area == 1 ) break;
		}
	...
	for ( v = 0 ; v < qm -> mesh -> totVerts; v++ ) {
		...
		for ( qq  = 0; qq < star -> nQ; qq++ )
		{
			...
			for ( it = 0 ; it < itMAX; it++ ) {
				...
				if ( angleA > maxAngle || ( qm -> smooth * area == 1 && angleA > optiAngle
				...
				printf(" AngleD = %f BAD %d,%d,%d opti angle %f\n ",angleD, iB, iA, iD, optiAngle);
				if ( angleD > PI || ( qm -> smooth * area == 1 && angleD > optiAngle
						&& nBoundLinks (qm, iB ) * nBoundLinks (qm, iD ) <= 1 )) {
					if ( angleD > PI ) ok  = 1;
					stat = EG_reduceAngle(qm, dtheta, iA, iB, iD , 1 );
					/*if ( fabs (angleD - PI ) < fabs ( angleD - 2.0 * PI ) )
		    stat = EG_reduceAngle(qm, dtheta, iA, iB, iD , 1 );
		  else
		    stat = EG_reduceAngle(qm, dtheta, iA, iB, iD , 0 );*/
				...
				else if ( qm -> smooth * area == 1 && angleD < optiAngle ) {
					stat = EG_reduceAngle(qm,dtheta, iA, iB, iD, 0);
					if ( stat != EGADS_SUCCESS ) {
						printf("EG_rotateVertex OAB -> %d!!\n ", stat);
						goto cleanup;
				...
				angleC = EG_angleAtVnormalPlane ( qm, iA, iC, iD );
				printf(" AngleD = %f CAD %d,%d,%d  maxangle %f \n ",angleD, iC, iA, iD, maxAngle );
				if ( angleC > PI ) {
				...
				if ( angleC > maxAngle ||
						( qm -> smooth * area == 1 && nBoundLinks (qm, iC ) * nBoundLinks (qm, iD ) <= 1 ) ) {
					//stat = EG_reduceAngle(qm, dtheta, iA, iB, iD, 1 );
					if ( angleC > PI ) ok   = 1;
					if ( stat != EGADS_SUCCESS ) {
						printf("EG_rotateVertex OAB -> %d!!\n ", stat);
...
static int moveDistAway ( meshMap *qm, double alpha, int iA, int iB) {
	int    i, stat, it;
	double range[4], seg, uvm[2], uvcopy[4], norm = 0.0, uv0[2], uv1[2], eval[18], size;
	...
	size = alpha * seg;
	if ( size  < qm -> minsize * 0.25) size = 0.25 * qm -> minsize;
	...
	it        = 0;
	while ( seg < size ) {
		uvm[0] += ( uvcopy[2] - uvcopy[0] ) * 0.25;
		uvm[1] += ( uvcopy[3] - uvcopy[1] ) * 0.25;
	...
	while ( fabs( seg - size ) / size > 0.005 ) {
		...
		if ( sqrt (norm ) < 1.e-08 || it > 100 ) {
			printf(" POINTS ARE TOO CLOSE TO EACH OTHER. BISECTION DIDN't CONVERGE\n");
			break;
		}
	}
...
EG_swappingOperation(meshMap *qm, quadGroup qg, int swap ) {
	...
	for ( j  = 0 ; j < 2; j++ ) {
		vc   = qg.verts[ j * 3 ];
		if ( qm -> mesh -> vType [ vc - 1] != -1 || vertexLinksToBounds (qm -> mesh, vc ) == -1 ) continue;
