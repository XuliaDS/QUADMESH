...
static int    checkInvalidElement     (meshMap *qm, int vID, int moveNearBd, double minAngle, double maxAngle );
...
averageCoordsUsingLinks ( meshMap *qm, int vc, int l1, int l2 ) {
	...
	for ( i    = 0 ; i < 3; i++ ) {
		uv[i] += qm -> mesh -> uvs[2 * ( l1 - 1) + i];
		uv[i] += qm -> mesh -> uvs[2 * ( l2 - 1) + i];
	}
	uv[0] /= 3; uv[1] /= 3;
	stat = EG_evaluate (qm -> face, uv, eval);
	for ( i = 0 ; i < 3; i++ ) {
...
averageCoordsMinusLinks ( meshMap *qm, int vc, int l1, int l2 ) {
	int i, ii, j, k, n, stat, *links = NULL ;
	double uv[2], eval[18] ;
	vc--;
...
static int vertexLinksToBounds (meshMap *qm, int vID ) {
	int i, j;
	if ( qm -> mesh -> vType[ vID -1 ] != -1 ) return 1;
	for (i = 0 ; i < qm -> mesh -> valence[ vID -1 ][0]; i++ ) {
		j  =  qm -> mesh -> valence[ vID -1 ][2 + i] - 1;
		if ( qm -> mesh -> vType[ j ] != -1 ) return 1;
	}
	return 0;
}
...
static int checkInvalidElement(meshMap *qm, int vID, int moveNearBd, double minAngle, double maxAngle ) {
	int    q, j, dir, vj, k, i0, i, ii, vi, stat, area,  aux, aux2, try, nl, links[2], orient[4], piv[4];
	double angles[4], cross[3], eval[18], anglebd, proj[3*2], vec1[4], vec2[4], tA[2], tB[2],
	tn[2], vt[2], normal[3],  range[4];
	vStar *star = NULL, *star2 = NULL;
	stat        = EG_buildStar (qm ->mesh, &star, vID);
	if ( stat  != EGADS_SUCCESS || star == NULL ) return stat;
	printf(" -----------   CheckInvalid for vertex %d MOVE NEAR BDS%d  min max angles %lf %lf------------\n", vID, moveNearBd, minAngle, maxAngle );
	for ( q = 0; q < star -> nQ; q++ ) {
		printf(" check quad %d \n ", star -> quads[q] );
		...
		for  ( k = 0; k < 4; k++) {
			vj       = qm -> mesh -> quadIdx [ 4 * ( star -> quads[q] - 1) +   piv[k]        ];
			printf(" Orientation at %d is %d \n ", vj, orient[piv[k]]);
			links[0] = qm -> mesh -> quadIdx [ 4 * ( star -> quads[q] - 1) + ( piv[k] + 1)%4 ];
			links[1] = qm -> mesh -> quadIdx [ 4 * ( star -> quads[q] - 1) + ( piv[k] + 3)%4 ];
			printf(" MOVE NEAR BD %d links to bd %d \n", moveNearBd , vertexLinksToBounds (qm, vj) );
			if ( moveNearBd == 0 && vertexLinksToBounds (qm, vj) == 1 ) continue;
			if ( (orient [piv[k]] == -1 && area == 1) || angles[ piv[k]] > maxAngle ) {
				averageCoordsMinusLinks (qm, vj, links[0], links[1]);
				break;
			}
			else if ( angles[ piv[k]] < minAngle ) {
		...
		printf(" We actually need to do some fixes. there area < 0 orientations\n ");
		for (j   = 0; j < 4; j++ ) {
			vj   = qm -> mesh -> quadIdx [ 4 * ( star -> quads[q] - 1) + piv[j] ];
			if ( moveNearBd == 1 || vertexLinksToBounds (qm, vj) == 0 )
				weightedAverage ( qm, vj ) ;
		}
		stat = quadAngleOrientation(qm, star -> quads[q], &area, orient, piv, angles);
	}
	if ( moveNearBd == 1 ) {
		...
		for ( q = 0; q < star -> nQ; q++ ) {
			...
			for ( j = 0 ; j < 4; j++ ) {
				...
				stat = EG_angleAtBoundaryVertex (qm, vj, links, &anglebd);
				printf(" CENTRE %d LINKS %d %d angle %f \n ", vj, links[0], links[1], anglebd );
				if ( stat != EGADS_SUCCESS){
					printf("Inside checkInvalid: EG_angleAtBoundaryVertex %d = %d!!\n", vj, stat);
					EG_free(star);
					return stat;
				}
				stat       = EG_buildStar ( qm -> mesh, &star2, vj );
				if ( stat != EGADS_SUCCESS || star2 == NULL ) {
					printf("checkInvalidElement: averageCoords:: buildStar %d is NULL --> %d!!\n", vj + 1, stat);
					EG_freeStar(&star);
					return stat;
				}
				stat = EG_normalToSurface (qm, &qm -> mesh -> uvs [ 2 * (vj - 1 ) ], normal);
				printQuadCoords ( qm, star -> quads[q] );
				printf(" normal to surface %lf %lf %lf \n ", normal[0], normal[1], normal[2] );
				if ( stat != EGADS_SUCCESS ) {
					printf("EG_checkInvalidElement: EG_normalToSurface at %d --> %d!!\n", vj + 1, stat);
					return stat;
				}
				for ( i   = 0 ; i < 2; i++ ) {
					tB[i] = qm ->mesh -> uvs [ 2 * ( links[0] - 1) + i];
					tA[i] = qm ->mesh -> uvs [ 2 * ( vj       - 1) + i];
				}
				stat      = EG_projectToTangentPlane(normal,
						&qm -> mesh -> xyzs [ 3 * (vj - 1)],
						&qm -> mesh -> xyzs [ 3 * (links[0] -1)], proj);
				for ( i   = 0 ; i < 3; i++ )
					vec1[i] = proj[i] - qm -> mesh -> xyzs [ 3 * (vj - 1) + i];
				unitVector (vec1, &vec1[3] );
				for ( i = nl = 0 ; i < star2 -> nQ; i++ ) {
					aux = star2 -> verts [ 2 * i + 1 ] -1 ;
					if ( qm -> mesh -> vType [aux] == -1 ) nl++;
					if ( aux + 1 == links[0] ) i0 = i;
				}
				for ( dir = 0 ; dir < 2; dir++ ) {
					angles[0]  = 1.0 / (double)(nl + 1) * anglebd ;
					if ( dir  ==  1 ) angles[0] = 2.0 * PI - angles[0];
					printf(" ANGLE TEST %f \n ", angles[0] );
					vt[0]      = ( tB[0] - tA[0] ) * cos ( angles[0] ) - ( tB[1] - tA[1] ) * sin ( angles[0] );
					vt[1]      = ( tB[0] - tA[0] ) * sin ( angles[0] ) + ( tB[1] - tA[1] ) * cos ( angles[0] );
					tn[0]      = tA[0] + vt[0];
					tn[1]      = tA[1] + vt[1];
					printf(" uA %d %f %f --- uB %d %f  %f  move away %f\n ", vj, tA[0], tA[1], links[0], tB[0], tB[1], 0.5 *qm -> minsize );
					stat       = moveDistAway ( qm, 0.5 * qm -> minsize , tA, tn );
					if ( stat != EGADS_SUCCESS ) {
						if ( dir == 1 ) {
					...
					stat = EG_projectToTangentPlane(normal, &qm -> mesh -> xyzs [ 3 * (vj - 1)], eval, &proj[3]);
					if ( stat != EGADS_SUCCESS ) {
						printf("EG_checkInvalidElement: EG_projectToTangentPlane %d, %d --> %d!!\n", vj, links[0], stat);
						return stat;
					}
					for ( i = 0 ; i < 3; i++ )
						vec2[i] = proj[3 + i] - qm -> mesh -> xyzs [ 3 * (vj - 1) + i];
				...
				printf(" d = %d -- NOW COMPUTE ALL COORDINATES \n ", dir ) ;
				for (k = 0 ; k < nl ; k++ ) {
					aux       = star2 -> verts [ star2 -> idxV [ 2 * (i0 + k + 1) + 1 ] ] - 1;
					angles[0] = (double ) ( k + 1) / (double)(nl + 1) * anglebd ;
					if ( dir ==  1 ) angles[0] = 2.0 * PI - angles[0];
					vt[0]      = ( tB[0] - tA[0] ) * cos ( angles[0] ) - ( tB[1] - tA[1] ) * sin ( angles[0] );
					vt[1]      = ( tB[0] - tA[0] ) * sin ( angles[0] ) + ( tB[1] - tA[1] ) * cos ( angles[0] );
					printf(" uA %f %f --- uB %f  %f  move away %f\n ", tA[0], tA[1], tB[0], tB[1], qm -> minsize );
					tn[0]      = tA[0] + vt[0];
					tn[1]      = tA[1] + vt[1];
					stat       = moveDistAway ( qm, qm -> minsize, tA, tn );
					stat      = EG_evaluate ( qm -> face, tn, eval);
					printf(" Evaluate %lf %lf --> %lf %lf %lf \n ", tn[0], tn[1], eval[0], eval[1], eval[2]);
					if ( stat != EGADS_SUCCESS ) {
					...
					printVertexCoords (qm, aux + 1);
					stat = checkInvalidElement ( qm , aux + 1, 0, minAngle, maxAngle);
				}
				EG_freeStar(&star2);
			}
		}
	...
	printMesh(qm, buffer,0);
	for ( i = q = 0; q < star -> nQ; q++ ) {
		stat = quadAngleOrientation(qm, star -> quads[q], &area, orient, piv, angles);
		if ( area != 1 || angles[ piv[0] ] > maxAngle || angles [ piv[3]] < minAngle ) i = 1;
	}
	EG_freeStar (&star);
	printMesh(qm, buffer,0);
	if ( i == 1 ) return EGADS_GEOMERR;
	return EGADS_SUCCESS;
}
...
quadAngleOrientation(meshMap *qm, int qID, int *validArea, int *ori, int *order, double *theta ) {
	...
	*validArea = 0;
	...
	for ( k = 0 ; k < 4; ++k) {
		...
		if ( ori[k] == 1 && EG_quadIsBoundary ( qm -> mesh, qID) == 0 ) *validArea = 1;
	}
	for ( sign = k = 0 ; k < 4; ++k) {
		vA   =   k        ;
		vB   = ( k + 1 )%4;
...
EG_swappingOperation(meshMap *qm, quadGroup qg, int swap ) {
	int stat, i0, i1, vc, i, j, q, adj, idx[4], qID[2], adjQmap[6], n, area, ori[4], order[4];
	double uvc[2], eval[18], angles[4];
	swap   = swap %3;
...
static int EG_swapSplit(meshMap *qm,quadGroup qg, int forcing, int *activity  ) {
	...
	*activity = 0;
	if  ( qg.vals[0] * qg.vals[3] != 20 ||
			validSwap (qm -> mesh, qg.verts[0], qg.verts[3] ) == 0 ||
			qm -> mesh -> vType [ qg.verts[0] - 1 ] == 4 || qm -> mesh -> vType [ qg.verts[3] - 1 ] == 4 )return EGADS_SUCCESS;
	if      (qg.vals[1] == 3 ) i3 = 1;
...
static int optimize_angles(meshMap *qm, int nP, /*@unused@*/ /*@null@*/int *pList, int fullRegularization)
{
	...
	printMesh(qm, buffer,0);
	if ( fullRegularization == 1 ) {
		printf(" FULL MES REGULARIZATION\n");
		for ( v = 0 ; v < nV; v++) {
			if ( vertexLinksToBounds (qm, vID[v] ) == 0 ) continue;
			stat = checkInvalidElement(qm, vID[v], 1, 0, PI ) ;
			if ( stat != EGADS_SUCCESS ) {
				printf(" Mesh is invalid for MIN MAX TOTALS [0, 200 deg]\n");
				snprintf(buffer,500, "badmeshStart_%d.txt", qm -> fID);
				printMesh(qm , buffer, 1);
				EG_free(vID);
				return stat;
			}
	...
	while ( it <= itMax ) {
		for ( q = v = 0 ; v < nV; v++) {
			if ( fullRegularization == 1 && vertexLinksToBounds (qm, vID[v] ) == 0 ) continue;
			stat = checkInvalidElement(qm, vID[v], 0, qm -> MINANGLE, qm -> MAXANGLE );
			if ( stat == EGADS_SUCCESS ) q++;
			else stat = checkInvalidElement(qm, vID[v], 1, MINVALIDANGLE, MAXVALIDANGLE);
			if ( stat != EGADS_SUCCESS ) {
				printf(" Mesh is invalid for MIN MAX TOTALS [0, 200 deg]\n");
				break;
			}
		...
		qm -> MAXANGLE -= dthetaM;
		for ( q = v = 0 ; v < nV; v++) {
			stat = checkInvalidElement(qm, vID[v], 0, 0.0, 180.0 ) ;
			if ( stat != EGADS_SUCCESS ) {
				printf(" Mesh is invalid for MIN MAX TOTALS [0, 200 deg]\n");
				break;
			}
