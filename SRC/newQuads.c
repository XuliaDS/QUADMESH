...
static void EG_unitVector(double *v, double *norm) {
	double n;
...
static int EG_cleanMesh (meshMap *qm, int domain, int operType, int *activity ) {
	...
	*activity   = 0;
	for ( count = i = 0 ; i < qm -> mesh -> totQuads; i++ ) {
		...
		if ( domain == INTERIOR && EG_quadIsBoundary (qm ->mesh, i + 1 ) != 0 ) continue;
		stat        = EG_cleanNeighborhood ( qm, i + 1, operType, 0, &act );
		if ( stat  != EGADS_SUCCESS ) {
				...
				for (q = 0 ; q < qm -> mesh-> totQuads; q++) {
					...
					while ( activity > 0 && iV > 2 && j < 20 ) {
						...
						for ( i = 0 ; i < 2; i++ ) {
							...
							printf(" CALL CLEAN NEIGHBORHOOD %d \n", quadPair[i]);
