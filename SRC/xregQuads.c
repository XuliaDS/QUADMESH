...
static int EG_placeVertex(meshMap *qm, int vID, double pass, int full, int report) {
	...
	if (signbit(aRatio[j]) == 1) sra[1] = MIN(sra[1], aRatio[j] );
	    ...
	    if (2 * j + 2 == corner)
	      ...
	      errbd = 0.0;//fabs(qm->star[v]->angles[j] - qm->star[v]->angles[(2 + j)%qm->star[v]->nQ]);
	    else errbd = fabs(1.0 - ERFC(PI * 16. / 18., PI, angpen));
            if (qInfo[j] >= bt ) minaux = MIN(minaux, errbd * aRatio[j]);
                                 sra[2] = MIN(sra[2], errbd * aRatio[j] );
	}
        if ( report == 1)
        printf(" OPT ANGLE %lf  ratio %lf  ANGERR %lf SRA2 %lf \n",
               angopt, aRatio[j], errbd, sra[2]);
    }
    if ( minaux < 0.1 ||
    ...
    for (round = 0; round < 2; round++) {
        ...
        for (q = 0; q <= nt; q++) {
            if (q > 0 && (
        	block > 0 || pass < 0.0 || qm-> star[v]->nQ == 3 ||
            ...
            for (j = 0 ;j < qm->star[v]->nQ; j++ ) {
                if (            report == 1) printf(" BT %d QINFO %d =  %d \n", bt, qm->star[v]->quads[j], qInfo[j]);
                if (signbit(aRatio[j]) == 1) srb[1] = MIN(srb[1], qm->star[v]->ratios[j] );
                else {
                    errbd = 1.0;
                    if (2 * j + 2 == corner)
                        errbd = fabs(angopt - qm->star[v]->angles[corner]);
                ...
                if ( report == 1)
                    printf(" OPT ANGLE %lf  ratio %lf  ANGERR %lf SRA2 %lf \n",
                           angopt,  qm->star[v]->ratios[j] , errbd, srb[2]);
            }
            if(report == 1) {
                if (q == 0 ) printf(" CENTROID %d\n", round);
                else printf("\n\n q - %d / %d MID POINTS BETWEEN %d %d \n ",q ,nt,  vl[0], vl[1]);
                printf(" NEW  AREA %d RATIOS %lf NEG %lf \n", jb, srb[2], srb[1]);
                printf(" BEST SO FAR AREA %d RATIOS %lf NEG %lf \n", ja, sra[2], sra[1]);
                printVertex(qm, vID);
