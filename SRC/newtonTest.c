...
static void EG_minArc(const ego face, const double fact,const double *uvm, const double *uvp, double *uv)
{
  int    i, it, nT = 100;
  ...
  uvIT[0] = 0.5 * (uvm[0] + uvp[0]);
  uvIT[1] = 0.5 * (uvm[1] + uvp[1]);
  uvIT[2] = 0.0;
...
// min 1/2 ( l_0 ^ 2 + l_1 ^ 2) + lambda * (l_1 - l_0) = L(u, v, lambda)
// grad (L) = (L_1, L_2, L_3) = (0, 0, 0) --> solution using Newton x_n+1 = x_n + delta_n (3x3 system)
static void EG_minArcV4(const ego face, const double fact1, const double fact2,
                        const double *uvp, double *uv)
{
  int    i, it, nT = 100;
  double dlu[4], dlv[4], ddl[4], x0, x1, x2, e1 = 0.0, e2 = 0.0, l[4], p0[18], p2[18], p1[18],
         p3[18], pIT[18], ALU[4 * 4], r0[3], r1[3], r2[3], r3[3], delta[4], L[4], uvIT[4];

  //Initial guess uv = 0.5 (uv0 + uv1)
  uvIT[0] = 0.25 * (uvp[0] + uvp[2] + uvp[3], uvp[4]);
  uvIT[1] = 0.25 * (uvp[1] + uvp[3] + uvp[5], uvp[7]);
  uvIT[2] = uvIT[3] = 0.0;
  i       = EG_evaluate(face,  uvp   , p0);
  i      += EG_evaluate(face, &uvp[2], p1);
  i      += EG_evaluate(face, &uvp[2], p2);
  i      += EG_evaluate(face, &uvp[2], p3);
  i      += EG_evaluate(face, uvIT, pIT);
  if (i  != EGADS_SUCCESS) {
   printf(" EG_minArc :: EG_evaluate %d !!\n ", i);
   return;
  ...
  l [2] = DOTFOUR(r2, r2);
  l [3] = DOTFOUR(r3, r3);
  b1    = sqrt (l[0]) + sqrt(l[2]);
  b2    = sqrt (l[1]) + sqrt(l[3]);
  printf(" \n\n --------------------------------------------------- \n");
  printf(" INITIAL GUESS: TOTAL ARCS l02 = %lf l0 = %lf l2 = %lf  RATIOS %lf  %lf  (EXP %lf  %lf)\n",
           b1, l[0], l[2], l[0] / (b1 * b1), l[2]/(b1 * b1),fact1 * fact1, (1.0 - fact1) * (1.0 - fact1) );
  printf(" ARC l13 = %lf l1 = %lf l3 = %lf  RATIOS %lf  %lf  (EXP %lf  %lf)\n",
           b2, l[1], l[3], l[1] / (b2 * b2), l[3]/(b2 * b2),fact2 * fact2, (1.0 - fact2) * (1.0 - fact2) );
  l[0]  = sqrt(l[0]);
  l[1]  = sqrt(l[1]);
  l[2]  = sqrt(l[3]);
  l[3]  = sqrt(l[2]);
  printf(" As distances sqrt(l0) = %lf sqrt (l1) %lf\n ratios %lf %lf (EXPECTED %lf %lf)\n",
           l[0], l[2], l[0] / b1, l[2]/b1, fact1, (1.0 - fact1));
  printf(" As distances sqrt(l0) = %lf sqrt (l1) %lf\n ratios %lf %lf (EXPECTED %lf %lf)\n",
           l[1], l[3], l[1] / b1, l[3]/b1, fact1, (1.0 - fact2));
  printf(" --------------------------------------------------- \n");
#endif

  k1 = (1 - fact1) * (1.0 - fact1) / (fact1 * fact1);
  k2 = (1 - fact2) * (1.0 - fact2) / (fact2 * fact2);
  for (it = 0; it < nT; it++) {
      r0[0] = pIT[0] - p0[0];
      r0[1] = pIT[1] - p0[1];
      r0[2] = pIT[2] - p0[2];
      ...
      // UPDATE FUNCTION L
      L[0]   = dlu[0] * (l[0] - k1 * uvIT[2]) + dlu[1] * (l[1] - k2 * uvIT[3]) +
               dlu[2] * (l[2] +      uvIT[2]) + dlu[3] * (l[3] +      uvIT[3]);
      L[1]   = dlv[0] * (l[0] - k1 * uvIT[2]) + dlv[1] * (l[1] - k2 * uvIT[3]) +
      ...
      ALU[0]   = ddl[0] * (l[0] - k1 * uvIT[2]) + ddl[1]* (l[1] + uvIT[2]) +
                 ddl[2] * (l[2] - k2 * uvIT[3]) + ddl[3]* (l[3] + uvIT[3]) ;
      ...
      ALU[1]  = ddl[0] * (l[0] - k1 * uvIT[2]) + ddl[1]* (l[1] + uvIT[2]) +
                ddl[2] * (l[2] - k2 * uvIT[3]) + ddl[3]* (l[3] + uvIT[3]) ;
      ALU[1] += dlu[0] * dlv[0] + dlu[1] * dlv[1] +
                dlu[2] * dlv[2] + dlu[3] * dlv[3];
      // duv
      ddl[0]  = 2.0 * (pIT[15] * r[0][0] + pIT[6] * pIT[6] +
                       pIT[16] * r[0][1] + pIT[7] * pIT[7] +
                       pIT[17] * r[0][2] + pIT[8] * pIT[8]);
      ...
      ALU[5]  = ddl[0] * (l[0] - k1 * uvIT[2]) + ddl[1]* (l[1] + uvIT[2]) +
                ddl[2] * (l[2] - k2 * uvIT[3]) + ddl[3]* (l[3] + uvIT[3]) ;
      ALU[5] += dlu[0] * dlv[0] + dlu[1] * dlv[1] +
                dlu[2] * dlv[2] + dlu[3] * dlv[3];

      ALU[2 ] = dlu[2] - k1 * dlu[0];
      ALU[3 ] = dlu[3] - k2 * dlu[1];
      ALU[6 ] = dlv[2] - k1 * dlv[0];
      ALU[7 ] = dlv[3] - k2 * dlv[1];
      ...
      ALU[12] = ALU[3];
      ALU[13] = ALU[7];
      ALU[10] = ALU[11] = ALU[12] = ALU[13] = 0.0;
  ...
  b2    = sqrt (l[1]) + sqrt(l[3]);
  printf(" \n\n --------------------------------------------------- \n");
  printf(" FINAL ARCS l02 = %lf l0 = %lf l2 = %lf  RATIOS %lf  %lf  (EXP %lf  %lf)\n",
           b1, l[0], l[2], l[0] / (b1 * b1), l[2]/(b1 * b1),fact1 * fact1, (1.0 - fact1) * (1.0 - fact1) );
  printf(" ARC l13 = %lf l1 = %lf l3 = %lf  RATIOS %lf  %lf  (EXP %lf  %lf)\n",
           b2, l[1], l[3], l[1] / (b2 * b2), l[3]/(b2 * b2),fact2 * fact2, (1.0 - fact2) * (1.0 - fact2) );
  l[0]  = sqrt(l[0]);
...
static int EG_centroid(meshMap *qm, int n, int *list, double *quv, int usequv)
{
  ...
  EG_minArc(qm->face, 0.3, xyz0, xyz1, quv);
  return EGADS_SUCCESS;
