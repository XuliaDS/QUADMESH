...
static void EG_minArc3 (const ego face, const double *uvp, double *uv)
{
  int    stat, i, it, nT = 100;
  double x0 = 0.0, x1 = 0.0, x2 = 0.0, e1 = 0.0, e2 = 0.0, pIT[18], lt, l[3], p0[18], p1[18], p2[18], dlu[3],
         dlv[3], L[4], ALU[4 * 4], uvIT[4], delta[4], r0[3], r1[3], r2[3], dluu[3], dluv[3], dlvv[3];

  //Initial guess uv = 0.5 (uv0 + uv1)
  // get all point coordinates
  uvIT[0] = (uvp[0] + uvp[2] + uvp[4]) / 3.0;
  uvIT[1] = (uvp[1] + uvp[3] + uvp[5]) / 3.0;

  uvIT[2] = uvIT[3] = 0.0;
  stat    = EG_evaluate(face,     uvp, p0);
  stat   += EG_evaluate(face, &uvp[2], p1);
  stat   += EG_evaluate(face, &uvp[4], p2);
...
printf(" %lf %lf %lf C\n", pIT[0], pIT[1], pIT[2]);
  ...
  for (it  = 0; it < nT; it++) {
      ...
      L  [2]  = l[1] - l[0];
      L  [3]  = l[2] - l[1];
      ALU[0]  = DOT(dluu, l) + DOT(dlu, dlu) + uvIT[2] * (dluu[1] - dluu[0]) +
                                               uvIT[3] * (dluu[2] - dluu[1]);

      ALU[1]  = DOT(dluv, l) + DOT(dlu, dlv) + uvIT[2] * (dluv[1] - dluv[0]) +
                                               uvIT[3] * (dluv[2] - dluv[1]);

      ALU[5]  = DOT(dlvv, l) + DOT(dlv, dlv) + uvIT[2] * (dlvv[1] - dlvv[0]) +
      ...
      ALU[12] = ALU[3];
      ALU[13] = ALU[7];
      ALU[10] = ALU[11] = ALU[14] = ALU[15] = 0.0;
      solveLU(4, ALU, L, delta);
      x2 = DOT4(delta, delta);
      uvIT[0]  -= delta[0];
      uvIT[1]  -= delta[1];
      uvIT[2]  -= delta[2];
...
// min 1/2 ( l_0 ^ 2 + l_1 ^ 2) + lambda * (l_1 - l_0) = L(u, v, lambda)
// grad (L) = (L_1, L_2, L_3) = (0, 0, 0) --> solution using Newton x_n+1 = x_n + delta_n (3x3 system)
static void EG_minArc2(const ego face, const double fact, const double *uvp, double *uv)
{
  int    i, it, nT = 100;
  double b, dlu0, dlu1, dlv0, dlv1, ddl0, ddl1, detJ, x0, x1, x2, e1 = 0.0, e2 = 0.0, ctt;
  double l[2], pM[18], pP[18], pIT[18], J[3][3], ATJ[3][3], r0[3], r1[3], delta[3], L[3], uvIT[3];

  //Initial guess uv = 0.5 (uv0 + uv1)
  printf(" NORMAL MIN ARC \n");
  uvIT[0] = 0.5 * (uvp[0] + uvp[2]);
  uvIT[1] = 0.5 * (uvp[1] + uvp[3]);
  uvIT[2] = 0.0;
  i       = EG_evaluate(face,  uvp   , pM);
  i      += EG_evaluate(face, &uvp[2], pP);
  i      += EG_evaluate(face, uvIT, pIT);
  if (i  != EGADS_SUCCESS) {
   printf(" EG_minArc :: EG_evaluate %d !!\n ", i);
   return;
  ...
  l[0], l[1], l[0] / b, l[1]/b, fact, (1.0 - fact));
  printf(" --------------------------------------------------- \n");
  if (fabs (l[0] / b -        fact)  > EPS08 ||
      fabs (l[1] / b - (1.0 - fact)) > EPS08) {
      printf(" EG_minArc2 Didn't work!!!!! \n");
  }
#endif
 uv[0] = uvIT[0];
 uv[1] = uvIT[1];
...
// grad (L) = (L_1, L_2, L_3) = (0, 0, 0) --> solution using Newton x_n+1 = x_n + delta_n (3x3 system)
static
void EG_minArc4(const ego face,  double fact1,  double fact2,
                 const double *uvp, double *uv)
{
  int    i, it, nT = 100;
  double x0, x1, x2, e1 = 0.0, e2 = 0.0, l[4], p0[18], p2[18], p1[18], p3[18], pIT[18],ALU[4 * 4], L[4],
         delta[4], dlu[4], dlv[4], ddl[4], k02, k13,lt02, lt13, r0[3], r1[3], r2[3], r3[3], uvIT[4];

  //Initial guess uv = 0.5 (uv0 + uv1)

  uvIT[0] = 0.25 * (uvp[0] + uvp[2] + uvp[4] + uvp[6]);
  uvIT[1] = 0.25 * (uvp[1] + uvp[3] + uvp[5] + uvp[7]);
  uvIT[2] = uvIT[3] = 0.0;
  i       = EG_evaluate(face,  uvp   , p0);
  i      += EG_evaluate(face, &uvp[2], p1);
  i      += EG_evaluate(face, &uvp[4], p2);
  i      += EG_evaluate(face, &uvp[6], p3);
  i      += EG_evaluate(face, uvIT, pIT);
#ifdef DEBUG
  printf("%lf %lf %lf 1\n", p0[0], p0[1], p0[2]);
  printf("%lf %lf %lf 2\n", p1[0], p1[1], p1[2]);
  ...
  lt02  = sqrt (l[0]) + sqrt(l[2]);
  lt13  = sqrt (l[1]) + sqrt(l[3]);
  printf(" \n\n --------------------------------------------------- \n");
  printf(" INITIAL GUESS: TOTAL ARCS l02 = %lf l0 = %lf l2 = %lf  RATIOS %lf  %lf  (EXP %lf  %lf)\n",
           lt02, l[0], l[2], l[0] / (lt02 * lt02), l[2]/(lt02 * lt02),fact1 * fact1, (1.0 - fact1) * (1.0 - fact1) );
  printf(" ARC l13 = %lf l1 = %lf l3 = %lf  RATIOS %lf  %lf  (EXP %lf  %lf)\n",
           lt13, l[1], l[3], l[1] / (lt13 * lt13), l[3]/(lt13 * lt13),fact2 * fact2, (1.0 - fact2) * (1.0 - fact2) );
  l[0]  = sqrt(l[0]);
  l[1]  = sqrt(l[1]);
  l[2]  = sqrt(l[2]);
  l[3]  = sqrt(l[3]);
...
#endif
   //fact1 = fact2 = 0.5;
  k02 = (1.0 - fact1) * (1.0 - fact1) / (fact1 * fact1);
  k13 = (1.0 - fact2) * (1.0 - fact2) / (fact2 * fact2);
  printf(" l2 = %lf l0  l3 = %lf l1\n", k02, k13);
  for (it   = 0; it < nT; it++) {
      r0[0] = pIT[0] - p0[0]; r0[1] = pIT[1] - p0[1]; r0[2] = pIT[2] - p0[2];
      r1[0] = pIT[0] - p1[0]; r1[1] = pIT[1] - p1[1]; r1[2] = pIT[2] - p1[2];
  ...
  printf(" r2 %lf %lf %lf\n"       , r2[0], r2[1], r2[2]);
  printf(" r3 %lf %lf %lf\n"       , r3[0], r3[1], r3[2]);
  printf(" NORMS %lf %lf %lf %lf\n", l[0], l[1], l[2], l[3]);
#endif
      dlu[0]  = 2.0 * (r0[0] * pIT[3] + r0[1] * pIT[4] + r0[2] * pIT[5]);
      dlu[1]  = 2.0 * (r1[0] * pIT[3] + r1[1] * pIT[4] + r1[2] * pIT[5]);
      dlu[2]  = 2.0 * (r2[0] * pIT[3] + r2[1] * pIT[4] + r2[2] * pIT[5]);
      dlu[3]  = 2.0 * (r3[0] * pIT[3] + r3[1] * pIT[4] + r3[2] * pIT[5]);

      dlv[0]  = 2.0 * (r0[0] * pIT[6] + r0[1] * pIT[7] + r0[2] * pIT[8]);
      dlv[1]  = 2.0 * (r1[0] * pIT[6] + r1[1] * pIT[7] + r1[2] * pIT[8]);
      dlv[2]  = 2.0 * (r2[0] * pIT[6] + r2[1] * pIT[7] + r2[2] * pIT[8]);
      dlv[3]  = 2.0 * (r3[0] * pIT[6] + r3[1] * pIT[7] + r3[2] * pIT[8]);

      // UPDATE FUNCTION L
      L[0]    = dlu[0] * (l[0] - k02* uvIT[2]) + dlu[1] * (l[1] - k13 * uvIT[3]) +
                dlu[2] * (l[2] +      uvIT[2]) + dlu[3] * (l[3] +       uvIT[3]);

      L[1]    = dlv[0] * (l[0] - k02* uvIT[2]) + dlv[1] * (l[1] - k13 * uvIT[3]) +
                dlv[2] * (l[2] +      uvIT[2]) + dlv[3] * (l[3] +       uvIT[3]);

      L[2]    =   l[2] - k02 * l[0];
      L[3]    =   l[3] - k13 * l[1];
      // duu
      ddl[0]  = 2.0 * (pIT[9 ] * r0[0] + pIT[3] * pIT[3] +
                      pIT[10] * r0[1] + pIT[4] * pIT[4] +
                      pIT[11] * r0[2] + pIT[5] * pIT[5]);

      ddl[1]  = 2.0 * (pIT[9 ] * r1[0] + pIT[3] * pIT[3] +
                      pIT[10] * r1[1] + pIT[4] * pIT[4] +
                      pIT[11] * r1[2] + pIT[5] * pIT[5]);

      ddl[2]  = 2.0 * (pIT[9 ] * r2[0] + pIT[3] * pIT[3] +
                      pIT[10] * r2[1] + pIT[4] * pIT[4] +
                      pIT[11] * r2[2] + pIT[5] * pIT[5]);

      ddl[3]  = 2.0 * (pIT[9 ] * r3[0] + pIT[3] * pIT[3] +
                      pIT[10] * r3[1] + pIT[4] * pIT[4] +
                      pIT[11] * r3[2] + pIT[5] * pIT[5]);
      ALU[0]  = ddl[0] * (l[0] - k02 * uvIT[2]) + ddl[2] * (l[2] + uvIT[2]) +
                ddl[1] * (l[1] - k13 * uvIT[3]) + ddl[3] * (l[3] + uvIT[3]) ;
      ALU[0] += dlu[0] * dlu[0] + dlu[1] * dlu[1] +
                dlu[2] * dlu[2] + dlu[3] * dlu[3];
      // duv
      ddl[0]  = 2.0 * (pIT[12] * r0[0] + pIT[3] * pIT[6] +
                       pIT[13] * r0[1] + pIT[4] * pIT[7] +
                       pIT[14] * r0[2] + pIT[5] * pIT[8]);

      ddl[1]  = 2.0 * (pIT[12] * r1[0] + pIT[3] * pIT[6] +
                       pIT[13] * r1[1] + pIT[4] * pIT[7] +
                       pIT[14] * r1[2] + pIT[5] * pIT[8]);

      ddl[2]  = 2.0 * (pIT[12] * r2[0] + pIT[3] * pIT[6] +
                       pIT[13] * r2[1] + pIT[4] * pIT[7] +
                       pIT[14] * r2[2] + pIT[5] * pIT[8]);

      ddl[3]  = 2.0 * (pIT[12] * r3[0] + pIT[3] * pIT[6] +
                       pIT[13] * r3[1] + pIT[4] * pIT[7] +
                       pIT[14] * r3[2] + pIT[5] * pIT[8]);
      ALU[1]  = ddl[0] * (l[0] - k02 * uvIT[2]) + ddl[2] * (l[2] + uvIT[2]) +
                ddl[1] * (l[1] - k13 * uvIT[3]) + ddl[3] * (l[3] + uvIT[3]) ;
      ALU[1] += dlu[0] * dlv[0] + dlu[1] * dlv[1] +
                dlu[2] * dlv[2] + dlu[3] * dlv[3];
      // duv
      ddl[0]  = 2.0 * (pIT[15] * r0[0] + pIT[6] * pIT[6] +
                       pIT[16] * r0[1] + pIT[7] * pIT[7] +
                       pIT[17] * r0[2] + pIT[8] * pIT[8]);

      ddl[1]  = 2.0 * (pIT[15] * r1[0] + pIT[6] * pIT[6] +
                       pIT[16] * r1[1] + pIT[7] * pIT[7] +
                       pIT[17] * r1[2] + pIT[8] * pIT[8]);

      ddl[2]  = 2.0 * (pIT[15] * r2[0] + pIT[6] * pIT[6] +
                       pIT[16] * r2[1] + pIT[7] * pIT[7] +
                       pIT[17] * r2[2] + pIT[8] * pIT[8]);

      ddl[3]  = 2.0 * (pIT[15] * r3[0] + pIT[6] * pIT[6] +
                       pIT[16] * r3[1] + pIT[7] * pIT[7] +
                       pIT[17] * r3[2] + pIT[8] * pIT[8]);
      ALU[5]  = ddl[0] * (l[0] - k02 * uvIT[2]) + ddl[2] * (l[2] + uvIT[2]) +
                ddl[1] * (l[1] - k13 * uvIT[3]) + ddl[3] * (l[3] + uvIT[3]) ;
      ALU[5] += dlv[0] * dlv[0] + dlv[1] * dlv[1] +
                dlv[2] * dlv[2] + dlv[3] * dlv[3];
      ALU[2 ] = dlu[2] - k02 * dlu[0];
      ALU[3 ] = dlu[3] - k13 * dlu[1];
      ALU[6 ] = dlv[2] - k02 * dlv[0];
      ALU[7 ] = dlv[3] - k13 * dlv[1];
      ...
      ALU[10] = ALU[11] = ALU[14] = ALU[15] = 0.0;
#ifdef DEBUGG
      printf(" CALL LU WITH A =  \n");
      printf("   %lf %lf %lf %lf \n", ALU[0] , ALU[1], ALU[2] , ALU[3]);
      printf("   %lf %lf %lf %lf \n", ALU[4] , ALU[5], ALU[6] , ALU[7]);
      printf("   %lf %lf %lf %lf \n", ALU[8] , ALU[9], ALU[10], ALU[11]);
      printf("   %lf %lf %lf %lf \n", ALU[12],ALU[13], ALU[14], ALU[15]);
  ...
  printf(" As distances sqrt(l0) = %lf sqrt(l2) %lf ratios %lf | %lf (EXP %lf %lf)\n",
           l[0], l[2], l[0] / lt02, l[2]/lt02, fact1, (1.0 - fact1));
  printf(" As distances sqrt(l1) = %lf sqrt (l3) %lf ratios %lf %lf (EXP %lf %lf)\n",
           l[1], l[3], l[1] / lt13, l[3]/lt13, fact2, (1.0 - fact2));
  printf(" --------------------------------------------------- \n");
...
static
void EG_equalArc(ego face, int n, double *uvIN, double *uvOUT) {
    switch(n) {
    case 2:
        EG_minArc2(face, 0.5, uvIN, uvOUT);
        return;
    case 3:
        EG_minArc3(face, uvIN, uvOUT);
        return;
    case 4:
        EG_minArc4(face, 0.5, 0.5, uvIN, uvOUT);
        return;
    default: return;
    }
}
  ...
  for (s = 0; s < qm->star[vID-1]->nQ; s++) {
      ...
      if (doublet == 0 && qa[0] > qEPS && qa[1] > qEPS &&
          ...
          if (qm->star[vID-1]->area[s] == QA0) {
              pen = 1.0 - ERFC(ANGCUT, 0.5 * ( PI + ANGCUT), maiv); // penalty kicks when angle > ANGCUT
...
static int EG_placeVertex(meshMap *qm, int vID, double pass, int full, int report) {
  ...
  double uv[6], sra[3], srb[3], angopt =0.0, angpen, errbd, suv[8];
  int *qInfo = NULL;
  char buffer[100];
  int d1, d2;
  double p[2], pos[18];
  ...
  for (round = 0; round < 2; round++) {
      for (q = 0; q <= nt; q++) {
          ...
          if (q == 0) {
              if (ja == QA0 && qm ->star[v]->nQ < 5 && full == 2) {
                  printf(" STAR CENTRE %d \n", v+ 1);
                  if (qm ->star[v]->nQ  == 4) printf(" s01 %d s23 %d s45 %d s67 %d\n",
                             qm->star[v]->verts[1],qm->star[v]->verts[3]
                          ,qm->star[v]->verts[5],qm->star[v]->verts[7]);
                  suv[0] = qm->uvs[2 * (qm->star[v]->verts[1] - 1)    ];
                  suv[1] = qm->uvs[2 * (qm->star[v]->verts[1] - 1) + 1];
                  suv[2] = qm->uvs[2 * (qm->star[v]->verts[3] - 1)    ];
                  suv[3] = qm->uvs[2 * (qm->star[v]->verts[3] - 1) + 1];
                  suv[4] = qm->uvs[2 * (qm->star[v]->verts[5] - 1)    ];
                  suv[5] = qm->uvs[2 * (qm->star[v]->verts[5] - 1) + 1];
                  if ( qm-> star[v] -> nQ == 4) {
                      suv[6] = qm->uvs[2 * (qm->star[v]->verts[7] - 1)    ];
                      suv[7] = qm->uvs[2 * (qm->star[v]->verts[7] - 1) + 1];
                  }
                  EG_equalArc(qm->face, qm->star[v] -> nQ, suv, &uv[2]);
              } else
              EG_centroid(qm, qm->star[v]->nQ, &qm->valence[v][3], &uv[2], 0);
              if (la != -1 && ja == QA0) {
                  if (full != 2) continue;
              ...
              EG_equalArc(qm->face, 2, suv, &uv[2]);
          } else if (la != -1) {
