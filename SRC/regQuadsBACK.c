                                  ...
                                  double *angle)
{
    ...
    *angle = 0.0;
    ...
    if (qm->vType[vC-1] != -1) {
      centre[0] = qm->xyzs[3 * (vC - 1)    ];
      centre[1] = qm->xyzs[3 * (vC - 1) + 1];
      centre[2] = qm->xyzs[3 * (vC - 1) + 2];
    }
    ...
    CROSS (v01, v02, cross);
    if (norm1 < qEPS) {
        v01[0] = qm->xyzs[3 * (v1 - 1)    ] - centre[0];
        v01[1] = qm->xyzs[3 * (v1 - 1) + 1] - centre[1];
        v01[2] = qm->xyzs[3 * (v1 - 1) + 2] - centre[2];
        v02[0] = qm->xyzs[3 * (v2 - 1)    ] - centre[0];
        v02[1] = qm->xyzs[3 * (v2 - 1) + 1] - centre[1];
        v02[2] = qm->xyzs[3 * (v2 - 1) + 2] - centre[2];
        v02[2] = xyz[2] - centre[2];
        norm1  = DOT(v01, v01);
        norm2  = DOT(v02, v02);
        norm1  = sqrt (norm1 * norm2);
...
{
    int s, i, k, k1, stat, vA, vB, vC, vD, count, doublet, qV[4], ori[4], bv[4], lr[2], iv = -1, bvp;
    int selfint = 1, cw, s1, s2, totArea = 0;
    double pABCD[12], cross[3], vNormal[3], vAB[3], vAC[3], vAD[3], vIDxyz[18], xyz[18], tria[8], vr[2];
    double c, dotNP, lambda, norm1, norm2, ang[4], dot, qa[2], pen = 1.0, ma = 0.0, maiv=0.0, mba = 0.0, uv[2];

    if (vID <= 0 || vID > qm->totV) return QA0;
    ...
    for (s = 0 ; s < qm->star[vID-1]->nQ; s++) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if ( report == 1 ) printf(" %lf %lf %lf %d\n",pABCD[3 * i    ], pABCD[3 * i + 1],pABCD[3 * i + 2], qV[i]+1 );
        }
        qa[0]   = qa[1] = 0.0;
        selfint = 1;
        iv      = -1;
        ...
        for (doublet = count = k = 0; k < 4; ++k) {
            ...
            if (qV[k] + 1 == vID) iv = k;
            if (qm->vType[qV[k]] != -1 ||
        	qm->vType[qm->valence[qV[k]][3] -1] > 0) bv[k] = k;
            if (qm->valence[qV[k]][2] * qm->vType[qV[k]] == -2) {
             ...
             //   tria[k] = 0.5 * sqrt(DOT(cross, cross)) ;
                ...
                /*if ( report == 1 ) printf(" ANGLE FAR PROJECTION:: %lf \n", ang[k]);
                uv[0]  = 0.1 * qm->uvs[2 * qV[vB]    ] + 0.9 * qm->uvs[2 * qV[vA]    ];
                uv[1]  = 0.1 * qm->uvs[2 * qV[vB] + 1] + 0.9 * qm->uvs[2 * qV[vA] + 1];
                EG_evaluate(qm->face, uv, xyz);
                vAB[0] = xyz[0] - qm->xyzs[3 * qV[vA]    ];
                vAB[1] = xyz[1] - qm->xyzs[3 * qV[vA] + 1];
                vAB[2] = xyz[2] - qm->xyzs[3 * qV[vA] + 2];
                uv[0]  = 0.1 * qm->uvs[2 * qV[vD]    ] + 0.9 * qm->uvs[2 * qV[vA]    ];
                uv[1]  = 0.1 * qm->uvs[2 * qV[vD] + 1] + 0.9 * qm->uvs[2 * qV[vA] + 1];
                EG_evaluate(qm->face, uv, xyz);
                vAD[0] = xyz[0] - qm->xyzs[3 * qV[vA]    ];
                vAD[1] = xyz[1] - qm->xyzs[3 * qV[vA] + 1];
                vAD[2] = xyz[2] - qm->xyzs[3 * qV[vA] + 2];
                norm1 = sqrt(DOT(vAB, vAB) * DOT(vAD, vAD));
                if (norm1 < qEPS ) dot = 1.0;
                else dot = DOT(vAB, vAD) / norm1;
                if      (fabs(dot - 1.0) < EPS08) ang[k] = 0.0;
                ...
                ma = MAX(ma, ang[k]);
                if ((qm->vType[qV[k]] > 0 && qm->bdAng[qV[k]] > PI) ||
                    (qm->vType[qm->valence[qV[k]][3] - 1] > 0 &&
                     qm->bdAng[qm->valence[qV[k]][3] - 1] > PI)) mba = MAX(mba, ang[k]);
            }
            if (ori[k] == -1) continue;
            selfint = 0;
            count++;
        ...
        if (doublet == 0 && qa[0] > qEPS && qa[1] > qEPS &&
            ...
            if ( report == 1)
                printf(" Tri ratio from  %lf  %lf ---> iv %d\n", vr[0], vr[1], iv%2);
        	...
        	if (report == 1)
        	  printf(" ANGLE %lf CALL ERFC WITH %lf %lf --> %lf\t", dot, ANGCUT, PI, pen);
        	if (ang[(iv + 2) %4] > ANGCUT) qm->star[vID-1]->ratios[s] = vr[0];
                else                           qm->star[vID-1]->ratios[s] = vr[0] * vr[1];//MIN(vr[0], vr[1]);
            } else {
                    vA = 0;
                    if (qa[0] > qa[1] ) vA = 1;
                    if (tria[k] + tria [k + 2] > qa[vA]) // MIN AREA = qa[vA]
                        qm->star[vID-1]->ratios[s] = -vr[0];
                    else if (tria[k1] / tria[(k1 + 2)%4] < 1.0)
                        qm->star[vID-1]->ratios[s] = -vr[1];
                    else
                        qm->star[vID-1]->ratios[s] = vr[0];
            }
        }
        for (k = 0 ; k < 4; k++ ) {
            ...
            for (lr[0] = lr[1] = i = 0; i < qm->star[qV[k]]->nQ; i++) {
                ...
                if ( qm->star[qV[k]]->quads[i] == -1) {
                    lr[1] = qm->star[qV[k]]->verts[2 * i + 1];
                    lr[0] = qm->star[qV[k]]->verts[qm->star[qV[k]]->idxV[2 * i + 3]];
                    break;
                }
            }
            ...
            if (k != (iv + 2)%4) {
                dotNP  = vNormal[0] * qm->xyzs[3 * (lr[1] -1)    ] +
                        vNormal[1] * qm->xyzs[3 * (lr[1] -1) + 1] +
                        vNormal[2] * qm->xyzs[3 * (lr[1] -1) + 2];
                lambda = (c - dotNP);
                xyz[3] = qm->xyzs[3 * (lr[1] -1)    ] + lambda * vNormal[0];
                xyz[4] = qm->xyzs[3 * (lr[1] -1) + 1] + lambda * vNormal[1];
                xyz[5] = qm->xyzs[3 * (lr[1] -1) + 2] + lambda * vNormal[2];

                vAB[0] = xyz[0] - pABCD[3 * k    ];
                vAB[1] = xyz[1] - pABCD[3 * k + 1];
                vAB[2] = xyz[2] - pABCD[3 * k + 2];
                vAC[0] = xyz[3] - pABCD[3 * k    ];
                vAC[1] = xyz[4] - pABCD[3 * k + 1];
                vAC[2] = xyz[5] - pABCD[3 * k + 2];
                CROSS(vAB, vAC, cross);
            ...
            if (lr[1] == qV[(k +1)%4] + 1 ||
                    lr[0] == qV[(k +3)%4] + 1) continue;
            for ( k1 = 1; k1 <= 2; k1++) {
                vB     = (k + k1    )%4;
                vC     = (k + k1 + 1)%4;
                if (qV[vB] + 1 == lr[0] || qV[vB] + 1 == lr[1] ||
                        qV[vC] + 1 == lr[0] || qV[vC] + 1 == lr[1] )continue;
                vAB[0] = pABCD[3*vB    ] - pABCD[3 * k    ];
                vAB[1] = pABCD[3*vB + 1] - pABCD[3 * k + 1];
                vAB[2] = pABCD[3*vB + 2] - pABCD[3 * k + 2];
                vAC[0] = pABCD[3*vC    ] - pABCD[3 * k    ];
                vAC[1] = pABCD[3*vC + 1] - pABCD[3 * k + 1];
                vAC[2] = pABCD[3*vC + 2] - pABCD[3 * k + 2];
                vAD[0] = xyz[0]          - pABCD[3 * k    ];
                vAD[1] = xyz[1]          - pABCD[3 * k + 1];
                vAD[2] = xyz[2]          - pABCD[3 * k + 2];
                CROSS(vAB, vAC, cross);
                cw = 0;
                ...
                if ((cw == 0 && s1 == 0 && s2 == 0) ||
                    ...
                    if ( report == 1)  {
                        ...
                        printf(" LINK AB AL %d %d  %d %d = %d\n ", qV[k] + 1, qV[vB] + 1, qV[k] + 1, lr[0], s1);
                        printf(" LINK AL AC %d %d  %d %d = %d\n ", qV[k] + 1, lr[0], qV[k] + 1, qV[vC] + 1, s2);
        ...
        if(report == 1) {
            if (qm->star[vID-1]->areas[s] != QA0) {
                ...
                if (qm->star[vID-1]->areas[s] % QA2 == 0 )
        ...
        if(report == 1) {
            printf("\n==========  star area %d Q %d BP %d AT V CENTRE %lf=======================\n",
                   vID, qm->star[vID-1]->quads[s],  bvp,  qm->star[vID-1]->angles[s]);
...
static int EG_placeVertex(meshMap *qm, int vID, double pass, int full, int report) {
    int q, v, j, i, k, *vl = NULL, ja = 0, jb = 0, update = 0, v1, v2, v3, corner,
         vqp = 0, vqn = 0, la = -1, lb, nt, block = 0, round, doublet = 0, bt = -1;
    double uv[6], sra[3], srb[3], *aAngle = NULL, *aRatio = NULL, angopt =0.0, angpen, errbd, minaux;
	...
	if (qm->vType[v1] > 0 || qm->vType[v2] > 0 ||
	    ...
	    if (qm->bdAng[v2] > PI || qm->bdAng[v3] > PI) qInfo[i] = 2;
	    qInfo[i] = MAX(qInfo[i], 1);
	}
	if ((qm->vType[qm->valence[v1][3] -1] > 0 &&
	    qm->bdAng[qm->valence[v1][3] -1] > PI ) ||
    ...
    uv [4] = uv[0];
    uv [5] = uv[1];
    sra[0] = sra[1] = sra[2] = sra[3] = minaux = 1.0;
	...
	if (aArea [j] >= QA3) continue;
        if ( report == 1) printf(" BT %d QINFO %d =  %d \n", bt, qm->star[v]->quads[j], qInfo[j]);
	if (signbit(aRatio[j]) == 1) sra[1] = MIN(sra[1], aRatio[j] );
	else {
	    if (qInfo[j] >= bt ) minaux = MIN(minaux, aRatio[j]);
	    sra[2] = MIN(sra[2], aRatio[j] );
	}
	/*else if (bt <= qInfo[j] || aRatio[j] < pass) {
            if (qInfo[j] >= bt ) minaux = MIN(minaux, aRatio[j]);
            sra[2] = MIN(sra[2], aRatio[j] );
        }*/
	angpen = MAX(angpen, aAngle[j]);
    }
    errbd = 1.0;
    if ( minaux < pass && sra[2] < minaux ) sra[2] = minaux;
    if (corner >= 0 && qm->star[v]->nQ >= 4 ) {
        EG_angAtBdVert(qm, qm->star[v]->verts[2 * corner + 2], vl, &angopt);
        if (angopt > PI) angopt = 2.0 * PI - angopt;
        errbd = fabs(angopt - qm->star[v]->angles[corner]);
        errbd = (PI - errbd)/ PI;
        if (report == 1) printf(" WE HAVE A CORNER ERROR AT V OPP %lf  \n", errbd);
    } else if (la != -1 && qm->valence[qm->star[v]->verts[la]- 1][2] == 4) {
        EG_angAtBdVert(qm, qm->star[v]->verts[la], vl, &angopt);
        angopt = (2.0 * PI - angopt);
        vqp = qm->star[v]->verts[qm->star[v]->idxV[la + 1]];
        vqn = qm->star[v]->verts[qm->star[v]->idxV[la - 1]];
        if (qm->vType[vqp -1] > 0) vqp = vID;
        else vqn = vID;
        EG_angleAtVnormalPlane(qm, qm->star[v]->verts[la], vqp, vqn, &errbd);
        if ( report == 1)
            printf("OPT ANGLE %lf  ANGLE %d %d %d is %lf ERRO %lf SCALED %lf  \n", angopt, qm->star[v]->verts[la], vqp, vqn, errbd,
                   fabs(angopt - errbd), ( PI - fabs(angopt - errbd)) / PI );
        errbd = fabs(angopt - errbd);
        errbd = (PI - errbd ) / PI ;
    }
    if (qm->star[v]->nQ == 4) {
        sra[0] = MAX(fabs(qm->star[v]->angles[0] - qm->star[v]->angles[2]),
                     fabs(qm->star[v]->angles[1] - qm->star[v]->angles[3]));
        sra[0] = (PI - sra[0] ) / PI; // valid star -> angles[i] < pi, forall i
        if ( report == 1) printf(" ANGLE AT Q4 ERR %lf \n", sra[0]);
    } else {
        sra[0] = fabs(1.0 - ERFC(PI * 16. / 18., PI, angpen));
        if (report == 1) printf(" VALENCE %d angpen %lf sra %lf\n", qm->star[v]->nQ , angpen, sra[0]);
    }
    if ( ja == QA0) sra[0] = MIN(sra[0], fabs(1.0 -ERFC(ANGCUT, PI, angpen)));
    sra[0] = MIN (errbd, sra[0]);
    if ( report == 1)
    printf(" ANGPEN %lf   ERFC %lf  --> SRA %lf  ERR AT BOUND %lf \n",
           angpen, ERFC(ANGCUT, PI, angpen), sra[0], errbd);
    block = 0;
    for (round = 0; round < 2; round++) {
        ...
        for (q = 0; q <= nt; q++) {
            if (q > 0 && block > 0) break;
            if (q > 0 &&
               ((pass < 0.0 || qm-> star[v]->nQ == 3) ||
                (qm ->star[v]->nQ >= 5 && ja == QA0 &&  bt != 2))) break;
            uv[2] = uv[0];
            uv[3] = uv[1];
            updateVertex(qm, vID, uv);
            if(report == 1)
                printf (" =1=1=11=1=1 ROUND %d  la %d  q %d / %d   RATIO PASS %lf\n ", round, la, q, nt, pass);
            if (q == 0) {
                EG_centroid(qm, qm->star[v]->nQ,
                                 &qm->valence[v][3],&uv[2], (round+1)%2);
                if ( la != -1 && ja == QA0 && round == 2) {
                ...
                if (la != -1) {
                    ...
                    if (q - 1 >= qm->star[v]->nQ ||
                       (   ja == QA0 &&  sra[2] > 0.1 && full == 2 )) break;
                    vl[0] = v + 1;
                    i     = 1;
                    for (k = 0; k < qm->star[v]->nQ; k++) {
                        j = qm->star[v]->verts[2 * k + 1] -1;
                        if (k == q - 1 || k == q ||
                                qm->vType[j] * qm->valence[j][2] == -2 ||
                                qm->vType[j] > 0) continue;
                        vl[i++] = j + 1;
                    }
            ...
            srb[0] = srb[1] = srb[2] = srb[3] = minaux = 1.0;
        	...
        	/*else if (bt <= qInfo[j] || qm->star[v]->ratios[j] < pass) {
                          if (qInfo[j] >= bt ) minaux = MIN(minaux, qm->star[v]->ratios[j]);
                          srb[2] = MIN (srb[2], qm->star[v]->ratios[j] );
                      }*/
        	else {
        	    if (qInfo[j] >= bt ) minaux = MIN(minaux, qm->star[v]->ratios[j]);
        	    srb[2] = MIN(srb[2], qm->star[v]->ratios[j] );
        	}
        	angpen = MAX (angpen, qm->star[v]->angles[j]);
            }
            errbd = 1.0;
            if ( minaux < pass && srb[2] < minaux ) srb[2] = minaux;
            if (corner >= 0 ) {
                errbd = fabs(angopt - qm->star[v]->angles[corner]);
                errbd = (PI - errbd)/ PI;
            }else if (la != -1 && qm->valence[qm->star[v]->verts[la]- 1][2] == 4) {
                EG_angleAtVnormalPlane(qm, qm->star[v]->verts[la], vqp, vqn, &errbd);
                if ( report == 1)
                    printf(" ANGLE %d %d %d is %lf ERRO %lf SCALED %lf  \n",  qm->star[v]->verts[la], vqp, vqn, errbd,
                           fabs(angopt - errbd), ( PI - fabs(angopt - errbd)) / PI );
            ...
            if (qm->star[v]->nQ == 4 ) {
                srb[0] = MAX(fabs(qm->star[v]->angles[0] - qm->star[v]->angles[2]),
                             fabs(qm->star[v]->angles[1] - qm->star[v]->angles[3]));
                srb[0] = (PI - srb[0] ) / PI; // valid star -> angles[i] < pi, forall i
            } else srb[0] = fabs(1.0 - ERFC(PI * 16. / 18., PI, angpen));
            if ( jb == QA0) srb[0] = MIN(srb[0], fabs(1.0 -ERFC(ANGCUT, PI, angpen)));
            srb[0] = MIN (errbd, srb[0]);
            if ( report == 1)
                printf(" ANGPEN %lf   ERFC %lf  --> SRA %lf  ERR AT BOUND %lf \n",
                       angpen, ERFC(ANGCUT, PI, angpen), sra[0], errbd);
            if ( sra[0] > 0.75 && srb[0] > 0.75 ) {
                sra[0] = 1.0; srb[0] = 1.0;
            }
            if(report == 1) {
                ...
                printf(" NEXT AREA %d RATIOS BIAS %lf NEG %lf MIN %lf PROD %lf\n"
                       " BEST AREA %d RATIOS BIAS %lf NEG %lf MIN %lf PROD %lf\n",
                            jb, srb[0], srb[1], srb[2], srb[0] * srb[2],
                            ja, sra[0], sra[1], sra[2], sra[0] * sra[2]);
                printVertex(qm, vID);
            ...
            if (signbit(pass) == 1 && bt < 2 && (jb <  QA2) ||
                ...
                if (report == 1) printf(" LOOKING CAREFULLY :: \n");
                if (q == 0 && round == 0) {
                    if ( bt == 2) {
                        if (pass >= 0.2 && srb[2] * srb[0] > pass ) k = 2;
                    }else if (srb[2] * srb[0] > pass) k = 2;
                }
                if ( k == 0 ) {
                    if ((sra[2] > pass && srb[2] < pass) ||
                        (sra[0] > pass && srb[0] < pass)) continue;
                    else if ((sra[2] < pass && srb[2] > pass) ||
                             (sra[0] < pass && srb[0] > pass)) k = 1;
            ...
            if(report == 1) {
                ...
                printf(" NEW  AREA %d RATIOS BIAS %lf NEG %lf MIN %lf PROD %lf\n"
                       " BEST AREA %d RATIOS BIAS %lf NEG %lf MIN %lf PROD %lf\n",
                            jb, srb[0], srb[1], srb[2], srb[0] * srb[2],
                            ja, sra[0], sra[1], sra[2], sra[0] * sra[2]);
            }
            ...
            if (ja != QA0) continue;
            if (  doublet == 1 || k == 2 ||
               (srb[2] > pass && srb[3] > 0.1 && qm->star[v]->nQ != 4)) block = 1;
        }
        //if (block > 0) break;
    }
    if(report == 1) {
...
static int EG_makeValidMesh(meshMap *qm, int nP, /*@null@*/ int *pList,
                            int fullReg)
{
    int    si, v, q, i, j, k, kv, it = 0, itMax, sum = 0, report, *qlist = NULL, kq = 0;
    int    recover = 1, stat = EGADS_SUCCESS, *mv = NULL, *area = NULL;
    double *uvxyz = NULL, pass;
    FILE     *fout = NULL;
    double pos[18], uv[2];
    char buffer[100];

    if (fullReg >= 1) report = 1;
    else report = 0;
#ifdef DEBUG
    report = 1;
#endif
    ...
    pass = -0.1;
    if (fullReg == 0) fullReg = 1;
    for (it = 0 ; it < itMax; it++) {
        if (it > 0 && pass < 0.0) pass = 0.1;
        if (it >= itMax - 4 && fullReg == 1 ) fullReg =   2;
        if (fullReg == 2 && it == itMax -1) pass = 0.25;
        for (j = k = 0 ; k < kv; k++) {
            if (qm->vType[qm->valence[mv[k]][3]-1] == -1 ) continue;
            area[k] = EG_placeVertex(qm,  mv[k] + 1, pass, fullReg, report);
            j      += area[k];
            if ( area[k] == 1) area[k] = QA0;
        }
        for (k = 0 ; k < kv; k++) {
            if (qm->vType[qm->valence[mv[k]][3]-1] != -1 ) continue;
            area[k] = EG_placeVertex(qm,  mv[k] + 1, pass, fullReg, report);
        ...
        if (fullReg == 1 && j == kv)  pass += 0.01;
        if (fullReg >= 1 && report == 1)
            printf(" FULL REG %d it %d -->> TOL %lf PASS %d / %d \n",
                fullReg, it, pass, j , kv);
        if (report == 1) {
        ...
        if (sum == 0) {
            if ( report == 1)
            printf(" YEAH!!!!!!!!!!!! FULL ROUND PASSES WITH GOOD AREAS\n");
            recover = 0 ;
            if (fullReg == 0) break;
        }
    }
    ...
    } else if (sum != 0 && fullReg > 0) stat = EGADS_GEOMERR;
#ifdef DEBUG
                                 ...
                                 int *activity)
{
    ...
    *activity   = 0;
    ...
    if ( i == 1) {
      ...
      id1 = 0;
      id0 = EG_placeVertex(qm, poly[3], 0.0, 0, i);
      if ( qm->vType[poly[0]-1] == -1)
          id1 = EG_placeVertex(qm, poly[0], 0.0, 0, i);
      if (id0 >= QA2) {
          if (id0 >= QA3 ) id0 = 1;
          else {
              for ( id0 = i = 0 ; i < qm->star[poly[3] -1]->nV; i++)
              if (qm->vType[qm->star[poly[3] -1]->verts[i]-1] >= 4) id0 = 1;
          }
      } else id0 = 0;
      if (id1 >= QA2) {
          if (id1 >= QA3 ) id1 = 1;
          else {
              for ( id1 = i = 0 ; i < qm->star[poly[0] -1]->nV; i++)
                  if (qm->vType[qm->star[poly[0] -1]->verts[i]-1] >= 4) id1 = 1;
          }
      }else id1 = 0;
      if (id0 == 0 && id1 == 0) {
        *activity = 1;
...
static int EG_mergeVertices(meshMap *qm, int qC, int centre, int *activity)
{
    ...
    *activity  = 0;
    ...
    if (qm->vType[oldQ[2] - 1] == -1 && doublet == 0 &&
        ...
        EG_placeVertex(qm, oldQ[2], 0.0,  0, 0);
    }
    ...
    *activity  = 1;
    ...
    if ( doublet == 1 ) {
        ...
        if (qm->vType[oldQ[1] -1] == -1)
            EG_placeVertex(qm, oldQ[1], 0.0,  0, 0);
        if (qm->vType[oldQ[3] -1] == -1)
            EG_placeVertex(qm, oldQ[3], 0.0, 0, 0);
        return EGADS_SUCCESS;
    }
                               ...
                               int *transfering, int *activity)
{
    ...
    *activity = 0;
    ...
    if (qm->regBd == 1 && EG_regBoundQuad(qm, qID[0]) == 1) {
#ifdef DEBUG
        printf(" EG_transferValences for quad %d is out of bounds or quad is regular!!\n ", qID[0]);
#endif
        return EGADS_SUCCESS;
    ...
    for (j = 0; j < 4; j++) {
        ...
        if (i == -1 || i == qID[1] ||
            (qm->regBd == 1 && EG_regBoundQuad(qm, i) == 1 &&
             EG_regBoundQuad(qm, qID[0]) == 0)) continue;
        if (*transfering == 0) {
...
#ifdef DEBUG
        printf("Transfer valences thru %d MIN %d REG BD %d  Q %d -- %d  Q %d -- %d \n ", swap, min,
                qm->regBd, qg.q[0], EG_regBoundQuad(qm, qg.q[0]),
                 qg.q[1], EG_regBoundQuad(qm, qg.q[1]) ) ;
                printQuadGroup(qm, qg);
#endif
        stat          = EG_swappingOperation(qm, qg, swap, &(*activity));
        if (stat     != EGADS_SUCCESS || *activity == 0) {
...
static void meshProperties (meshMap *qm, int sweep )
{
    ...
    for (k = 0 ; k < qm->totV; k++) {
        ...
        for ( j = 0; j < qm->star[k]->nQ; j++) {
            ...
            minT = MIN(minT, qm->star[k]-> angles[j]);
            maxT = MAX(maxT, qm->star[k]-> angles[j]);
            minR = MIN(minR, qm->star[k]-> ratios[j]);
            maxR = MAX(maxR, qm->star[k]-> ratios[j]);
            if     (qm->star[k]-> angles[j] < (20. /180. * PI)) quarts[0]++;
            else if(qm->star[k]-> angles[j] < (80. /180. * PI)) quarts[1]++;
            else if(qm->star[k]-> angles[j] < (160./180. * PI)) quarts[2]++;
            else                                                quarts[3]++;
            if     (qm->star[k]-> ratios[j] < 0.1 ) quarts[4]++;
            else if(qm->star[k]-> ratios[j] < 0.25) quarts[5]++;
            else if(qm->star[k]-> ratios[j] < 0.6 ) quarts[6]++;
            else                                    quarts[7]++;
        }
    }
    ...
    for (k = 0 ; k < qm->totQ; k++) {
        ...
        for ( i = 0 ; i < 4; i++) {
            ...
            minT = MIN(minT, qm->star[v[i]]-> angles[j]);
            maxT = MAX(maxT, qm->star[v[i]]-> angles[j]);
            minR = MIN(minR, qm->star[v[i]]-> ratios[j]);
            maxR = MAX(maxR, qm->star[v[i]]-> ratios[j]);
        }
        uv0[0] = qm->uvs[2 * v[0]    ];
    ...
    for (k = 0 ; k < qm->totV; k++) {
        ...
        for ( j = 0; j < qm->star[k]->nQ; j++) {
            ...
            minR = MIN(minR, qm->star[k]-> ratios[j]);
            maxR = MAX(maxR, qm->star[k]-> ratios[j]);
            if     (qm->star[k]-> angles[j] < (15. /180. * PI)) quarts[0]++;
            else if(qm->star[k]-> angles[j] < (40. /180. * PI)) quarts[1]++;
            else if(qm->star[k]-> angles[j] < (155./180. * PI)) quarts[2]++;
            else                                                quarts[3]++;
            if     (qm->star[k]-> ratios[j] < 0.1 ) quarts[4]++;
            else if(qm->star[k]-> angles[j] < 0.25) quarts[5]++;
            else if(qm->star[k]-> angles[j] < 0.6 ) quarts[6]++;
            else                                    quarts[7]++;
        }
    }
    minT *= 180.0 / PI;
            ...
            if (ni > 0 ) {
                ...
                for (ni = i = 0 ; i < qm->totV;i++ ) {
                    ...
                    for ( j = 0 ; j < qm->star[i]->nQ; j++) {
                        if (qm->star[i]->areas[j] > QA2 &&
                                (ni == 0 || inList(ni, &qm->vInv[1], i + 1) == -1)) {
                            qm->vInv[++ni] = i + 1;
                            break;
                        }
                    }
        ...
        if (qm->vInv != NULL && qm->vInv[0] > 0) {
            for (j = 0 ; j < qm->vInv[0]; j++) {
                ...
                for (i = 0; i < qm->star[k]->nQ; i++) {
                    ...
                    stat  = EG_collapse(qm, qm->star[k]->quads[j], &activity, 3, 0);
                    if (stat != EGADS_SUCCESS) {
