...
int solveLU(int nn, double A[], double bb[], double x[] )
{
     int i,j,k;
     double y;
     // Compute the LU decomposition in place
     for(k = 0; k < nn-1; k++) {
         if (A[k*nn+k] == 0) return EGADS_DEGEN;
         for(j = k+1; j < nn; j++) {
             y = A[j*nn+k]/A[k*nn+k];
             for(i = k; i < nn; i++) {
                 A[j*nn+i] = A[j*nn+i]-y*A[k*nn+i];
             }
             A[j*nn+k] = y;
         }
     }
...
static void EG_minL (ego face, double w1, double w2, double w3, double lambda,
                     double mu, double *pO, double *pA, double *pB, double *pC, double *L)
{
    double da, db, dc, crA[3], crB[3], crC[3], duA[3], duB[3], duC[3],dvA[3], dvB[3],
...
static void EG_baricentricSplit(ego face, double w1, double w2, double w3,
                                double *uvA, double *uvB, double *uvC, double *uvOUT)
{
  ...
  double tol, pA[18], pB[18], pC[18], OA[3], OB[3], OC[3], pIT[18], J[16], JI[16],
         ...
         dvA[3], dvB[3], dvC[3], AC[3], BA[3], da, db, dc, aa, bb, cc, det, l[2], tmp;
  ...
  for (it = 0; it < nT; it++) {
    ...
    EG_minL(face, w1, w2, w3, uvIT[2], uvIT[3], pIT, pA, pB, pC, L);
    printf("it %d ----- L %lf %lf %lf %lf  \n",it,  L[0], L[1], L[2], L[3]);
...
static void EG_baricentricSplit2(ego face, double w1, double w2, double w3,
                                double *uvA, double *uvB, double *uvC, double *uvOUT)
{
  int    i, it, nT = 100, stat;
  double tol, pA[18], pB[18], pC[18], OA[3], OB[3], OC[3], pIT[18], J[16], JI[16],
  ...
  for (it = 0; it < nT; it++) {
    ...
    b  = DOT(crB, crB);
    c  = DOT(crC, crC);
    aa = pow(a, 1.5); bb = pow(b, 1.5); cc = pow(c, 1.5);
    a  = sqrt(a); b = sqrt(b); c = sqrt(c);
#ifdef DEBUG
    printf(" A %lf AA %lf SHOULD BE %lf = %lf\n", a,aa, a/b, ka);
    printf(" C %lf CC %lf SHOULD BE %lf = %lf\n", c,cc, c/b, kc);
    d = a + b + c;
    printf("BB %lf it %d  TOT %lf \n\n",bb, it, d);
#endif
    /* Evaluate fn */
    /* a = w1 (a + b + c)
     * b = w2 (a + b + c)
    ...
    L[0] = DOT(duA, crA) * da / a + DOT(duB, crB) * db / b +
           DOT(duC, crC) * dc / c;
    L[1] = DOT(dvA, crA) * da / a + DOT(dvB, crB) * db / b +
           DOT(dvC, crC) * dc / c;
    L[2] = a - ka * b;
    L[3] = c - kc * b;
    tmp  = l[1];
    l[1] = sqrt(L[0] * L[0] + L[1] * L[1] + L[2] * L[2] + L[3] * L[3]);
    #ifdef DEBUG
        printf("it %d ----- L %lf %lf %lf %lf  SIZE %lf OLD %lf \n",it,  L[0], L[1], L[2], L[3], l[1], l[0] );
    #endif
    if ( it < 0 && l[1] > l[0] ) {
      delta[0] *= 0.5;
      delta[1] *= 0.5;
      delta[2] *= 0.5;
      delta[3] *= 0.5;
      uvIT[0]   = uvIT[4] - delta[0];
      uvIT[1]   = uvIT[5] - delta[1];
      uvIT[2]   = uvIT[6] - delta[2];
      uvIT[3]   = uvIT[7] - delta[3];
      i = EG_evaluate(face, uvIT, pIT);
      if (i != EGADS_SUCCESS ) {
    ...
    l[0]  = l[1];
    J[2 ] = DOT(duA, crA) / a - DOT(duB, crB) * ka / b;
    J[3 ] = DOT(duC, crC) / c - DOT(duB, crB) * kc / b;
    J[6 ] = DOT(dvA, crA) / a - DOT(dvB, crB) * ka / b;
    J[7 ] = DOT(dvC, crC) / c - DOT(dvB, crB) * kc / b;
    J[8 ] = J[2];
    J[12] = J[3];
    J[9 ] = J[6];
    J[13] = J[7];
    J[10] = J[11] = 0.0;
    J[14] = J[15] = 0.0;
    printf(" da / a = %.12f da / aa %.12f\t  da / a = %.12f da / aa %.12f  da / a = %.12f da / aa %.12f",
    da / a, da / aa, db / b, db / bb, dc / c, dc / cc);
    J[0]  = DOT(duA, duA) * da / a + DOT(duB, duB) * db / b +
            DOT(duC, duC) * dc / c;
    J[1]  = DOT(duA, dvA) * da / a + DOT(duB, dvB) * db / b +
            DOT(duC, dvC) * dc / c;
    J[5]  = DOT(dvA, dvA) * da / a + DOT(dvB, dvB) * db / b +
            DOT(dvC, dvC) * dc / c;
    J[0] += DOT(duA, crA) * DOT(duA, crA) * da / aa +
            DOT(duB, crB) * DOT(duB, crB) * db / bb +
            DOT(duC, crC) * DOT(duC, crC) * dc / cc;
    J[1] += DOT(duA, crA) * DOT(dvA, crA) * da / aa +
            DOT(duB, crB) * DOT(dvB, crB) * db / bb +
            DOT(duC, crC) * DOT(dvC, crC) * dc / cc;
    J[5] += DOT(dvA, crA) * DOT(dvA, crA) * da / aa +
            DOT(dvB, crB) * DOT(dvB, crB) * db / bb +
            DOT(dvC, crC) * DOT(dvC, crC) * dc / cc;
    grad[0] = pIT[9]; grad[1] = pIT[10]; grad[2] = pIT[11];
    CROSS(grad, CB, duA);
    CROSS(grad, AC, duB);
    CROSS(grad, BA, duC);
    J[0]   += DOT(duA, crA) * da / a + DOT(duB, crB) * db / b +
              DOT(duC, crC) * dc / c;
    grad[0] = pIT[12]; grad[1] = pIT[13]; grad[2] = pIT[14];
    CROSS(grad, CB, duA);
    CROSS(grad, AC, duB);
    CROSS(grad, BA, duC);
    J[1]    += DOT(duA, crA) * da / a + DOT(duB, crB) * db / b +
               DOT(duC, crC) * dc / c;
    grad[0]  = pIT[15]; grad[1] = pIT[16]; grad[2] = pIT[17];
    CROSS(grad, CB, duA);
    CROSS(grad, AC, duB);
    CROSS(grad, BA, duC);
    J[5] += DOT(duA, crA) * da / a + DOT(duB, crB) * db / b +
            DOT(duC, crC) * dc / c;
    J[4]  = J[1];
#ifdef DEBUGGG
    printf(" JACOBIAN MATRIX 4 x 4 \n");
    printf(" %lf %lf %lf %lf\n",J[0], J[1], J[2], J[3]);
    printf(" %lf %lf %lf %lf\n",J[4], J[5], J[6], J[7]);
    printf(" %lf %lf %lf %lf\n",J[8], J[9], J[10], J[11]);
    printf(" %lf %lf %lf %lf\n",J[12], J[13], J[14], J[15]);
...
#ifdef DEBUG
    printf("\n Xn = (%lf %lf %lf %lf )\n L = %lf %lf %lf %lf DELTA %lf %lf %lf %lf SIZE  %1.2e < %1.2e\n",
    uvIT[0], uvIT[1], uvIT[2], uvIT[3], L[0], L[1], L[2], L[3],
   delta[0], delta[1], delta[2], delta[3],x2, tol );
#endif
    i = EG_evaluate(face, uvIT, pIT);
...
static void EG_v3Split(meshMap *qm, int v, double *uvOUT)
{
  int vA, vB, vC;
  vA = qm->star[v-1]->verts[1] - 1;
  vB = qm->star[v-1]->verts[3] - 1;
  vC = qm->star[v-1]->verts[5] - 1;
	EG_baricentricSplit(qm->face, 0.2, 0.35, 0.45, &qm->uvs[2 * vA],
                     &qm->uvs[2 * vB], &qm->uvs[2 * vC],uvOUT);
	return;
}
...
#ifdef DEBUG2
  if ( sumuv != n) {
      printf("%lf %lf %lf GUESS \n", xyz0[0], xyz0[1], xyz0[2]);
      printf("%lf %lf %lf FIND  \n", xyz1[0], xyz1[1], xyz1[2]);
  }
#endif
                          ...
                          double *normal, double *angle)
{
  ...
  double cross[3],  r[2], xyz[18], v01[3],
         v02[3], norm1, norm2, c, uv[2], dot, p2[18], p1[18];
  ...
  *angle = 0.0;
  ...
  r[0]      = r[1] = 0.0;
  p1[0] = qm->xyzs[3 * (v1 - 1)    ];
  p1[1] = qm->xyzs[3 * (v1 - 1) + 1];
  p1[2] = qm->xyzs[3 * (v1 - 1) + 2];

  p2[0] = qm->xyzs[3 * (v2 - 1)    ];
  p2[1] = qm->xyzs[3 * (v2 - 1) + 1];
  p2[2] = qm->xyzs[3 * (v2 - 1) + 2];
  if (sing == 0 && qm->fin >= 1) {
      if (t1 + tc <= -1) {
          EG_getSidepoint(qm->face, 0.5, &qm->uvs[2 * (vC - 1)], &qm->uvs[2 * (v1 - 1)], uv);
          EG_evaluate(qm->face, uv, p1);
          dot  = normal[0] * p1[0] + normal[1] * p1[1] + normal[2] * p1[2];
          r[0] = (c - dot);
      }
      if (t2 + tc <= -1) {
        EG_getSidepoint(qm->face, 0.5, &qm->uvs[2 * (vC - 1)], &qm->uvs[2 * (v2 - 1)], uv);
        EG_evaluate(qm->face, uv, p2);
        dot  = normal[0] * p2[0] + normal[1] * p2[1] + normal[2] * p2[2];
        r[1] = (c - dot);
    }
  }
  // re-center at vertex
  v01[0] = p1[0] + r[0] * normal[0] - qm->xyzs[3 * (vC - 1)    ];
  v01[1] = p1[1] + r[0] * normal[1] - qm->xyzs[3 * (vC - 1) + 1];
  v01[2] = p1[2] + r[0] * normal[2] - qm->xyzs[3 * (vC - 1) + 2];
  v02[0] = p2[0] + r[1] * normal[0] - qm->xyzs[3 * (vC - 1)    ];
  v02[1] = p2[1] + r[1] * normal[1] - qm->xyzs[3 * (vC - 1) + 1];
  v02[2] = p2[2] + r[1] * normal[2] - qm->xyzs[3 * (vC - 1) + 2];
      ...
      for (q = 0; q <= nt; q++) {
          if (i0 == 1) {
              ...
              if (q == 0) {
                uv[2] = uv[0];
                uv[3] = uv[1];
                if (ta[0] == QA0) EG_v3Split(qm, v+1, &uv[2]);
...
printf(" DOMINANT ANGLE %lf MAX ANGLE %lf\n", errbd, auxd);
  ...
  printf(" j %d AREA %d RATIO %lf ERRBD %lf res %lf %lf %lf\n",
          ...
          if (pass == -1 ) {
              if ((bt <= 2 &&  ta[1] == QA0) ||
                  (bt <= 0 && (ta[1]   < QA2 || (qm->pp == 1 && ta[1] <= QA2)))) {
                      k     = 2;
                      block = 1;
	          }
          }
  ...
  for (k = 0 ; k < kv; k++) {
      ...
      for (i = 0; i < qm->star[mv[k]]->nV; ++i) {
          ...
          if (qm->vType[si] >= 0) {
              ...
              for (ii = 0 ; ii < qm->star[si]->nV; ii++) {
                  v   = qm->star[si]->verts[ii] - 1;
                  if (qm->vType[v] >= 0) {
  ...
  if (i != QACB && j != QACB) {
      if (i != QA0 || j != QA0) {
          if (qm->vType[qg.verts[0]-1] == -1 )
              EG_placeVertex(qm, qg.verts[0],1,  0);
          if (qm->vType[qg.verts[3]-1] == -1 )
              EG_placeVertex(qm, qg.verts[3],1,  0);
          i = EG_vertexArea(qm, qg.verts[0]);
          j = EG_vertexArea(qm, qg.verts[3]);
      }
      if (i + j < QA3 && EG_makeValidMesh(qm, 6, qg.verts, 0) == EGADS_SUCCESS) {
          EG_free(quad);
          return EGADS_SUCCESS;
      }
  } else {
  ...
  for (v = 0; v < 4; v++) {
    ...
    if (validSplit == 1)  {
      stat = EG_splittingOperation(qm, qm->star[si]->verts[0], qm->star[si]->verts[id0],
                                   ...
                                   &(*activity));
      if (stat == EGADS_SUCCESS && *activity == 0 && qm->star[si]->nQ == 6) {
          id0 += 2;
          id6[0] = qm->star[si]->verts[qm->star[si]->idxV[id0       ]] - 1;
          id6[1] = qm->star[si]->verts[qm->star[si]->idxV[id0 + dist]] - 1;
          if (qm->vType  [id6[0]]    * qm->vType  [id6[1]]    ==  1 &&
              qm->valence[id6[0]][2] * qm->valence[id6[1]][2] <= 16)
              stat = EG_splittingOperation(qm, qm->star[si]->verts[0],
                                           id6[0] + 1, id6[1] + 1, &(*activity));
          if (*activity == 0) {
              id0 += 2;
              id6[0] = qm->star[si]->verts[qm->star[si]->idxV[id0       ]] - 1;
              id6[1] = qm->star[si]->verts[qm->star[si]->idxV[id0 + dist]] - 1;
...
{
  int j, k, i, v[4], quarts[8], n, Nx = 10;
  //double minT, maxT, minR, maxR, uv0[2], uv1[2], uv3[2], p[18];
  double minT, maxT, minR, maxR, x0[3], x1[3], x2[3],x3[3], p[18], dt;
  FILE *fil =  NULL;
  char buffer[100];
  ...
  for (k = 0 ; k < qm->totQ; k++) {
      ...
      x0[0] = qm->xyzs[3 * v[0]    ];
      x0[1] = qm->xyzs[3 * v[0] + 1];
      x0[2] = qm->xyzs[3 * v[0] + 2];

      x1[0] = qm->xyzs[3 * v[1]    ];
      x1[1] = qm->xyzs[3 * v[1] + 1];
      x1[2] = qm->xyzs[3 * v[1] + 2];

      x2[0] = qm->xyzs[3 * v[2]    ];
      x2[1] = qm->xyzs[3 * v[2] + 1];
      x2[2] = qm->xyzs[3 * v[2] + 2];

      x3[0] = qm->xyzs[3 * v[3]    ];
      x3[1] = qm->xyzs[3 * v[3] + 1];
      x3[2] = qm->xyzs[3 * v[3] + 2];
      for (n = 1; n < Nx; n++) {
          dt = (double)n / (double)Nx;
          p[0] = x0[0] + (x3[0] - x0[0]) * dt;
          p[1] = x0[1] + (x3[1] - x0[1]) * dt;
          p[2] = x0[2] + (x3[2] - x0[2]) * dt;
          fprintf(fil,"%f %f %f %f %f %f %f %d \n",
                  p[0], p[1], p[2], minT* 180.0 / PI,
                  maxT* 180.0 / PI, minR, maxR, k + 1);
          p[0] = x1[0] + (x2[0] - x1[0]) * dt;
          p[1] = x1[1] + (x2[1] - x1[1]) * dt;
          p[2] = x1[2] + (x2[2] - x1[2]) * dt;
          fprintf(fil,"%f %f %f %f %f %f %f %d \n",
                  p[0], p[1], p[2], minT* 180.0 / PI,
                  maxT* 180.0 / PI, minR, maxR, k + 1);
      }
      for (n = 1; n < Nx; n++) {
          dt = (double)n / (double)Nx;
          p[0] = x0[0] + (x1[0] - x0[0]) * dt;
          p[1] = x0[1] + (x1[1] - x0[1]) * dt;
          p[2] = x0[2] + (x1[2] - x0[2]) * dt;
          fprintf(fil,"%f %f %f %f %f %f %f %d \n",
                  p[0], p[1], p[2], minT* 180.0 / PI,
                  maxT* 180.0 / PI, minR, maxR, k + 1);
          p[0] = x3[0] + (x2[0] - x3[0]) * dt;
          p[1] = x3[1] + (x2[1] - x3[1]) * dt;
          p[2] = x3[2] + (x2[2] - x3[2]) * dt;
          fprintf(fil,"%f %f %f %f %f %f %f %d\n\n\n",
                  p[0], p[1], p[2], minT* 180.0 / PI,
                  maxT* 180.0 / PI, minR, maxR, k + 1);
      }
