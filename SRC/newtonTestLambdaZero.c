...
// min 1/2 ( l_0 ^ 2 + l_1 ^ 2) using l1 = (1-k)/k l0
// grad (L) = (L_1, L_2) = (0, 0) --> solution using Newton x_n+1 = x_n + delta_n (2x2 system)
static int EG_minArcFact(ego face, double fact, double *uv0, double *uv1, double *uv)
{
  int    stat, it, nT = 100;
  double tol = 1.e-14, l[2], p0[18], p1[18], pn[18], J[2][2], ATJ[2][2],
         r[3], b, delta[2], F[2], detJ, x0, x1, x2,  e1, e2, uvn[2], q01[3], kp, k0, k1;

  fact = 0.25;
  kp   = 2.0 * fact * fact + 1.0 - 2.0 * fact;
  k0   =     - fact * fact;
  k1   = 2.0 * fact - fact * fact - 1.0;
  //Initial guess uv = 0.5 (uv0 + uv1)
  //uvn[0] = (1.0 - fact) * uv0[0] + fact * uv1[0];
  //uvn[1] = (1.0 - fact) * uv0[1] + fact * uv1[1];
  uvn[0] = 0.25 * uv0[0] + 0.75 * uv1[0];
  uvn[1] = 0.25 * uv0[1] + 0.75 * uv1[1];
  stat   = EG_evaluate(face, uv0, p0);
  stat  += EG_evaluate(face, uv1, p1);
  stat  += EG_evaluate(face, uvn, pn);

  printf("FACE ORIENTATION %d\n", face->mtype);
#ifdef DEBUG
  r[0]  = pn[0] - p0[0];
  r[1]  = pn[1] - p0[1];
  r[2]  = pn[2] - p0[2];
  ...
  l[1]  = sqrt(DOT(r, r));
  b     = l[0] + l[1];
  printf(" FACT %lf Initial approximation ||PM - P0|| = %lf ||PM - P1|| = %lf ratio %lf %lf (EXPECTED %lf %lf)\n",
           fact, l[0], l[1], l[0] / b, l[1]/b, fact, 1.0 - fact);
  printf("%lf %lf %lf P0\n", p0[0], p0[1], p0[2]);
  printf("%lf %lf %lf P1\n", p1[0], p1[1], p1[2]);
  printf("%lf %lf %lf Pn\n", pn[0], pn[1], pn[2]);
   #endif
  ...
  q01[0] = k0 / kp * p0[0] + k1 / kp * p1[0];
  q01[1] = k0 / kp * p0[1] + k1 / kp * p1[1];
  q01[2] = k0 / kp * p0[2] + k1 / kp * p1[2];
  for (it = 0; it < nT; it++) {
  ...
  pn[12], pn[13], pn[14], pn[15], pn[16], pn[17]);
      ...
      printf(" R %lf %lf  %lf \n", r[0], r[1], r[2]);
      b       =       r[0] * pn[9 ] +  r[1] * pn[10] +  r[2] * pn[11];  //J = Puu * R + PuPu
      J[0][0] = b + (pn[3] * pn[3 ] + pn[4] * pn[4 ] + pn[5] * pn[5 ]);

      b       =       r[0] * pn[12] +  r[1] * pn[13] +  r[2] * pn[14];  //J = Puv * R + PuPv
      J[0][1] = b + (pn[3] * pn[6 ] + pn[4] * pn[7 ] + pn[5] * pn[8 ]);

      J[1][0] = J[0][1];

      b       =       r[0] * pn[15] +  r[1] * pn[16] +  r[2] * pn[17];  //J = Pvv * R + PvPv
      J[1][1] = b + (pn[6] * pn[6 ] + pn[7] * pn[7 ] + pn[8] * pn[8 ]);

      // Solve Linear System: J * delta = - f_n
      // Invert Jacobian directly --> delta = J^-1 f_n
      ...
      ATJ[0][0] =   J[1][1];
      if (fabs(ATJ[0][0]) < EPS) ATJ[0][0] = 0.0;
      ATJ[0][1] = - J[0][1];
      if (fabs(ATJ[0][1]) < EPS) ATJ[0][1] = 0.0;
      ATJ[1][0] = - J[1][0];
      if (fabs(ATJ[1][0]) < EPS) ATJ[1][0] = 0.0;
      ATJ[1][1] =   J[0][0];
      if (fabs(ATJ[1][1]) < EPS) ATJ[1][1] = 0.0;
      ...
      F[0]     =  r[0] * pn[3] + r[1] * pn[4] + r[2] * pn[5];
      F[1]     =  r[0] * pn[6] + r[1] * pn[7] + r[2] * pn[8];
      detJ     =   1.0 / detJ;
      delta[0] = -detJ * (ATJ[0][0] * F[0] + ATJ[0][1] * F[1]);
      delta[1] = -detJ * (ATJ[1][0] * F[0] + ATJ[1][1] * F[1]);
      uvn[0]  += delta[0];
      uvn[1]  += delta[1];
      ...
      if      (it == 0) x0 = x2;
          ...
          e2 = fabs(x2 / x1);
#ifdef DEBUG
          printf("IT %d (xn - xn-1)/(xn-1 - xn-2) %lf (xn+1 - xn)/(xn - xn-1) %lf --> CONVERGENCE %lf \n",
#endif
          it, e1, e2, log(e2) / log(e1));
          x0 = x1;
          x1 = x2;
      }
      stat = EG_evaluate(face, uvn, pn);
      ...
      uvn[0], uvn[1], F[0], F[1], delta[0], delta[1], x2, tol );
#endif
  }
  uv[0] = uvn[0];
  uv[1] = uvn[1];
  ...
  printf(" Leaving frac solver with final chords ||PM - P0|| = %lf ||PM - P1|| = %lf\n ratio %lf %lf (EXPECTED %lf %lf)\n",
           l[0], l[1], l[0] / (b * b), l[1]/(b * b), fact * fact, (1.0 - fact) * (1.0 - fact) );
  l[0] = sqrt(l[0]);
  l[1] = sqrt(l[1]);
  b    = l[0] + l[1];
  printf(" Leaving frac solver with final chords ||PM - P0|| = %lf ||PM - P1|| = %lf\n ratio %lf %lf (EXPECTED %lf %lf)\n",
...
static void EG_minArcV(ego face, double *uvf, double *uv)
{
  int    i, it, nT = 100, stat;
  double tol, xyzs[4 * 3], p[18], pn[18], J[2][2], ATJ[2][2], r[4 * 3],
         l[4],  q[3], delta[2], L[2], detJ, x0, x1, x2,  e1, e2, uvn[2];

  //Initial guess uv = 0.5 (uv0 + uv1)
  uvn[0] = uvn[1] = 0.0;
  for (i = 0; i < 4; i++) {
      uvn[0] += 0.25 * uvf[2 * i    ];
      uvn[1] += 0.25 * uvf[2 * i + 1];
      stat    = EG_evaluate(face, &uvf[2 * i], p);
      if (stat != EGADS_SUCCESS) {
          printf(" EG_minArcFactN stat %d is %d!!\n", i + 1, stat);
          return;
      }
      xyzs[3 * i    ] = p[0];
      xyzs[3 * i + 1] = p[1];
  ...
  printf(" REG QUAD INITIAL GUESS %lf %lf \n", uvn[0], uvn[1]);
  printf("%lf %lf %lf %d\n"    , pn  [0], pn[1], pn[2], 4);
  printf("%lf %lf %lf %d\n\n\n", xyzs[0], xyzs[1], xyzs[2], 0);
  printf("%lf %lf %lf %d\n"    , pn  [0], pn[1], pn[2], 4);
  printf("%lf %lf %lf %d\n\n\n", xyzs[3], xyzs[4], xyzs[5], 1);
  printf("%lf %lf %lf %d\n"    , pn  [0], pn[1], pn[2], 4);
  printf("%lf %lf %lf %d\n\n\n", xyzs[6], xyzs[7], xyzs[8], 2);
  printf("%lf %lf %lf %d\n"    , pn  [0], pn[1], pn[2], 4);
  printf("%lf %lf %lf %d\n\n\n", xyzs[9], xyzs[10], xyzs[11], 3);
  ...
  pn[12], pn[13], pn[14], pn[15], pn[16], pn[17]);*/
      ...
      q[0]  = 4.0 * pn[0] - xyzs[0] - xyzs[3] - xyzs[6] - xyzs[9 ];
      q[1]  = 4.0 * pn[1] - xyzs[1] - xyzs[4] - xyzs[7] - xyzs[10];
      q[2]  = 4.0 * pn[2] - xyzs[2] - xyzs[5] - xyzs[8] - xyzs[11];

      L[0]    = pn[3] * q[0] + pn[4] * q[1] + pn[5] * q[2];
      L[1]    = pn[6] * q[0] + pn[7] * q[1] + pn[8] * q[2];

      J[0][0] = pn[9 ] * q[0] + 4.0 * pn[3] * pn[3] +
                pn[10] * q[1] + 4.0 * pn[4] * pn[4] +
                pn[11] * q[2] + 4.0 * pn[5] * pn[5];

      J[0][1] = pn[12] * q[0] + 4.0 * pn[3] * pn[6] +
                pn[13] * q[1] + 4.0 * pn[4] * pn[7] +
                pn[14] * q[2] + 4.0 * pn[5] * pn[8];

      J[1][0] = J[0][1];

      J[1][1] = pn[15] * q[0] + 4.0 * pn[6] * pn[6] +
                pn[16] * q[1] + 4.0 * pn[7] * pn[7] +
                pn[17] * q[2] + 4.0 * pn[8] * pn[8];
  ...
  for (i = 0; i < 4; i++) {
      r[3 * i    ] = pn[0] - xyzs[3 * i    ];
      r[3 * i + 1] = pn[1] - xyzs[3 * i + 1];
      r[3 * i + 2] = pn[2] - xyzs[3 * i + 2];
      l[i]         = r[3 * i    ] * r[3 * i    ] +
                     r[3 * i + 1] * r[3 * i + 1] +
                     r[3 * i + 2] * r[3 * i + 2];
#ifdef DEBUG
      printf("-------------   ARC LENGTHS L %d %lf  \n", i, l[i]);
#endif
  }
...
static void EG_minArcRegV(ego face, double *uvf, double *uv)
{
  int    i, it, nT = 100, stat;
  double tol, xyzs[4 * 3], p[18], pn[18], J[2][2], ATJ[2][2], r[4 * 3], k[3],
         l[4], l02, l13, q02[3], q13[3], s[3], delta[2], L[2], detJ, x0, x1, x2,  e1, e2, uvn[2];
  ...
  if (EG_evaluate(face, uvn, pn) != EGADS_SUCCESS) {
   printf(" EG_minArc :: EG_evaluate %d !!\n ", i);
   return;
  }
  ...
  pn[12], pn[13], pn[14], pn[15], pn[16], pn[17]);*/
      ...
      l02     = r[0] * r[0] + r[1] * r[1] + r[2] * r[2] ;
      l13     = r[3] * r[3] + r[4] * r[4] + r[5] * r[5] ;

      q02[0]  = 2.0 * pn[0] - xyzs[0] - xyzs[6 ];
      q02[1]  = 2.0 * pn[1] - xyzs[1] - xyzs[7 ];
      q02[2]  = 2.0 * pn[2] - xyzs[2] - xyzs[8 ];

      q13[0]  = 2.0 * pn[0] - xyzs[3] - xyzs[9 ];
      q13[1]  = 2.0 * pn[1] - xyzs[4] - xyzs[10];
      q13[2]  = 2.0 * pn[2] - xyzs[5] - xyzs[11];

      s[0]    = l02 * q02[0] + l13 * q13[0];
      s[1]    = l02 * q02[1] + l13 * q13[1];
      s[2]    = l02 * q02[2] + l13 * q13[2];

      L[0]    = pn[3] * s[0] + pn[4] * s[1] + pn[5] * s[2];
      L[1]    = pn[6] * s[0] + pn[7] * s[1] + pn[8] * s[2];

      k[0]    = l02 + l13 + q02[0] * (pn[0] - xyzs[0]) + q13[0] * (pn[0] - xyzs[3]);
      k[1]    = l02 + l13 + q02[1] * (pn[1] - xyzs[1]) + q13[1] * (pn[1] - xyzs[4]);
      k[2]    = l02 + l13 + q02[2] * (pn[2] - xyzs[2]) + q13[2] * (pn[2] - xyzs[5]);

      J[0][0] = pn[9 ] * s[0] + 2.0 * pn[3] * pn[3] * k[0] +
                pn[10] * s[1] + 2.0 * pn[4] * pn[4] * k[1] +
                pn[11] * s[2] + 2.0 * pn[5] * pn[5] * k[2];

      J[0][1] = pn[12] * s[0] + 2.0 * pn[3] * pn[6] * k[0] +
                pn[13] * s[1] + 2.0 * pn[4] * pn[7] * k[1] +
                pn[14] * s[2] + 2.0 * pn[5] * pn[8] * k[2];

      J[1][0] = J[0][1];

      J[1][1] = pn[15] * s[0] + 2.0 * pn[6] * pn[6] * k[0] +
                pn[16] * s[1] + 2.0 * pn[7] * pn[7] * k[1] +
                pn[17] * s[2] + 2.0 * pn[8] * pn[8] * k[2];
  ...
  #ifdef DEBUG
  printf(" LEAVING WITH FINAL ARCS \n");
  // arcs for direction l02
  printf(" ||Pn - P0|| = %lf ||Pn - P2|| = %lf RATIOS %lf %lf\n",
  l[0], l[2], l[0] / l02, l[2] / l02);
  // arcs for direction l13
  printf(" ||Pn - P1|| = %lf ||Pn - P3|| = %lf RATIOS %lf %lf\n",
  l[1], l[3], l[1] / l13, l[3] / l13);
...
void
EG_minArc(const ego face, double fact, const double *uvm, const double *uvp, double *uv)
{
    int    i, it, nT = 100, stat;
    double tol, l[2], p0[18], p1[18], pn[18], J[2][2], ATJ[2][2],
    r[3], b, delta[2], L[2], detJ, x0, x1, x2,  e1 = 0.0, e2 = 0.0, uvn[2];

    //Initial guess uv = 0.5 (uv0 + uv1)
    uvn[0] =  0.5 * (uvm[0] + uvp[0]);
    uvn[1] =  0.5 * (uvm[1] + uvp[1]);
    i      = EG_evaluate(face, uvm, p0);
    i     += EG_evaluate(face, uvp, p1);
    i     += EG_evaluate(face, uvn, pn);
    if (i != EGADS_SUCCESS) {
        printf(" EG_minArc :: EG_evaluate %d !!\n ", i);
        return;
...
#ifdef DEBUG
    printf("Looking for midpoint between (%lf %lf %lf) and (%lf %lf %lf)  \n",
    p0[0], p0[1], p0[2], p1[0], p1[1], p1[2]);
    printf(" INITIAL GUESS %lf %lf %lf\n", pn[0], pn[1], pn[2]);
    r[0] = pn[0] - p0[0];
    r[1] = pn[1] - p0[1];
    r[2] = pn[2] - p0[2];
    l[0] = sqrt(r[0] * r[0] + r[1] * r[1] + r[2] * r[2]);

    r[0] = pn[0] - p1[0];
    r[1] = pn[1] - p1[1];
    r[2] = pn[2] - p1[2];
    l[1] = sqrt(r[0] * r[0] + r[1] * r[1] + r[2] * r[2]);
    printf("------------- INIT ARC LENGTHS %lf %lf  \n",
    l[0], l[1]);
#endif // DEBUG

    tol = 1.e-10; //pretty useless
    for (it = 0; it < nT; it++) {
#ifdef DEBUGG
        printf(" IT %d NEW POINT \n %d %lf %lf %lf %lf %lf\n",
        pn[0], pn[1], pn[2], uvn[0], uvn[1]);
        printf(" du  %lf %lf %lf\n dv  %lf %lf %lf\n"
        " duu %lf %lf %lf\n duv %lf %lf %lf\n"
        " dvv %lf %lf %lf\n",   pn [3], pn [4], pn[5],
        ...
        pn[12], pn[13], pn[14], pn[15], pn[16], pn[17]);
#endif
        r[0]    =  2.0 * pn[0] - p1[0] - p0[0];
        r[1]    =  2.0 * pn[1] - p1[1] - p0[1];
        r[2]    =  2.0 * pn[2] - p1[2] - p0[2];
        L[0]    = r[0] * pn[3] + r[1] * pn[4] + r[2] * pn[5];
        L[1]    = r[0] * pn[6] + r[1] * pn[7] + r[2] * pn[8];

        b       =             r[0] * pn[9 ] +  r[1] * pn[10] +  r[2] * pn[11];
        J[0][0] = b + 2.0 * (pn[3] * pn[3 ] + pn[4] * pn[4 ] + pn[5] * pn[5 ]);

        b       =             r[0] * pn[12] +  r[1] * pn[13] +  r[2] * pn[14];
        J[0][1] = b + 2.0 * (pn[3] * pn[6 ] + pn[4] * pn[7 ] + pn[5] * pn[8 ]);

        J[1][0] = J[0][1];

        b       =             r[0] * pn[15] +  r[1] * pn[16] +  r[2] * pn[17];
        J[1][1] = b + 2.0 * (pn[6] * pn[6 ] + pn[7] * pn[7 ] + pn[8] * pn[8 ]);
        ...
        if (stat != EGADS_SUCCESS || x2 < tol ) {
#ifdef DEBUG
            printf("IT %d Newton enter tol: EG_evaluate %d  DELTA SIZE %1.2e < %1.2e \n",
                    it, stat, x2, tol);
            if (e1 > tol && e2 >tol)
            printf("(xn - xn-1)/(xn-1 - xn-2) %lf (xn+1 - xn)/(xn - xn-1) %lf CONVERGENCE %lf \n",
            e1, e2, log(e2) / log(e1));
#endif
            break;
        }
#ifdef DEBUGG
        printf(" it %d --xn %lf %lf L %lf %lf DELTA %lf %lf   SIZE  %1.2e < %1.2e\n",
                 it, uvn[0], uvn[1], L[0], L[1], delta[0], delta[1], x2, tol);
#endif
    }
    uv[0] = uvn[0];
    uv[1] = uvn[1];
    ...
    r[2] = pn[2] - p1[2];
    l[1] = sqrt(r[0] * r[0] + r[1] * r[1] + r[2] * r[2]);
    printf("------------- FINAL INIT ARC LENGTHS %lf %lf  \n",
    l[0], l[1]);
#endif
  return ;
}
...
{
  int i, sum = 0;
  double  xyz0[18], xyz1[18], uv[2 * 4];

  xyz0[0] = qm->uvs[2 * (list[0] - 1)    ];
  xyz0[1] = qm->uvs[2 * (list[0] - 1) + 1];
  i       = 1;
  if (n > 2 ) i++;
  xyz1[0] = qm->uvs[2 * (list[i] - 1)    ];
  xyz1[1] = qm->uvs[2 * (list[i] - 1) + 1];
  EG_minArc(qm->face, 0.5, xyz0, xyz1, quv);
  return EGADS_SUCCESS;
  if ( n == 4 ) {
      uv[0] = qm->uvs[2 * (list[0] - 1)    ];
      uv[1] = qm->uvs[2 * (list[0] - 1) + 1];
      uv[2] = qm->uvs[2 * (list[1] - 1)    ];
      uv[3] = qm->uvs[2 * (list[1] - 1) + 1];
      uv[4] = qm->uvs[2 * (list[2] - 1)    ];
      uv[5] = qm->uvs[2 * (list[2] - 1) + 1];
      uv[6] = qm->uvs[2 * (list[3] - 1)    ];
      uv[7] = qm->uvs[2 * (list[3] - 1) + 1];
      EG_minArcV(qm->face, uv, quv);
      return EGADS_SUCCESS;
  } else EG_minArcFact(qm->face,0.3,  xyz0, xyz1, quv);
  return EGADS_SUCCESS;
